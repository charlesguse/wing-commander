...
# jq's Windows CRLF shim (#446) is ensure_jq()'s own job now -- it is called
# before _bindir() in both run_sweep() and run_window() below, and shims the
# real jq on PATH itself (#459) rather than leaving each caller to notice.
# See verify-metrics-persist-retry.py's case_jq_cr_shim_has_exactly_one_home.
def _bindir(work):
