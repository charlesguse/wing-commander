...
def _validate_string(value, spec, where):
    ...
    try:
        ...
        # #583: a lone surrogate (JSON "\ud800") parses but cannot be
        # written back out as UTF-8.
        return "{0} is not valid Unicode text (a lone surrogate)".format(where)
...
# and a lone surrogate in `what`, which cannot be encoded as UTF-8.
# ----------------------------------------------------------------------------
...
def _fixture_files():
    ...
    if len(found) != 9:
        sys.exit("::error::board-review-finding-schema: expected exactly 9 "
