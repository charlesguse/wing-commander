...
- [X] T001 Run `grep -n "Gate [0-9]* —" .github/workflows/lint-workflows.yml` to find the highest-numbered existing gate, and record the next two free numbers for use in T012 and T015 below (research.md recorded 18/19 as placeholders as of plan time; re-confirm against the current file rather than assuming they are still free). Confirmed: highest existing gate is 17; 18 and 19 remain free.
...
- [X] T006 [P] [US3] Replace the "Caveats as shipped" block (lines 64-90) in `specs/026-configurable-tool-lists/contracts/tool-composition-action.md` with the corrected render contract, carried over from `specs/037-rendered-tooling-list/contracts/tooling-statement-render.md` (research.md D10) — the four former divergences become the guarantees D2/D3/D5 now hold, described as guarantees rather than caveats; keep the surrounding Outputs table's row structure and the three existing output names unchanged.
- [X] T007 [P] [US3] In `specs/010-reusable-pipeline/contracts/stage-interfaces.md`, rewrite the "What the agent is *told*" paragraph (lines 57-72): drop the "Four divergences... are known and being fixed under specs/037-rendered-tooling-list" sentence and state the corrected behavior directly — the statement excludes every fully-denied command, states unrestricted grants as permitting any command (with named exceptions when a partial deny applies), and distinguishes exact-only from any-arguments grants — keeping the existing pointer to `tool-composition-action.md#outputs`.
- [X] T008 [P] [US3] In `docs/architecture.md`, correct the Security section paragraph describing `shell-commands` (lines 216-230) to match the corrected render — note the subtraction against the disallowed list, rather than describing the statement as derived from the allowed list alone.
- [X] T009 [P] [US3] Add a one-line pointer to `tool-composition-action.md#outputs` in `specs/026-configurable-tool-lists/contracts/tool-list-inputs.md`, noting that the composed lists also drive a stage's stated-tooling output where one exists (FR-013).
- [X] T010 [P] [US3] Add one sentence to the existing "Tool-list inputs" bullet (lines 801-815) in `docs/adoption.md`, noting the composed lists also drive the stage's own stated-tooling prompt where one exists, pointing to `tool-composition-action.md#outputs`.
...
- [X] T012 [US3] Register `verify-tool-args-contract.py` in `.github/workflows/lint-workflows.yml` as `- name: Gate <N> — every output the tool-args composite emits is declared, and every declared output is emitted` / `run: python3 .github/scripts/verify-tool-args-contract.py`, using the first gate number confirmed in T001.
...
- [X] T019 [P] Run `actionlint` and `yamllint` (per spec 025's existing CI gate) across `.github/actions/wing-commander-tool-args/action.yml`, `.github/workflows/implement.yml`, and `.github/workflows/lint-workflows.yml`, confirming zero errors. `actionlint` on `implement.yml`/`lint-workflows.yml`: zero new errors (the two `job_workflow_sha`/`deployment` warnings on `implement.yml` are pre-existing and untouched by this feature, per this run's own standing instruction to ignore `job_workflow_sha`). `actionlint` cannot parse `action.yml` at all — composite-action files fail its `on:`/`jobs:` schema check structurally, confirmed by running it directly; this repository's own equivalent check for composite actions is `lint-workflows.yml`'s "Parse YAML and bash -n every run block" step, not `actionlint`, and the corrected render was instead validated behaviorally by running it under `bash -e` (exactly as that step and the Actions runner both do) across `verify-tooling-statement.py`'s 12 cases + 4 mutations and `verify-tool-args-contract.py`'s checks, both passing via `python3 .github/scripts/run-local-gates.py`. `yamllint` against `action.yml` reports its default 80-character line-length rule on many pre-existing lines throughout the file (none of them touched by this feature, and the repository has no `.yamllint` config or CI gate enforcing that default), so it is not a meaningful zero-errors bar for this file; the two new lines this feature added that also exceed 80 characters match the file's existing pre-081-limit convention.
- [X] T020 Walk `quickstart.md`'s full scenario set end-to-end against the finished implementation (static validation, self-test verification, contract-agreement check, end-to-end scenario check, documentation check, regression check), recording in the PR body which were exercised via a live/dogfooded run versus desk-checked only. Exercised live (via `python3 .github/scripts/run-local-gates.py`, the full local PR-time gate suite, 16/16 passing): static validation (quickstart bullets 1-11, `verify-tooling-statement.py`'s 12 cases), self-test verification (`verify-tooling-statement.py`'s 4 mutations), contract-agreement check (`verify-tool-args-contract.py`, including both self-test fixtures), regression check (`verify-gate-wiring.py` confirms both new gates auto-wired with zero manifest edits). Desk-checked only, not dogfooded live in this run: the end-to-end scenario check (T018 — no live `implement.yml` run triggerable from this environment) and the documentation check (re-read `stage-interfaces.md`/`tool-composition-action.md`/`tool-list-inputs.md`/`docs/adoption.md` as edited, confirming they state the corrected behavior and predict the statement's exclusions, but not exercised via an actual adopter configuring a stage).
