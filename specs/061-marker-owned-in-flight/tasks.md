...
- [ ] Fix `in_flight_candidate()` in `.github/scripts/board_eligibility.py` so a marker recording the `prove` step (written with `pr=None` at both `board-loop.yml:2703` and `:2715`) is not silently dropped from in-flight candidacy by `int(None)` raising `TypeError` at `board_eligibility.py:170` (caught by the existing `except (TypeError, ValueError): continue`). `prove` is non-terminal (not in `TERMINAL_STEPS`) but its marker never carries a `pr`, so `FIX_OR_LATER_STEPS` membership can never be satisfied for it today -- treat it like the pre-fix steps (qualify on step alone) or otherwise handle `pr=None` explicitly, and add a fixture case to `.github/scripts/tests/board-eligibility/in-flight/` covering it (extends FR-012's coverage).
- [ ] Add `gh label create "board:owned" --color 0E8A16 --description "Board loop: this PR was opened by board-loop.yml" --force` immediately before the `gh pr create ... --label board:owned` call in board-loop.yml's "Push and open the PR" step (~line 1454), so the call does not 404 against a label that does not yet exist in the live repository (confirmed via `gh api repos/charlesguse/wing-commander/labels/board:owned` -> 404).
- [ ] Fix the FR-007 resume fallback's `gh api ... --paginate --jq '{number, body}'` call (board-loop.yml:407-411): the `/issues` endpoint returns a JSON array per page, so `{number, body}` must be applied per-element (`--jq '.[] | {number, body}'` piped to `jq -s '.'`), matching the stream-then-slurp shape this file already uses elsewhere (the state-normalizer calls) and documented in `specs/033-pr-conversation-commands/quickstart.md` T067. The trailing `2>/dev/null` currently hides this failure entirely, so `fallback_number` is always empty.
- [ ] Fix `read_marker_with_timestamp()` (`.github/scripts/board_item_marker.py:19-43`) so an unparsable *newest* marker degrades the whole result to `None`, per its own docstring and FR-054's re-derivation guarantee, instead of silently returning the newest *parsable* older comment's marker.

## Maintainer Feedback (continued)

- [ ] Change `set -uo pipefail` to `set -euo pipefail` on the "Push and open the PR" step (board-loop.yml:1449) so a `gh pr create` failure (e.g. the missing `board:owned` label) fails the step immediately at its source instead of cascading into a confusing downstream error from `write_marker`/`int(os.environ['PR_NUMBER'])`.
- [ ] Stop leaking a stale `pr_number`/`pr_state` into the `step=triage` output path (board-loop.yml:440) when the marker-named PR resolves but is disqualified -- clear both before falling through to triage, so the step summary/metrics record does not carry misleading values.
- [ ] Consolidate the PR-state jq normalizer (`if .merged then "MERGED" elif .state == "open" then "OPEN" else "CLOSED" end`), currently pasted at board-loop.yml:297 and :392 with separately-worded comments, into one canonical filter per CLAUDE.md's "shared logic has exactly one home" rule.
