...
- [X] Fix `in_flight_candidate()` in `.github/scripts/board_eligibility.py` so a marker recording the `prove` step (written with `pr=None` at both `board-loop.yml:2703` and `:2715`) is not silently dropped from in-flight candidacy by `int(None)` raising `TypeError` at `board_eligibility.py:170` (caught by the existing `except (TypeError, ValueError): continue`). `prove` is non-terminal (not in `TERMINAL_STEPS`) but its marker never carries a `pr`, so `FIX_OR_LATER_STEPS` membership can never be satisfied for it today -- treat it like the pre-fix steps (qualify on step alone) or otherwise handle `pr=None` explicitly, and add a fixture case to `.github/scripts/tests/board-eligibility/in-flight/` covering it (extends FR-012's coverage).
- [X] Add `gh label create "board:owned" --color 0E8A16 --description "Board loop: this PR was opened by board-loop.yml" --force` immediately before the `gh pr create ... --label board:owned` call in board-loop.yml's "Push and open the PR" step (~line 1454), so the call does not 404 against a label that does not yet exist in the live repository (confirmed via `gh api repos/charlesguse/wing-commander/labels/board:owned` -> 404).
- [X] Fix the FR-007 resume fallback's `gh api ... --paginate --jq '{number, body}'` call (board-loop.yml:407-411): the `/issues` endpoint returns a JSON array per page, so `{number, body}` must be applied per-element (`--jq '.[] | {number, body}'` piped to `jq -s '.'`), matching the stream-then-slurp shape this file already uses elsewhere (the state-normalizer calls) and documented in `specs/033-pr-conversation-commands/quickstart.md` T067. The trailing `2>/dev/null` currently hides this failure entirely, so `fallback_number` is always empty.
- [X] Fix `read_marker_with_timestamp()` (`.github/scripts/board_item_marker.py:19-43`) so an unparsable *newest* marker degrades the whole result to `None`, per its own docstring and FR-054's re-derivation guarantee, instead of silently returning the newest *parsable* older comment's marker.

## Maintainer Feedback (continued)
...
- [X] Fix: a stuck `prove`-step marker with `pr=None` is treated as in-flight forever by `in_flight_candidate()` (`.github/scripts/board_eligibility.py:140`), permanently pinning the board loop because no job on the scheduled/`workflow_dispatch` path ever consumes `step == 'prove'` -- confirmed none of the `triage` (line 632), `fix` (1245), `review` (1609), or `readiness` (2224) `if:` conditions in `board-loop.yml` check for it, and `prove-gate`/`prove` themselves only run `if: github.event_name == 'pull_request'`. This converts the documented FR-043 "could not correlate/conclude" degrade branch into a permanent stall of the whole board loop, with no `board:stalled` applied to break the cycle. Exclude `prove`-marker issues from `in_flight_candidate()`'s priority path unless a real consumer for `step=prove` exists off the scheduled trigger, or apply a stalled-style exclusion after some bound. (Found by review of PR #475)

## Maintainer Feedback

- [X] Fix: resume's step-resolution script (`board-loop.yml:460-471`) clears `pr_number`/`pr_state` when disqualifying a stale fix-or-later marker, but `branch` (line 505) and `round`/`base-sha` (509+) are echoed straight from bash variables the Python step-resolution never touches, leaking them into `$GITHUB_OUTPUT` alongside `step=triage`. This lets the fix job's "Fetch main / checkout the fix branch" step (`EXISTING_BRANCH: ${{ needs.select.outputs.branch }}`) silently resume on the old, abandoned branch instead of cutting a fresh one off current main (violates FR-022), and lets the review job's `ROUND` output pick up the leaked round count, starting a brand-new PR's review cycle already partway through its round budget instead of at 0. Clear `branch`/`round`/`base-sha` alongside `pr`/`pr-state` in both disqualification branches (the `elif pr_from_marker and marker_step in FIX_OR_LATER_STEPS` branch and the final `else` branch). (Found by review of PR #475)

## Maintainer Feedback

- [X] Cleanup (non-blocking): `board-loop.yml:318` redirects `board_eligibility.py`'s stderr into `$RUNNER_TEMP/board-eligibility-provenance.json`, which is only ever read for `.decided_by_marker`/`.multiple_found`. An unhandled exception in that script leaves no traceback on stdout or the visible log, and the run silently reports "nothing to do" with an empty `issue_number` instead of surfacing the crash. Surface the script's stderr/exception output on failure rather than swallowing it. (Found by review of PR #475)

## Maintainer Feedback

- [X] Fix: `select()`'s oldest-first fallback (`.github/scripts/board_eligibility.py:199-207`, the `classify_issue`/`is_excluded` scan reached when `in_flight_candidate()` returns no priority candidate) does not filter on marker step at all. `in_flight_candidate()` excludes `step == "prove"` from the priority path (the prior fix for the earlier stuck-`prove` finding), but once such an issue ages to the front of the oldest-first queue, the fallback returns it exactly as it would any other eligible issue — `is_excluded()` only checks `closed`/`board:stalled`/`disposition:*`/`stage:*`/`spec:*` labels, none of which a stuck-prove issue necessarily carries. Resume then correctly resolves `step="prove"` for it, but no job on the scheduled/`workflow_dispatch` path (`triage` at board-loop.yml:664, `fix` at 1277, `review` at 1641, `readiness` at 2256) ever consumes `step=="prove"` — only `prove-gate`/`prove` do, and those run solely on `pull_request: closed`. No downstream job fires, no marker rewrite happens, and no `board:stalled` label is applied (every occurrence of that label-add lives inside a job that never runs on this path), so the issue's state is unchanged and the next scheduled run repeats identically: the loop starves every issue created after it, forever, silently, with no priority marker to surface the stall. Give the oldest-first fallback in `select()` the same prove-exclusion `in_flight_candidate()` already has (skip an issue whose newest marker records `step="prove"`, mirroring `in_flight_candidate()`'s `continue`), keeping "which issues can `select()` ever return" governed by the one function CLAUDE.md's single-home rule designates for that decision. Add a fixture case to `.github/scripts/tests/board-eligibility/` covering a stuck-`prove`-marker issue reaching the oldest-first fallback (e.g. `prove-no-pr` extended, or a new case), and update `verify-board-eligibility.py`/the relevant contract doc accordingly. (Found by review of PR #475)
