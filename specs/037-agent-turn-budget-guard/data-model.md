...
| `multiplier` | float, optional (default `2.5`), must be `> 0` | `wing-commander-turn-ceiling`'s own input default | Single source of truth for the whole fleet (research.md R1); overridable per site only if a specific site's own future evidence justifies a different margin — no site overrides it at this feature's landing. `0` is rejected by the action (it would compute `ceiling=0`, i.e. an unbounded step by another name) and a literal `1` is rejected by Gate 23 (the ceiling would equal the intended budget, which is the raw passthrough the gate exists to catch). |
| `ceiling` | integer (action output) | `wing-commander-turn-ceiling`'s only output | `ceil(intended-turns * multiplier)`. The literal value that reaches `claude_args`' `--max-turns` flag at every site, and — passed on as `wing-commander-metrics-summary`'s `ceiling` input — the number the exhaustion banner names, since a run cut off by the runtime was cut off at this, not at `intended-turns`. |
