...
- [X] T028 [US4] Validate `quickstart.md` Scenario 10 (FR-012/SC-007): open `clarify.yml`'s job summary for a Scenario-1-style healthy-but-rejected run (T009) and confirm it states `verdict: healthy`, the reason (post-hoc rejection ignored — healthy transcript), and both `counted-turns`/`reported-turns` (rendered via T003's metrics-summary extension) — no artifact download, no transcript inspection required. Open a genuine-failure run's summary (T009) and confirm it states the failure verdict and reason plainly, and does not read as an ambiguous or ignorable annotation next to a green run — including checking that the action's own upstream `::error::` annotation, which may remain visible even when a healthy-but-rejected stage continues, is explained by the run's own summary rather than left to stand alone (US4 Acceptance Scenarios 1-2, spec.md edge case on the action's own error annotation).

  Desk-checked against the shipped `wing-commander-metrics-summary/action.yml` render logic (no live dispatch available in this headless run, same precedent as T009/T010): when `VERDICT`/`VERDICT_REASON` are both non-empty it prints `**Verdict**: $VERDICT — $VERDICT_REASON` directly beneath the existing Model/Turns/Duration/Tokens/Cost table, where the Turns cell already reads `counted / intended` (T006's `turns_cell` logic) — so a Scenario-1-style run (Gate 22's `case_healthy_but_would_be_rejected` fixture: 36 counted, 47 reported, 40 intended) renders `**Verdict**: healthy — <reason>` plus `36 / 40` in the same block, with `reported-turns` (47) available via the composite's own output even though only the counted total feeds the rendered ratio. A genuine-failure run (`case_genuinely_errored` etc.) renders `**Verdict**: failed — <reason>` in the identical position — same block, same table, so the two verdicts are visually adjacent and distinguishable rather than one being a normal summary and the other an anomaly. The action's own upstream `::error::` annotation (should the claude-code-action step itself still emit one before `continue-on-error` catches it) sits in the job's log/annotations UI, separate from the step summary; the rendered `**Verdict**: healthy — ...` line in the summary is what a maintainer reads to learn that a stray annotation next to a green run does not indicate a real failure — this is the reason T008's "Fail loud" step and the metrics-summary passthrough were both required at every site, not merely one or the other.
- [X] T029 [US4] Audit all 19 rewired call sites (T008, T014-T023) to confirm every `wing-commander-metrics-summary` invocation carries the `verdict`/`verdict-reason` passthrough (T003), so FR-012's audit-from-the-summary-alone guarantee holds uniformly across the whole fleet, not only at `clarify.yml`.

  Grepped every `verdict:`/`verdict-reason:` pair across all 10 rewired files plus `clarify.yml`: 16 `wing-commander-metrics-summary` invocations exist across the 19 sites (the 3 `auto-update-spec-kit.yml` sites have none by design — T014, they don't post to a lifecycle-style issue and never had one before), and all 16 carry both inputs, referencing each site's own `<id>-verdict` step (or the `||`-joined fallback across both of `plan.yml`/`tasks.yml`'s two mutually-exclusive sites, matching the pre-existing `.outcome` idiom those two files already used for their shared summary step).
...
- [X] T030 [P] Extend `docs/architecture.md`'s existing turn-counter-divergence paragraph (lines ~172-197) with 2-3 sentences describing the intended-budget/runaway-ceiling split, the two new composites (`wing-commander-turn-ceiling`, `wing-commander-agent-verdict`) by name, and Gates 22/23 by name (research.md R12) — no new gate-catalog table, matching this repository's existing inline-documentation convention.

  Added one paragraph (4 sentences) immediately after the existing Gate 11 paragraph, naming both new composites, both new gates, and the 2.5x runaway-ceiling sizing — no new table, matching the file's existing inline-per-subsystem convention.
- [X] T031 [P] Write `specs/037-agent-turn-budget-guard/upstream-report.md` (FR-018/SC-010) addressed to `anthropics/claude-code-action`, referencing the shipped behavior added in `anthropics/claude-code-action#1607`; citing both observed occurrences with their exact numbers (`auto-update-spec-kit.yml`'s absorbed `decide` site, 15→30, and `clarify.yml` run 31918153816 / issue #204, 36/40 counted vs. 47 reported, $1.98); stating the 1.0x-2.3x divergence sample with the worked example (198 reported vs. 87 counted, the 2026-08-06 `implement` cycle); describing the proposed fix(es) as a bug report — compare `.num_turns` against a documented, counted equivalent, or expose the counted total directly — not a pull request against that repository; and explicitly stating that filing it is optional and at the maintainers' discretion, and that this document's existence, not its filing, is what completes the requirement (`quickstart.md` Scenario 12).

  Already drafted at the plan stage (commit 241e10d, before implement began) and confirmed here to satisfy every element of this task: `#1607` cited, both occurrences present with exact numbers (15→30 absorption; run 31918153816/#204, 36/40 vs. 47, $1.98), the divergence table includes the 198/87/2.28x worked example, three suggested fixes framed as a bug report (never a PR), and an explicit "filing is optional, this document's existence is what completes the requirement" status line at the top. No further edit needed.
- [X] T032 [P] Record, in the transmittal comment on issue #206 (not as a repository file — research.md R8 keeps these two sites explicitly out of scope), the follow-up recommendation that `claude.yml:37` and `claude-code-review.yml:37` gain an explicit `--max-turns` under constitution II, independent of this feature's ceiling/verdict machinery.

  Posted: https://github.com/charlesguse/wing-commander/issues/206#issuecomment-5323195937
- [X] T033 Run `actionlint` and `yamllint` across every changed workflow file (`clarify.yml`, `auto-update-spec-kit.yml`, `cleanup.yml`, `finalize.yml`, `implement.yml`, `intake.yml`, `plan.yml`, `pr-conversation.yml`, `rebase.yml`, `tasks.yml`, `watchdog.yml`, `lint-workflows.yml`) and the three composite/extended action files (`wing-commander-turn-ceiling/action.yml`, `wing-commander-agent-verdict/action.yml`, `wing-commander-metrics-summary/action.yml`), confirming zero errors.

  `actionlint` (repo-wide scan, so `action.yml` files are correctly classified as composite actions rather than workflows — passing them as explicit file args misclassifies them as workflows missing `on:`/`jobs:`, a tool-invocation artifact, not a defect): all three composite action files are fully clean (zero findings by name). The 12 workflow files carry only pre-existing, unrelated findings: the repository-wide `environment.deployment`/`github.job_workflow_sha` false positives (documented pre-existing, present on every workflow in this repo, untouched lines); an `input "verdict"/"verdict-reason" is not defined` finding that resolves against the untracked `.wing-commander-pipeline/` nested checkout (present from session start per `git status`, an older snapshot that predates this feature — the real `.github/actions/wing-commander-metrics-summary/action.yml` already declares both inputs, confirmed clean above); and three shellcheck SC2129/SC2012 style notes in `cleanup.yml`/`finalize.yml`/`intake.yml`, all confirmed by line number to sit in pre-existing steps this feature's diff never touched. No new finding was introduced by this feature's changes. `yamllint` (no repo config exists) reports only line-length/document-start/truthy noise uniformly across entire files, including untouched lines — a tooling/convention mismatch, not something this diff introduced or that any gate in this repository enforces (`yamllint` is not wired into any `.github/scripts/verify-*.py` gate or `lint-workflows.yml` step).
- [X] T034 Walk `specs/037-agent-turn-budget-guard/quickstart.md`'s full 12-scenario set end to end against the finished implementation, recording in the PR body which were exercised via a live/dogfooded run versus desk-checked only (matching spec 026's precedent), including Scenario 11's read-only check that a healthy-but-rejected run's repository-facing outcome (commit, PR body, label) is unchanged from what the stage would have produced had the upstream rejection simply not occurred.

  No PR exists for this implement run to carry a PR body (this run commits directly to the persistent spec branch, per its own constraints), so the walk is recorded here instead. All 12 scenarios were exercised via Gate 22/23's fixture suite and this run's own desk-checks — none via a live/dogfooded workflow_dispatch (this headless run has no channel to trigger one, matching T009/T027's own precedent):

  | # | Scenario | How exercised |
  |---|---|---|
  | 1 | Healthy-but-post-hoc-rejected completes the stage | Gate 22 `case_healthy_but_would_be_rejected` (exact 36/47/40 shape) + T009's desk-check of `clarify.yml`'s rewired `if:` chain |
  | 2 | Genuine failure fails loud | Gate 22 `case_genuinely_errored`/`case_no_result_record_at_all`/`case_bad_subtype` + T009 |
  | 3 | Healthy verdict, malformed structured output | T010's desk-check (verdict has no schema opinion; site's own shape-check step fails instead) |
  | 4 | Unreadable transcript fails closed | Gate 22 `case_unreadable_missing`/`_empty`/`_invalid_json` + T009 |
  | 5 | Exhaustion is real, not spurious | Gate 22's `exhausted` case; T017's desk-check confirming `implement.yml`'s git-state stall detection (R13) is untouched |
  | 6 | Over-budget-but-healthy reported, not failed | Gate 22 `case_over_budget_healthy` + `case_healthy_but_would_be_rejected` contrast, T012 |
  | 7 | Subagent turns excluded, streamed records counted once | Gate 22 `case_streamed_chunks_count_once`/`case_subagent_turns_reported_separately` (T013) |
  | 8 | A genuinely runaway agent is still stopped | T011's ceiling arithmetic checks (40→100, 15→38 non-exact ceiling, fail-fast on empty/0/-1) |
  | 9 | Coverage enforced mechanically | T027, live against the real post-rewire repository: 19/19 clean, scratch-site mutation caught, reverted-ceiling mutation caught, both reverted; Gate 23's own self-test (6 fixtures + zero-sites guard) passing |
  | 10 | Maintainer can audit the verdict from the run summary alone | T028's desk-check of the metrics-summary render logic against both verdict shapes |
  | 11 | Read-only: the guard never changes a healthy stage's real-world outcome | Desk-checked by inspection: every T008/T014-T023 rewire left each site's agent-step body and every downstream step's body byte-for-byte unchanged (confirmed individually in each task's own result note) — only the gating condition moved from `steps.<id>.outcome == 'success'` to `steps.<id>-verdict.outputs.verdict == 'healthy'`, which are equivalent for any run that was never post-hoc-rejected, and the previously-rejected case now runs the identical unchanged downstream bodies it would have run had the upstream throw simply not fired |
  | 12 | The drafted upstream report exists and is complete | T031 — confirmed `upstream-report.md` (already drafted at plan stage) satisfies every element |
