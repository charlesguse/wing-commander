...
**Why this priority**: Removing the spurious failure must not remove the ceiling with it. The project's cost discipline requires every agent step to run under a bounded budget, and the `implement` stage consumes budget exhaustion as a real signal. A fix that only makes the cap bigger buys silence at the cost of letting a runaway agent spend several times more before anything stops it, so the budget and the ceiling have to be separable and both real.
...
1. **Given** an agent run whose counted main-loop turns reach or exceed the stage's intended budget, **When** the step ends, **Then** the run is identified as budget-exhausted and that fact is reported where a maintainer will see it.
...
All agent invocation sites across the pipeline carry the same protection, and a newly added agent step cannot ship without it. The coverage is asserted mechanically rather than depending on whoever adds the next step remembering the history.
...
- **An agent run that is both healthy and over its intended budget.** Two independent facts; the verdict must not collapse them into one.
...
- **FR-016**: [NEEDS CLARIFICATION: Does this feature deliver the shared agent-verdict evaluation for all agent steps including the six that today have no rescue wiring at all (subsuming #193), or only the healthy-versus-failed discrimination, leaving #193 to wire the rescue into those steps separately?]
- **FR-017**: When an agent run is healthy but its counted main-loop turns reach or exceed its intended budget, the stage MUST [NEEDS CLARIFICATION: fail the run, continue and report the over-budget condition as a warning, or apply a per-stage policy where stages that consume exhaustion as a signal continue while others fail?]
- **FR-018**: [NEEDS CLARIFICATION: Is reporting the counter mismatch upstream to `anthropics/claude-code-action` part of this feature's deliverable — and if so, does the deliverable end at a drafted report for a maintainer to file, since the pipeline does not open issues in repositories it does not own?]
