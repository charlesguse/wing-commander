...
- **AWS configuration must reach a stage that runs as a called workflow**: Adopters call the pipeline's stages as reusable workflows, which execute in their own isolated jobs. AWS credentials that a consumer sets up in their own calling job do not automatically carry into the pipeline's stage job. The pipeline resolves this by accepting an **AWS role ARN and region as stage inputs** and running credential configuration (OIDC, via the `configure-aws-credentials` action) **inside each stage**, so no long-lived AWS secrets are needed. The caller must grant the workflow `id-token: write` permission so the OIDC token can be issued; the role ARN and region become part of the stage interface.
- **Default model identifiers under Bedrock**: The stages default to Anthropic model names for their agents. Bedrock addresses models by different identifiers, so a run that enables Bedrock but leaves the default Anthropic model names in place could reference a model Bedrock does not recognize. Model resolution is **pure pass-through**: when Bedrock is enabled, the consumer MUST supply Bedrock-compatible model identifiers through the existing per-stage model settings, and the pipeline conveys them unchanged — it does not translate its default Anthropic model tiers to Bedrock equivalents. A missing or wrong identifier surfaces as a clear error rather than a silent mismatch.
...
- **FR-003**: The pipeline MUST accept an AWS role ARN and region as stage inputs and, when Bedrock is selected, configure AWS credentials via OIDC (the `configure-aws-credentials` action) inside each stage job, so that the agent's model requests reach the consumer's AWS account without long-lived AWS secrets. The pipeline MUST rely on the caller granting `id-token: write` for OIDC to succeed.
...
- **FR-006**: When Bedrock is selected, the consumer MUST supply Bedrock-compatible model identifiers for each stage's agent through the existing per-stage model settings, and the pipeline MUST convey them to the agent unchanged (pure pass-through). The pipeline MUST NOT translate its default Anthropic model tiers to Bedrock equivalents; a missing or incompatible identifier MUST surface as a clear error.
...
- **Bedrock Configuration**: The consumer-supplied, trusted configuration required to serve requests through Bedrock — the enablement flag, the AWS role ARN (assumed via OIDC inside each stage), the AWS region, and the Bedrock-compatible model identifiers the consumer supplies per stage. Owned by the consuming repository, never derived from untrusted content.
