...
- **FR-011**: The action-required convention MUST be applied consistently across every stage that can require human action, so that the meaning of the convention is learnable once and trusted thereafter. [NEEDS CLARIFICATION: Which moments must be treated as action-required is a scope decision — see Question 1. At minimum the two PR review gates and residual manual work are in scope; whether clarification-needed and failure/stall states must also adopt the same convention in this feature affects the breadth of changes.]
- **FR-012**: The chosen presentation for action-required callouts MUST make the latest outstanding action discoverable even on a long, busy issue. [NEEDS CLARIFICATION: Presentation approach — see Question 2. Post a fresh, clearly-marked comment at each action moment (append-only, matches current behavior) versus maintaining a single continuously-updated status comment/section that always reflects the current next step.]
