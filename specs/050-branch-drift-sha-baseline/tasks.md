...
- [X] Make `wing-commander-metrics-persist/action.yml` (line ~151) pick up `wing-commander-metrics-record-branch-advance.json` (written by `implement.yml` line ~2198) instead of silently skipping it (line ~152), so the durable history and the Gate 41 rollup filter (~390) exercise the real artifact layout rather than only a synthetic batch.
  The discover loop now globs `"$adir"*.json` instead of the hardcoded plain filename. `verify-metrics-persist-retry.py`'s `case_validate_then_append_persists_reusable_workflow_records` (the one case already exercising the real artifact layout, not a synthetic batch) gained a `metrics-record-branch-advance` fixture; confirmed to fail on the pre-fix code.
- [X] Fix `watchdog.yml` line ~817 so a `commits`-null pair with `before_sha != after_sha` reads as healthy per FR-019/FR-017/SC-002, not as a false lost-progress signal. Add a Gate 63 scenario for this case.
  The healthy-arm condition now reads `before != after && commits != "0"` (no longer requiring `commits` to also be non-empty). Gate 63 (`verify-branch-drift-sha-baseline.py`, renumbered from the plan's provisional 53 once main's own gate count passed it) gained `scenario_exact_sha_commits_unavailable_reads_healthy`; confirmed to fail on the pre-fix code.
- [X] Fix `metrics-summary/action.yml` line ~256 so `available`/`branch`-nulling matches the contract delta (available needs a branch and at least one SHA) and the `valid-branch-advance-before-unavailable`/`after-unavailable` fixtures (available: true, branch kept), instead of nulling `branch` and forcing `available: false` while leaving the other SHA set. Add a gate that runs the composite over each fixture case to cover the emission side.
  `branch_advance_available` now requires `before-sha-available OR after-sha-available` (was AND). Gate 43 (`verify-metrics-summary-record-emission.py`) gained `case_branch_advance_availability_follows_contract_or_rule`, running the real composite over the before-only/after-only/both-unavailable states; confirmed to fail on the pre-fix code.
- [X] Fix `watchdog.yml` line ~751 so `measure_branch` is read from `branch_advance.branch` when available, rather than always being derived from prefix and slug, per FR-003.
  The exact-sha arm now reads `branch_advance.branch` from the same record scan that already reads the two SHAs, falling back to the prefix+slug derivation only when the record carries no branch name.
- [X] Extend `wing-commander-metrics-persist`'s inline jq validator to check `branch_advance` at the same strictness as Gate 39, and add the FR-008-required wrong-typed fixture for each new field beyond `commits`.
  The validator's `ok=` jq program now checks `branch_advance`'s 8 sub-fields (presence + type) when the key is present, mirroring `REQUIRED_BRANCH_ADVANCE`. Added 7 new `invalid-branch-advance-*-wrong-type.json` fixtures (one per field besides `commits`, which already had one) and bumped Gate 39's pinned fixture count from 16 to 23.
- [X] Verify: confirm whether the three new implement.yml steps (`Record branch advance (cycle)` and its metrics-summary/upload steps) should use `always()`/`!cancelled()` like their sibling metrics/upload steps rather than implicit `success()`, and align them if the siblings' intent requires it, so a hard failure in an earlier step doesn't skip the record on exactly the cycles where it matters.
  Determined the relevant sibling pattern is "Record truncated-cycle count"/"Flip stage label (first cycle)" (both `continue-on-error` with no `always()`, relying on every step between the cycle's push and them staying `continue-on-error` so the job's implicit `success()` stays true) rather than the earlier "Agent run metrics summary (cycle)" pair (which uses `always()` because a real agent failure is expected right after the cycle step). "Record branch advance (cycle)" and "Agent run metrics summary (branch advance)" already carried `continue-on-error: true`; "Upload metrics record (branch advance)" was the one step in the trio missing it, added it to close the gap.
- [ ] Confirm FR-020's follow-up issue status: T027 remains unchecked/blocked (no `gh issue create` in this run's tool surface) — no new action needed beyond what's already documented, but note the review's reiteration on the PR.
  **BLOCKED this cycle too**: this run's tool surface has no `gh pr comment` (only `gh issue view`/`gh issue comment`), so the reiteration could not be posted to PR #354 directly. Left unchecked deliberately; a maintainer or a future run with PR-comment tooling should post it.
