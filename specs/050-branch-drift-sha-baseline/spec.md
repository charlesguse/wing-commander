...
1. **Given** a dispatched implement run whose record carries the spec branch's tip at cycle start and after its push, **When** those two SHAs are equal, **Then** a lost-progress signal is emitted naming the branch and both SHAs.
...
**Independent Test**: Inspect a run whose metrics record carries no branch SHAs and confirm the watchdog's step summary names the baseline it fell back to (or states that it skipped), and that the collector still reports a successful outcome rather than an untrusted read.

**Acceptance Scenarios**:

1. **Given** a dispatched implement run whose metrics record marks the branch SHAs unavailable, **When** the watchdog inspects it, **Then** the step summary states that the exact-SHA baseline was unavailable and names what was done instead, and the collector's outcome is still recorded as successful.
...
- **Recorded SHAs are no longer resolvable.** A force-push can orphan the commits the recorded SHAs name, so a later attempt to walk between them can fail even though both SHAs are known. Whether the signal needs a commit count at all, or only the equal/not-equal answer, is an open question below.
- **Both SHAs present but the "after" is an ancestor of the "before".** A cycle that reset the branch backwards is not "no progress", but it is not forward progress either; the collector must not silently report it as healthy.
...
- **FR-001**: The implement stage MUST record, in its own metrics record for a cycle, the spec branch's tip commit as observed at the start of that cycle ("before") and the spec branch's tip commit as observed after the cycle's final push ("after").
- **FR-002**: Each recorded branch point MUST be accompanied by an explicit availability marker following the record's existing convention, so a reader can distinguish "not measurable for this run" from "measured and equal".
- **FR-003**: The record MUST also name the branch the pair refers to, so a reader never has to reconstruct it from a slug and a configurable prefix.
...
- **FR-007**: The published record contract MUST be updated to describe the new fields, including their availability semantics and the not-yet-created-branch and failed-push cases, so a reader of the contract alone knows the full shape.
- **FR-008**: The gate that validates record conformance MUST require the new fields at the same strictness as existing fields, with positive and negative fixtures covering: both points present and different, both present and equal, "before" unavailable, "after" unavailable, and a wrong-typed value.
...
- **FR-011**: The watchdog MUST report the branch and both compared points in the signal it emits, so the finding names the exact evidence it decided on.
...
- **FR-018**: [NEEDS CLARIFICATION: When a dispatched implement run's metrics record carries no usable branch points, should the watchdog fall back to today's since-created timestamp baseline (keeps today's detection rate, keeps today's two miss cases for that run only) or skip the measurement entirely for that run (no possibility of a rebase-inflated or neighbour-inflated read, but no detection either)?]
- **FR-019**: [NEEDS CLARIFICATION: Should the emitted signal still carry a numeric count of commits pushed — which requires both recorded commits to still be resolvable at inspection time, and can therefore be unavailable after a force-push has orphaned them — or is the equal/not-equal answer plus the two SHAs the whole fact set, with no count?]
- **FR-020**: [NEEDS CLARIFICATION: Which stages record the branch pair — only implement, whose push target is unconditional, or also plan and tasks, whose push target depends on a review mode the record does not currently carry (which would require recording the mode or the resolved target branch as well)?]

### Key Entities

- **Agent run metrics record**: the durable per-run, per-step record the pipeline already emits and persists. Gains a description of the branch this run advanced and the two points it advanced it between, each with its own availability marker.
- **Branch advance pair**: the "before" and "after" tip commits of the branch a stage pushes to for one cycle, together with the branch's name. The unit of evidence that replaces the time window.
- **Branch-drift signal**: the normalized signal the watchdog emits when a push-expected run advanced its branch by nothing. Its facts change from `{branch, since, after-sha, commits}` to a pair of exact points, subject to FR-019.
...
- **SC-004**: Every one of the record's new-field states — both available, "before" unavailable, "after" unavailable, both unavailable — is exercised by a checked-in fixture, so no state is proven only by a live run.
...
- **SC-007**: A maintainer reading a filed lost-progress finding can state the branch and the two commits compared without opening any artifact the watchdog read.
...
- Recording only helps runs from this version forward. Historical records are not backfilled, and runs against them keep whatever behaviour FR-018 resolves to.
