...
on:
  pull_request:
    ...
    paths: ["specs/**"]
  workflow_dispatch:
    inputs: { slug: { required: true } }   # manual (re)start, same idiom as the plan stage
```

`workflow_dispatch` exists so the stalled path's documented restart ("delete
`tasks/NNN-slug`, restart the tasks stage") is actually executable: the
idempotency guard (below) blocks a plain re-run once `stage` durably reads
`"stalled"`, so a manual dispatch — and only a manual dispatch — may also
proceed from `stage == "stalled"`.
