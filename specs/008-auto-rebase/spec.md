...
- A specification's working branch was updated by an in-flight pipeline stage between the moment the stage read it and the moment it tries to publish the rebased result: the update is rejected because the branch moved, and the stage must not overwrite the concurrent change; it skips the branch silently and relies on the next run — see FR-011.
- The same specification remains in conflict across several consecutive runs: after asking for human help once, the stage skips that branch on subsequent runs until it changes, avoiding repeated AI cost and comment noise — see FR-012.
- A specification is no longer actively progressing (for example, it has been marked stalled, or it has already completed): its working branch is excluded from rebasing, so only actively-progressing specifications form the branch set the stage acts on — see FR-002.
...
- **FR-002**: The system MUST identify the set of in-flight specification persistent working branches to keep current, and act on each of them. "In-flight" means only specifications that are actively progressing through the pipeline; specifications that are no longer progressing — marked stalled, or already completed/merged — MUST be excluded from rebasing.
...
- **FR-011**: When publishing a rebased working branch, the system MUST NOT overwrite changes made to that branch concurrently by another actor (for example, an in-flight pipeline stage that pushed to the branch after the stage read it); if the branch has moved, the system MUST decline to overwrite it and leave the concurrent change intact. When a concurrent update blocks the publish, the system MUST skip that branch silently — leaving the drift to be corrected on the next run or the nightly schedule — and MUST NOT post a comment about the deferred update.
- **FR-012**: The system MUST behave sensibly when the same specification remains unresolvable across consecutive runs, avoiding runaway cost and comment noise. On a branch that stays unresolvable, the system MUST ask for human help once (per FR-008) and then skip that branch on subsequent runs until it changes — that is, until the working branch itself moves or the main line advances in a way that affects the branch — at which point it re-attempts the rebase.
