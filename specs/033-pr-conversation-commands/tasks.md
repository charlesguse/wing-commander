...
**Tests**: Not requested â€” no automated test suite exists for any pipeline
...
â€” the first wrapper in this repository to listen to `pull_request_review`
and `pull_request_review_comment` (research.md D1). Because almost every
task edits `pr-conversation.yml`, `[P]` is used sparingly â€” only for tasks
...
onto these stories as: in-scope-change â†’ US1; new-functionality,
small-unrelated-change â†’ US2; needs-info, push-back â†’ US3;
manual-step-permission â†’ US4; the announce/confirm/stop mechanism that
guards every route â†’ US5; question â†’ US6; the outstanding-task-item
traceability guarantee that ties every out-of-PR artifact back to the
lifecycle issue â†’ US7.
...
empty-bodied skeletons â€” the scaffold every story's logic attaches to.
...
- [X] T001 [P] Create `.github/workflows/pr-conversation.yml` as a
  ...
  full typed inputs â€” `pr-number` (number, required), `event-kind` (string
  ...
  (research.md D6 â€” the exact canonical group `specs/013-serialize-rebase-stages`
...
- [X] T002 [P] Create `.github/workflows/wing-commander-9-pr-conversation.yml`
  ...
  via the job-level `if:` to `github.event.issue.pull_request != null` â€”
  the inverse of `wing-commander-2-clarify.yml`'s exclusion guard); one job
  whose `if:` is the actor gate (bot excluded outright, no run at all â€”
  FR-002; no `|| actor.id == issue.author.id` requester carve-out, unlike
  clarify/intake â€” FR-019); eventâ†’input extraction per `event-kind`
  ...
  â€” read only here, never inside the stage); calls
  `./.github/workflows/pr-conversation.yml`. No `workflow_dispatch` trigger
  (wrapper-gate.md's Non-goals â€” this wrapper is purely event-triggered).

**Checkpoint**: Both workflow files parse and are wired end-to-end with
empty job bodies â€” ready for Foundational steps.
...
intent before any mutation â€” the structural ordering FR-023 requires for
...
- [X] T004 In the `classify-and-announce` job, implement the stage-level
  authorized-actor gate (FR-019, FR-021 â€” the wrapper in T002 has already
...
- [X] T005 In the `classify-and-announce` job, stage `body` (and, for
  ...
  a file exactly as `clarify.yml` already does â€” never shell-interpolated,
...
- [X] T006 In the `classify-and-announce` job, implement the
  ...
  inputs (T002's `resolve-model` output, research.md D2 â€” sonnet default,
  ...
  `pr-conversation.classify` row â€” `Read,Grep,Glob,Bash(git log:*),Bash(git diff:*),
  Bash(git show:*),Bash(cat:*),Bash(gh pr view:*),Bash(gh issue view:*),
  Bash(gh search issues:*)`; `Write,Edit,WebSearch,WebFetch` and every
  `git`/messaging tool disallowed â€” FR-016, constitution V); prompt frames
...
- [X] T007 In the `classify-and-announce` job, implement the
  ...
  input (comma-separated list, or the literal `all`) â€” **not**
...
- [X] T008 In the `classify-and-announce` job, implement the
  ...
  use) â€” posted to the PR always, and additionally to the lifecycle issue
  ...
  `classify-and-announce` â€” `act`'s `environment:` binding (T029) cannot
...
- [X] T009 In the `act` job, implement the relayed-request risk-confirmation
  ...
  until a matching confirmation reply arrives on a later run â€” applies
...
- [X] T010 In the `act` job, wire the shared bounded agent step every
  ...
  (`contracts/reusable-pr-conversation.md`'s `pr-conversation.act` row â€”
  ...
  `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage` disallowed â€”
  FR-016).
...
## Phase 3: User Story 1 - In-scope review feedback re-drives the implement/converge loop (Priority: P1) ðŸŽ¯ MVP
...
the PR â€” with zero trace left on the lifecycle issue.
...
- [X] T011 [US1] In the `act` job of `.github/workflows/pr-conversation.yml`,
  ...
  steps 1â€“5: read `stage`/`iteration` from `spec-meta.json`; append a
  `## Maintainer Feedback (PR #<pr-number>, comment <comment-id>)` section
  to `<spec-dir>/tasks.md` (T010's agent step, using
  `drafted-content.tasks-md-section` from T006 â€” append-only, mirroring
...
- [X] T012 [US1] Implement step 6: dispatch
  ...
  -f issue=<issue-number> -f iteration=<recorded+1>` â€” the exact,
...
- [X] T013 [US1] Implement step 7: reply on the PR confirming the fold-in
  ...
  reply states that possibility on the **PR** (FR-005 â€” `implement.yml`
...
- [X] T014 [US1] Verify (`quickstart.md` scenario 2, spec.md US1 Acceptance
  #3): confirm T008's `IntentAnnouncement` posting and T011â€“T013's route
  never post to the lifecycle issue for `in-scope-change` â€” the
  conversation and its resolution stay entirely on the PR.

**Checkpoint**: User Story 1 is fully functional and independently
testable â€” SC-001 is satisfied for the in-scope-change route.
...
lifecycle issue (`quickstart.md` scenarios 3â€“5).
...
- [X] T015 [US2] In the `act` job, for `category == "new-functionality"`
  ...
  in-scope change) â€” no new artifact, not an outstanding task item
  (FR-013).
- [X] T016 [US2] For `fold-target == "new-spec"` (research.md D7), open a
  ...
  â€” the same label `wing-commander-1-intake.yml` already gates its own
...
- [X] T017 [US2] Implement the shared `OutstandingTaskItem` posting
  ...
  (`- [ ] <artifact kind, human phrase> â€” <url> (from PR #<pr-number>)`)
...
- [X] T018 [US2] For `category == "small-unrelated-change"`
  ...
  hardcoded threshold (documented default: â‰¤ 3 files, â‰¤ 40 changed lines).
  Within threshold: open a PR to the default branch from
  `drafted-content.pr-title`/`.pr-body`/the diff, branched from the
  default branch (independent of `spec/<slug>`) â€” `SpinOffArtifact`
...
- [X] T019 [US2] Exceeds threshold: re-route the classification as
  ...
  opening a PR â€” never opened once the backstop trips, regardless of what
...
- [X] T021 [US2] Verify (`quickstart.md` Static validation #4): exercise
  ...
  the documented threshold â€” confirm the over-threshold case re-routes via
  T019 rather than opening a PR.

**Checkpoint**: User Stories 1 AND 2 both work independently â€” the
...
is posted instead of any action (`quickstart.md` scenarios 6â€“7).
...
- [X] T023 [US3] For `category == "push-back"`, post a PR reply naming
  `constitution-conflict` and declining; no artifact, no fold (FR-010) â€”
...
**Checkpoint**: User Story 3 works independently â€” SC-003 (never performs
...
- [X] T025 [US4] In the `act` job, for `category == "manual-step-permission"`
  ...
  (FR-011 first clause) â€” not a spin-off artifact.
- [X] T026 [US4] For `drafted-content == {performed: false, reason}`,
  report the reason on the PR (FR-011 second clause) â€” not a spin-off
  artifact.
- [X] T027 [US4] For `drafted-content == {needs-permission, pr-title,
  ...
  reply linking that prior conversation instead of opening anything new â€”
  ...
  (`gh label create --force` on first use) â€” `SpinOffArtifact`
...
**Checkpoint**: User Story 4 works independently â€” all three
...
categories, and a maintainer can stop an announced action â€” either by
...
- [X] T029 [US5] In `.github/workflows/pr-conversation.yml`, bind `act`'s
  ...
  `deployment: false` â€” reusing `specs/031-stage-environment-binding`'s
...
- [X] T030 [US5] For `category == "stop"`, implement the reply-based stop
  ...
  â†’ `run-id`; `gh run cancel <run-id>`; if that announcement's
...
- [X] T031 [US5] When `gh run cancel` reports the target run already
  ...
  prior run's own final reply already reported â€” never implying the
...
- [X] T032 [US5] Verify: direct cancellation (a maintainer cancelling the
  ...
  request â€” no separate, weaker gate for stop requests (edge case: "a stop
  request from a non-authorized actor... does not stop the run").

**Checkpoint**: User Story 5 works independently â€” SC-008 and SC-009 both
...
answered on the PR with no mutation, no spin-off, and no code change â€”
...
are otherwise untouched (`quickstart.md` scenario 13â€“14).
...
- [X] T034 [US6] Verify (`quickstart.md` scenario 14): a comment mixing a
  ...
  separately-routed classification for the actionable part â€” each gets
...
**Checkpoint**: User Story 6 works independently â€” SC-010 holds.
...
outstanding task item, while purely PR-scoped discussion â€” including a
whole in-scope-change cycle â€” leaves no trace on the issue at all, and a
...
- [X] T036 [US7] Sweep verification: after driving T016 (new-issue),
  ...
  the PR, while T011â€“T014's in-scope cycle left no trace there at all.
- [X] T037 [US7] Verify FR-017 (edge case: "a comment is pure
  ...
  spin-off â€” and, per FR-014's "actionable request" scoping, no PR reply
  is required beyond what T008's announcement step already decides to
  post for a non-actionable classification.

**Checkpoint**: All seven user stories are independently functional â€” the
full `quickstart.md` scenario set (1â€“15) plus its edge-case checks pass.
...
- [X] T039 [P] Add the new wrapper's example and its trigger shape (no
  ...
  `docs/adoption.md`.
- [X] T040 [P] Add a "Stage 10 â€” PR Conversation" section to
...
- [X] T042 Static validation sweep (`quickstart.md` "Static validation"):
  ...
  bot comment â†’ job does not run; a `NONE`-association human comment â†’
  job runs, T004's notice-and-stop fires; an OWNER/MEMBER/COLLABORATOR
  comment â†’ proceeds past the gate â€” confirming no lifecycle-issue-author
  carve-out); exercise T003's `PullRequestIdentity.qualifies` check
  against fixture PR metadata (`spec-draft/<slug>`â†’default branch â‡’
  false; `plan/<slug>`â†’`spec/<slug>` â‡’ false; `spec/<slug>`â†’default
  branch â‡’ true).
  ...
  **Status (iteration 1, desk-checked, no live CI run available in this
  ...
  quickstart.md names â€” both now correct; tracing T002 surfaced a real bug
  ...
  binds to `matrix['confirm-environment']` (T029/FR-020 â€” required so a
  ...
  same-job leg independence â€” so this cannot be fixed inside
  ...
  **Bug found and fixed during this sweep**: `wing-commander-9-pr-conversation.yml`'s
  ...
  authorization") â€” a non-bot, non-authorized commenter got silently
  ...
  **Status (iteration 2, desk-checked)**: T044 (Phase 11) resolved the
  ...
  script â€” `lint-workflows.yml` Gate 7 now passes on
  ...
  CI run available in this environment â€” a maintainer should confirm
  Gate 7 passes live (`python3 .github/scripts/verify-gate-7.py` and the
  real `lint-workflows.yml` run) before merging.
- [X] T043 Walk `quickstart.md`'s full scenario set (1â€“15), its edge case
  ...
  **Status (iteration 1)**: not performed â€” this build-and-reassess cycle
  ...
  PR is opened, a maintainer should exercise at least scenarios 1â€“2 (the
  MVP fold-in/re-dispatch loop) and 9â€“12 (propose-and-confirm/stop) live
  against it before merging, given T042's Gate 7 finding above.

  **Status (iteration 6)**: performed as a desk-check of all 24 items
  (static validation 1â€“4, scenarios 1â€“15, four edge cases, regression)
  ...
  this PR merges** â€” not a scheduling choice: the wrapper triggers only on
  ...
  invokes â€” `lint-workflows.yml` implements the equivalent YAML-parse and
...
- [X] T044 Constitution VII requires that "a stage that must deviate
  carries a registered, machine-checked exception naming the reason â€”
  ...
  deviates from every other job in every published stage â€” it binds
  ...
  a code comment above that block (added in this cycle) â€” not a
  ...
  independence (verified by T032) â€” whichever a human decides is
  correct; this is a design call, not a mechanical fix. CRITICAL
  (Constitution VII, contradicts).

  **Status (iteration 2, desk-checked)**: built the first option â€” an
  ...
  job cannot use the exception to go fully unbound) â€” only the
...
- [X] T045 In `.github/workflows/pr-conversation.yml`'s "Dispatch
  ...
  `run_url`-empty fallback branch (the run-link poll timing out) â€” when
...
- [X] T046 In `.github/workflows/pr-conversation.yml`, the "Dispatch
  ...
  returns `mutated: false` (a valid schema value â€” e.g. it could not
  ...
  and no reply is ever posted on the PR for that classification â€” every
  ...
  **Status (iteration 3, desk-checked)**: relaxed "Report mutation
  outcome"'s exclusion exactly as the task's own alternative suggests â€”
  ...
  rather than silently dropping the leg â€” left unchanged as out of this
...
- [X] T047 FR-022 requires that once a relaying maintainer confirms a
  ...
  instead arrives as a later, separate comment (the documented flow â€”
  ...
  confirmed relayed request is never actually re-executed â€” the original
  ...
  **Status (iteration 3, desk-checked)**: built the persisted-data resume
  ...
  ... -->`) in its own risk-warning PR comment â€” mirroring the stop
  ...
  end-to-end (embed â†’ resume â†’ re-announce â†’ re-act).
- [X] T048 `contracts/autonomy-and-confirmation.md`'s reply-based-stop
  ...
  run's own reply for what it already did" â€” a pointer, not a summary â€”
  ...
  **Status (iteration 3, desk-checked)**: the already-completed branch
  ...
  a relay-risk marker â€” that is the prior run's own final outcome reply.
...
- [X] T050 CRITICAL (Constitution V, contradicts): the
  ...
  `wing-commander:pr-conversation-relay` marker (T047), but â€” unlike the
  Stop procedure's identically-shaped scan, which filters to
  `select(.user.login == env.BOT_LOGIN)` â€” this scan applies no such
  ...
  step feeds the forged `classification` â€” including its `drafted-content`
  (e.g. a `tasks-md-section` to append, or a diff/PR body) â€” straight past
...
- [X] T051 The permission-request dedup search in the "Resolve effective
  ...
  actually creates is a PR â€” the act agent's own prompt (`manual-step-permission`,
  ...
  permission-request PR â€” `permission_match` always resolves to `"none"`,
...
- [X] T052 The "Relayed-request risk-confirmation gate" step (`act` job)
  ...
  request whenever ANY comment they have ever posted on the PR â€” on
  either surface â€” contains the word "confirm", with no lower bound
  ...
  confirming acceptance of THIS stated risk before proceeding â€” an
...
- [X] T053 CRITICAL (contradicts): all three "find my own bot's prior PR
  comment" lookups in `.github/workflows/pr-conversation.yml` â€” the
  ...
  (T030, lines 1323/1326-1330) â€” set `BOT_LOGIN: ${{ steps.ctx.outputs.bot-slug }}`
  ...
  `<slug>[bot]`)" â€” i.e. `bot-slug` is the raw slug, while a GitHub App's
  ...
  matches, so: the Stop procedure's comment fetches are always empty â†’
  `run_url` is always empty â†’ every reply-based stop request reports "No
  ...
  â†’ `resumed` is always `false` â†’ a maintainer's confirmation of a risky
  relayed request never actually resumes the original classification â€”
  ...
  relay-gate anchor lookup is always empty â†’ a risky relayed request can
...
- [X] T054 The "Check for relay confirmation" step (`relay-resume`, T047)
  ...
  `.[0]` â€” the single most recent marker on the PR system-wide â€” then
  ...
  classification is invisible to relay-resume â€” it only ever inspects the
  ...
  **Status (iteration 5, desk-checked)**: the relay-resume step's marker
  ...
  set rather than the PR-wide most-recent marker regardless of actor â€”
  ...
  JSON payload â€” **not** to strip the marker block outright. Stripping it
  ...
  decide its own `proceed=true` â€” a stripped marker made that anchor
...
- [X] T055 `pr-conversation.act`'s tool allowlist
  ...
  agent has no way to reach a new branch without a branch-creation tool â€”
  ...
  `Bash(git push:*)`) â€” neither of which the prompt or contract
...
- [X] T056 CRITICAL (contradicts): the "Post intent announcements" step
  ...
  classification with no category exclusion â€” including for
  `category == "stop"` itself â€” embedding `RUN_URL` = the **current**
  ...
  recent `**Run:**` comment on the thread â€” the Stop procedure's scan
  ...
  itself, not the announced action â€” the targeted prior run is never
  ...
  post its own "Stopped: â€¦" confirmation reply. This is not an edge
  case â€” it fires on every reply-based stop request â€” and directly
  ...
  announce/cancel) â€” whichever a human decides is the cleaner fix.
  ...
  **Status (iteration 6, desk-checked)**: confirmed against the shipped
  ...
  fires â€” still a reply, never a silent wrong cancel (SC-009).
  ...
  is a non-announcement reply â€” plus a defect witness confirming the
...
- [X] T057 The "Resolve effective category and route" step (`act` job)
  ...
  string as a live `jq` regex pattern â€”
  `jq -r --arg cap "$capability" '[.[] | select(.title | test($cap;
  "i"))] | .[0].url // empty'` â€” to search prior permission-request PR
  ...
  unbalanced parenthesis or bracket â€” plausible for unconstrained
  ...
  its `GITHUB_OUTPUT` writes â€” silently denying that `manual-step-
  ...
  **Status (iteration 6, desk-checked)**: confirmed and fixed by the
  ...
  unrelated permission-request PR as a "confident" match â€” a wrong answer
  ...
  unchanged: no match â‡’ `uncertain`/`none` â‡’ open the permission-request
  ...
  lone `.`, empty capability, and a genuine miss â€” plus a defect witness
...
- [X] T058 The `small-unrelated-change` size backstop re-routes to
  ...
  `matrix['drafted-content']` **unmodified** â€” still the
  small-unrelated-change shape (`pr-title`, `pr-body`, `file-changes`) â€”
  ...
  improvising an issue title/body from a diff â€” exactly the "drafted
  ...
  **Status (iteration 7)**: fixed by deriving `issue-title` from the
  ...
  override in `act`'s route step; T060 then moved the whole backstop â€”
  measurement and reshape together â€” into `classify-and-announce`, so the
  ...
  `.path` were silently dropped from the list but still counted â€” now
...
- [X] T060 CRITICAL (contradicts): the `small-unrelated-change` size
  ...
  `environment:` binding is evaluated from the matrix at job start â€”
  ...
  under-threshold case â€” a `small-unrelated-change` leg reaching `act` is
  under-threshold by construction â€” with a comment at the old site so a
...
- [X] T061 CRITICAL: `Compute confirmation requirements` built its
  ...
  for `confirm-categories` is `""` â€” zero bytes, so there is no line, the
  ...
  â€” before any announcement is posted and before `act` can start. This is
...
- [X] T059 `quickstart.md`'s "pure acknowledgement" edge case requires
  ...
  own internal decision â€” no PR reply is required," citing FR-014's
  ...
  nothing, and is excluded from the reply step) â€” the contradiction is
  ...
  **Status (iteration 7)**: resolved in favour of the workflow yielding â€”
...
- [X] T062 CRITICAL (contradicts): the "Stop procedure" step (`act` job)
  ...
  Pull requests read-write, "Everything else: **No access**" â€” yet the
  ...
  this problem one step earlier â€” "Dispatch implement and reply" sets
  `DISPATCH_TOKEN: ${{ github.token }}` and calls `GH_TOKEN=
  "$DISPATCH_TOKEN" gh workflow run ...`, leaning on the `act` job's own
  `actions: write` grant â€” but the stop path never got that treatment.
  ...
  already completed â€” nothing was cancelled" while the targeted run is
  ...
  token-aware test would have caught â€” see T064. FR-024/SC-009
  (contradicts).
  ...
  **Status (iteration 8)**: fixed. `DISPATCH_TOKEN: ${{ github.token }}`
  ...
  409 still produces the already-completed reply including T048's prior-run
  summary; the impl-run cancel is routed the same way â€” plus a defect
  witness showing the pre-fix code 403s and reports it as
  already-completed.
- [X] T063 CRITICAL (contradicts): the "Dispatch implement and reply"
  ...
  under the step's `env` `GH_TOKEN` â€” the App token again, so it 403s.
  The step runs under `set -euo pipefail`, so the `run_url=$(gh run list
  ...)` assignment aborts the whole step â€” *after* the implement
  ...
  Story 1, in-scope change â€” the MVP this feature exists for) is that
...
- [ ] T064 Add a machine-checked gate for the defect CLASS T062 and T063
  ...
  times â€” spec 005's `gh workflow run` 403 (fixed by dispatching with the
  default token plus a job-level `actions: write`), and T062/T063 above â€”
  ...
  IV, and constitution I â€” the repo is its own first example). Add it to
  ...
  effect for that invocation â€” step-level `env.GH_TOKEN`, job-level env,
  or a per-command override like `GH_TOKEN="$X" gh ...` â€” and classify it
  ...
  run list` for its agent) â€” each one is either a real defect or an
  ...
  permission failure is reported to the user as a successful outcome â€”
...
- [X] T065 `pr-conversation.act`'s default allowed-tools list grants the
  ...
  which per `docs/setup.md` has no Actions permission â€” so any of the
  ...
  **Status (iteration 8)**: dropped, as recommended â€” `Bash(gh run
...
- **Foundational (Phase 2)**: Depends on Phase 1 (job skeletons must exist
  before adding steps to them) â€” BLOCKS all user stories.
...
- **User Story 7 (Phase 9)**: Depends on User Story 2 (T016, T018) AND
  User Story 4 (T027) â€” it verifies artifacts those phases create.
...
- **User Story 1 (P1)**: Independently implementable and testable after
  Foundational â€” the first story to exercise the fold-in mechanism User
...
- **User Story 3 (P2)**: Independently implementable and testable after
  Foundational alone â€” no dependency on User Stories 1 or 2's routes.
...
- **User Story 6 (P3)**: Independently implementable and testable after
  Foundational alone.
- **User Story 7 (P3)**: Verification-only â€” depends on User Stories 2 and
...
- Within Foundational: PR-identity gate (T003) â†’ actor gate (T004) â†’
  staging (T005) â†’ classify step (T006) â†’ confirmation gate (T007) â†’
  announcement (T008) â†’ relay gate (T009) â†’ shared act-job agent step
  (T010), in that order â€” each later step depends on the prior one's
  output.
- Within User Story 1: fold-in (T011) â†’ dispatch (T012) â†’ reply (T013) â†’
  verification (T014).
...
- Within Foundational and every user-story phase, almost every task edits
  ...
  job, or across the two dependent jobs) â€” treat as sequential, not `[P]`,
...
```bash
# Launch together â€” two different workflow files:
...
```bash
# Launch together â€” four different doc/contract files:
...
Task: "Add a Stage 10 â€” PR Conversation section to docs/architecture.md"
...
4. **STOP and VALIDATE**: Run `quickstart.md` scenarios 1â€“2 independently
5. This alone delivers SC-001 â€” the headline value the maintainer's own
...
1. Setup + Foundational â†’ the classify/announce scaffold is ready for
   every category
2. Add User Story 1 â†’ validate scenarios 1â€“2 â†’ mergeable increment (MVP)
3. Add User Story 2 â†’ validate scenarios 3â€“5 â†’ mergeable increment
   (new-functionality routing and the small-change spin-off both work)
4. Add User Story 3 â†’ validate scenarios 6â€“7 â†’ mergeable increment
   (push-back and needs-info)
5. Add User Story 4 â†’ validate scenario 8 â†’ mergeable increment
   (manual-step/permission handling)
6. Add User Story 5 â†’ validate scenarios 9â€“12 â†’ mergeable increment
   (propose-and-confirm and the stop procedure)
7. Add User Story 6 â†’ validate scenarios 13â€“14 â†’ mergeable increment
   (questions answered with zero mutation)
8. Add User Story 7 â†’ validate scenario 15 â†’ mergeable increment (full
   traceability confirmed across everything built so far)
9. Polish â†’ documentation, contract-doc update, static validation, and
...
"traceability" code path to build â€” User Story 7's task is confirming, by
