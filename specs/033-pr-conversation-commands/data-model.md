...
| `requires-confirmation` | boolean | Computed deterministically from `category` against `WING_COMMANDER_PR_CONVERSATION_CONFIRM_CATEGORIES` (D9) — not agent-decided, so autonomy configuration can never be influenced by comment content (FR-020). Computed AFTER `small-unrelated-change`'s size backstop (D8) has already rewritten `category`/`fold-target`/`drafted-content` in place, so this check (and `act`'s `environment:` binding, which reads its result) always sees the backstop-corrected category, never the raw classify-time one — see D8's ordering note. Forced false when `permission-match == "confident"` (below): a leg that only posts a link has nothing to confirm. The classification array is emitted sorted with every `requires-confirmation: false` leg first, because `act`'s `max-parallel: 1` matrix gives a leg awaiting approval the only slot — that ordering, not concurrency, is what makes FR-020's "act immediately on in-PR actions" true (T079). |
| `permission-match` / `permission-match-url` | enum `confident` \| `none`; string | Resolved deterministically in `classify-and-announce`, alongside `requires-confirmation` and for the same ordering reason (T080): the `gh search prs --label permission-request` dedup (D11) decides whether a `manual-step-permission` leg creates an artifact at all, so it must precede the announcement. `confident` ⇒ the leg only links the prior conversation: `out-of-pr` is false (no lifecycle-issue cross-post), `requires-confirmation` is forced false (nothing to approve), and `planned-action` is rewritten to say what will actually happen. `none` for every other category. |
| `out-of-pr` | boolean | Computed deterministically: true iff this leg will create an artifact outside the PR (`new-functionality`+`new-spec`, `small-unrelated-change`, or `manual-step-permission` with `needs-permission` **and** no confident dedup match). Drives both the lifecycle-issue cross-post of the announcement (FR-013) and, in `act`, the requirement that a `SpinOffArtifact` URL actually exist (T081). |
