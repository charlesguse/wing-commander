...
| `requires-confirmation` | boolean | Computed deterministically from `category` against `WING_COMMANDER_PR_CONVERSATION_CONFIRM_CATEGORIES` (D9) — not agent-decided, so autonomy configuration can never be influenced by comment content (FR-020). |
...
| Field | Type | Notes |
|---|---|---|
| `target-run-url` | string \| null | Extracted by scanning the PR thread for the most recent bot-posted `IntentAnnouncement` (D10); `null` if none found (e.g. the prior action already finished and its announcement is stale/unmatched). |
