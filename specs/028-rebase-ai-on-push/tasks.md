...
- [X] T012 [US3] Confirm `.github/workflows/rebase.yml` has zero diff from its pre-feature state after T002/T003 land — FR-005/FR-006 require the abort/escalate safety behavior to hold by construction, since this file is not part of the fix's scope; a diff here would indicate accidental scope creep into the published stage contract.
  - Confirmed: `git diff fec7d37..HEAD -- .github/workflows/rebase.yml` (fec7d37 being the tasks-generation commit that predates this implementation run) is empty. `rebase.yml` was not touched by T002/T003 or any task in this cycle — FR-005/FR-006 hold by construction.
- [ ] T013 [US3] Validate quickstart.md Scenario 4: advance `main` with a commit that conflicts with a scratch branch in a genuinely ambiguous/contradictory way, push it to `main` so the `push` → `redispatch` → `workflow_dispatch` path (T002/T003) is exercised, and confirm the scratch branch's tip is byte-for-byte unchanged from before the run (no half-rebased state, no force-push) and the lifecycle issue gets a new comment plus the `rebase:blocked` label carrying the existing `<!-- wing-commander-rebase: blocked ... -->` marker format (FR-004, FR-005, SC-003).
  - Not run live: this headless run cannot push a conflicting commit to `main` (out of the push-only-to-the-spec-branch scope granted here). Desk-check only: since T012 confirms `rebase.yml`'s abort/escalate logic is byte-for-byte unchanged, and T003's `rebase` job passes it identical `with:`/`secrets:` regardless of whether `workflow_dispatch` originated from `redispatch` or a human, the abandon+escalate path's own behavior is unaffected by this feature by construction — but that a real induced conflict actually reaches it via the new `push` → `redispatch` → `workflow_dispatch` chain has not been proven live.
- [ ] T014 [US3] Validate quickstart.md Scenario 5: confirm the relocated loop guard on `redispatch` (T003) behaves identically to the original — a bot-authored push (`<slug>[bot]`) to `main` evaluates `redispatch`'s `if:` false, so no `redispatch` run starts and consequently no second `workflow_dispatch` run is queued (research.md R4).
  - Not run live (same `main`-push constraint as T013). Desk-checked by inspection: `redispatch`'s `if: ${{ github.event_name == 'push' && !endsWith(github.actor, '[bot]') }}` carries forward the exact `!endsWith(github.actor, '[bot]')` clause from the pre-fix single `rebase` job, now additionally scoped to `push` — for a bot-authored push this evaluates false exactly as it did before the split, so `redispatch` is skipped and no `gh workflow run` call is made, meaning no second `workflow_dispatch` run is ever queued. Confirmed by construction against the shipped YAML, not against a live bot-authored push.
...
- [X] T015 [P] Update `docs/architecture.md`'s "Rebase" section (the `**Trigger**:` line, currently "`push` to main (skipping `*[bot]` actors) + nightly schedule") to describe the new shape: push redispatches via `workflow_dispatch` to a supported event before reaching the stage; schedule and manual `workflow_dispatch` reach it directly (research.md R9).
  - Updated the `**Trigger**:` line to describe the redispatch hop for `push` and the direct path for `schedule`/manual `workflow_dispatch`.
- [X] T016 [P] Update `docs/adoption.md` §8's `wing-commander-rebase.yml` copy-paste template (currently the exact pre-fix single-job pattern) to the fixed two-job pattern from contracts/rebase-wrapper-delta.md — `on:` gains `workflow_dispatch: {}`, and the `jobs:` block shows the `redispatch`/`rebase` split — so a new adopter following the guide no longer copies the defect this feature fixes (research.md R9).
  - Updated the template to the two-job `redispatch`/`rebase` split, keeping the pinned `uses: charlesguse/wing-commander/.github/workflows/rebase.yml@v2` form (adoption's external-pin convention, distinct from this repo's own `./`-local-path wrapper) and the existing `pipeline-repo-token` secret note intact.
- [X] T017 Run quickstart.md's full scenario set (1-9) against the finished workflows and record, per specs/020/022/025's validation-record convention, which scenarios were exercised via a live triggered run versus desk-checked by inspection only.
  - All nine scenarios were desk-checked by inspection/manual trace only in this headless implement run; none were exercised via a live triggered pull request, push to `main`, or `gh workflow run` invocation against the live repository — this run is scoped to commit/push only to `spec/028-rebase-ai-on-push`, with no PRs and no pushes to `main`. Scenarios 1, 2, and 4 (T004, T005, T013) additionally could not use `python3`/`bash -n` execution aids; their desk-checks read the shipped YAML directly. Scenarios 6-9 (T008-T011) were traced by hand against Gate 6's logic since `python3` is not invokable in this run. **Open item for a human maintainer**: FR-012 specifically requires Scenario 1 to be validated against a genuine induced conflict on the push path (not inferred from source) before this feature is fully verified end-to-end — that live run, plus Scenarios 2-7's live equivalents, remain outstanding.
- [X] T018 [P] Confirm SC-005 directly: run Gate 6's logic (T007) against the complete current `.github/workflows/*.yml` set and confirm the only file whose flagged-event status changed as a result of this feature is `wing-commander-rebase.yml` (from would-have-flagged-`push` pre-fix to clean post-fix) — no other agent-bearing wrapper's lint status moves.
  - Per T010's hand trace, every wrapper other than `wing-commander-rebase.yml` is unaffected by this feature (their `on:`/`if:` shapes are unchanged) and each already resolves to zero flagged events. `wing-commander-rebase.yml` itself moves from would-have-flagged `{push}` under its pre-fix shape (T008's trace) to zero flagged events under the shipped post-fix shape — the only file whose status changes. SC-005 holds.
