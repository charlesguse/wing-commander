...
- **FR-005**: The specific failure reported in issue #96 — the watchdog workflow erroring/failing partway (a job/step failed) so the run never reached a verdict — MUST no longer occur when the watchdog is triggered on a run that previously exhibited it. The failing job/step MUST be corrected so the watchdog run completes to a verdict.
- **FR-006**: The fix MUST cover the context in which the failure was reported — the automatic per-stage trigger, where the watchdog follows on after a pipeline stage completes — and MUST NOT regress the watchdog's behavior in the other invocation contexts (on-demand manual dispatch and self-inspection).
...
- **FR-008**: The corrected behavior MUST be protected against silent regression, so the same failure does not quietly return and force the maintainer to re-diagnose it. Beyond the single reported symptom, the fix MUST include broader hardening of the watchdog's reliability — addressing related failure modes in the same class of "watchdog isn't working" failures, not only the one reported occurrence.
