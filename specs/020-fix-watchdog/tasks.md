...
- [X] T009 [P] Add a fifth bullet to `docs/architecture.md`'s Stage 9 — Watchdog Design list (after the existing `act` bullet, ~line 363), documenting `report-unhandled-failure`: `needs: [collect, diagnose, triage, act]`, `if: always()`, no-ops when every job succeeded, otherwise posts a "could not inspect this run: the `<job>` job ended `<result>` unexpectedly" report (to the lifecycle issue if one resolves, else the run summary) — the structural safety net that makes a hard job failure in any of the four preceding jobs still end in a truthful verdict instead of silence (research.md R1/R4).
- [X] T010 [P] Validate the edited `.github/workflows/watchdog.yml` parses as valid YAML and passes `bash -n` on its embedded `run:` scripts, matching `.github/workflows/lint-workflows.yml`'s own CI checks — run locally or trigger `lint-workflows.yml` itself (`actionlint`/`yamllint` per specs/019's precedent, if `python3`/`bash -n` aren't available in this environment).
- [X] T011 Run quickstart.md's full scenario set against the finished workflow: Scenario 1 (automatic per-stage trigger still reaches a verdict on a normal, non-failing run — confirms no regression in the common path), Scenario 2 (User Stories 1-3, T003-T008, fault injection proves the safety net), Scenario 3 (reproduces T001's confirmed failure class and confirms the fix yields a valid verdict where the original symptom occurred), Scenario 4 (manual `workflow_dispatch` and self-inspection both still reach a verdict exactly as before — confirms FR-006's no-regression requirement across all three invocation contexts), and Scenario 5 (median time from stage completion to verdict stays under 10 minutes, confirming the new job's `if:`-guarded no-op path adds no material latency on the common case). Record in the PR body which scenarios were exercised via a live dogfooded run versus desk-checked only.

**Validation record (desk-checked only — no PR exists in this pipeline flow to
record it in, and `gh` access to trigger/read live runs was unavailable in
this headless implement run, per T001's note in research.md R3)**:

- *Scenario 1* (automatic trigger, no-failure path): Desk-checked. Every
  pre-existing job (`collect`/`diagnose`/`triage`/`act`) is unmodified;
  `report-unhandled-failure`'s `if:` gate (`steps.gate.outputs.any-failed`)
  evaluates false whenever all four `needs.*.result` are `success` or
  `skipped`, so its report/issue/job-urls steps are skipped and it posts
  nothing — no duplicate verdict on the common path.
- *Scenario 2* (fault injection): Desk-checked, not live-triggered (would
  require pushing a deliberately-broken throwaway branch and running
  `workflow_dispatch`, which this headless run did not do). Traced by
  inspection: a `collect` failure sets `needs.collect.result == 'failure'`,
  `diagnose`/`triage`/`act` are skipped by unchanged Actions `needs:`
  semantics, `report-unhandled-failure` still runs (`if: always()`), its
  gate step sets `any-failed=true`/`failed-jobs=collect:failure`, and the
  two-branch report step posts to the lifecycle issue (or run summary
  fallback) with the exact contract wording.
  T001's live confirmation against issue #96's actual run is still open —
  see the flag left in research.md R3 for a human maintainer.
- *Scenario 4* (no regression in manual dispatch / self-inspection): Desk-
  checked. No existing step in `collect`/`diagnose`/`triage`/`act`, the
  self-dispatch cap, or the pause switch was modified — the new job is
  strictly additive at the end of the job graph.
- *Scenario 5* (timing): Desk-checked. `report-unhandled-failure` has no
  agent/LLM step; on the common (no-failure) path its steps evaluate one
  `if:` guard each and no-op, adding negligible latency ahead of SC-006's
  10-minute budget.

**Open item for a human maintainer**: run Scenario 2's live fault injection
(or confirm T001 directly against a still-inspectable run) before treating
this feature as fully verified end-to-end; the design and every desk-checked
trace above are consistent with the contract, but no step in this
implementation run actually observed a live GitHub Actions execution of the
new job.
