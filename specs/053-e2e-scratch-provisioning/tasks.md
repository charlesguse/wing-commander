...
## Maintainer Feedback (round 2)

Findings from charlesguse's second code review of PR #374 at head 8334bdb. Confirmed against the current branch by reading `.github/scripts/e2e-provisioning/checks.sh`, `.github/scripts/provision-e2e-target.sh`, `.github/scripts/e2e-provisioning-tests/gh_stub.py`, and `.github/scripts/e2e-provisioning-tests/t9_maintainer_feedback.sh` directly.

- [X] T043 Fix `check_app_installation` (checks.sh:34-51): the local path's fallback `gh api repos/$1/$2/installation` call is App-JWT-only and returns `401` for a maintainer's own token regardless of whether the App is installed, so the local path can never converge `app_installation` to ready even after a human installs the App (breaks FR-015 / US1 Acceptance Scenario 5). Report the local path's `app_installation` element as the declared manual step needing confirmation at https://github.com/settings/installations (not-ready without failing the run) instead of relying on a call that cannot succeed with this credential. Delete `gh_stub.py`'s answer for `repos/OWNER/NAME/installation` (gh_stub.py:239-247) so `t1`/`t3` cannot pass on a stub that diverges from the real API (FR-002, FR-004, FR-006).
- [X] T044 Fix `--check-only` against a hand-onboarded, non-empty, unmarked target (provision-e2e-target.sh `assemble_report` call at the bottom of the script; `check_scratch_marker` in checks.sh): today it correctly emits a full `ReadinessReport` (T037) but still reports `scratch_marker` not-ready, so the run exits 1 and reports the target not-ready overall, violating FR-011 and User Story 2's Independent Test (readiness check against the existing hand-onboarded target must report ready). Make the readiness result for `--check-only` independent of the marker element, or report that element as not applicable on the read-only path, and update `t9_maintainer_feedback.sh`'s T037 case (currently asserting exit code `1`) to match.
- [X] T045 Fix the self-targeting refusal fail-open (provision-e2e-target.sh:94-103): `THIS_REPO="$(this_repo_from_git || true)"` skips the FR-007 refusal entirely when `this_repo_from_git` fails (no origin remote, or an unparsable one), leaving only the marker rail between a mutating run and this repository. On the mutating path, fall back to `GITHUB_REPOSITORY` when `this_repo_from_git` fails, and refuse with a message naming why if neither resolves. Add a test that runs with no resolvable remote and asserts refusal.
- [X] T046 Check the return code of `act_scratch_marker` (provision-e2e-target.sh:224-226, `gh repo edit ... --description`): the script runs under `set -uo pipefail` without `-e`, so a failed marker write (permission, rate limit) is silently ignored and the run proceeds to push wrapper content, leaving a non-empty, unmarked repository that a later run refuses permanently — the exact outcome the marker-first ordering (T038/T039) was meant to prevent. Exit non-zero before any content push if the marker write fails.
- [X] T047 Keep stderr out of the captured value in `check_container_image_pin`'s companion read at checks.sh:168 (`got="$(gh variable list ... 2>&1)"`): folding stderr noise from a successful call into the captured value can produce a false not-ready mismatch. Capture stdout only and check the exit code separately.
- [X] T048 Make `gh_stub.py` emit the scratch-marker em-dash consistently as UTF-8 regardless of host locale: without `PYTHONUTF8=1`, a Windows local run of the gate suite fails 16 assertions in `t8`/`t9` because the stub prints the marker in cp1252 and `has_scratch_marker` mismatches. CLAUDE.md names the local gate run as the PR-time set and maintainers may run it on Windows.
- [X] T049 Verify T042's wrapper-pinning warning is sufficient: it only warns when local `HEAD` differs from the local upstream ref, but an unpushed SHA (or a stale local notion of upstream) can still produce a `ready: true` report with wrappers pinned to a commit unreachable from `spec-kit-scratch`'s Actions runs. State what behavior is intended here and whether a non-fatal warning meets it, or harden it if not.

---

## Phase 8: Convergence

- [X] T050 Reconcile spec.md's FR-015, SC-001, and User Story 1's narrative/Acceptance Scenario 5 with the verified real-API constraint T043 established: `GET /repos/{owner}/{repo}/installation` is App-JWT-only, so no call made under a maintainer's own `gh` authentication (the local, privileged path FR-014 mandates) can ever report `app_installation` ready, regardless of true install state. FR-015 ("MUST converge to ready when re-invoked"), SC-001 ("one confirming re-invocation of the same command"), and User Story 1's own narrative and Acceptance Scenario 5 ("the same command is invoked a second time... reports ready") all currently describe convergence via re-running the local script a second time -- a promise no implementation using a maintainer's own credential can keep. Update this wording to describe convergence via dispatching the readiness check (User Story 2's `auto-update-spec-kit-scratch-preflight.yml`, already implemented and already the mechanism `quickstart.md` was corrected to point at this cycle), leaving the underlying guarantee (one declared manual step, one confirming action, no second manual step introduced) intact. (contradicts, FR-015/SC-001/US1-AC5)

**Resolution notes (Phase 8)**: FR-015, SC-001, User Story 1's narrative, and Acceptance Scenario 5 all now describe convergence via dispatching the readiness check workflow against the target rather than via a second local invocation of `provision-e2e-target.sh`, matching what T043 established and what `quickstart.md` already documented. `data-model.md`'s state-transition diagram was updated the same way for consistency, since it described the same now-impossible local re-invocation path. T032 (the manual quickstart walkthrough against a real disposable repository) remains unchecked: it requires creating a real GitHub repository and performing the App installation by hand in the GitHub UI, which this automated session has no credential or tooling to do; it is left for a maintainer to run and record.

**Resolution notes (round 2)**: T043 removed the JWT-only `gh api .../installation` call entirely from `check_app_installation` -- it can never succeed for either caller this feature has, so it is no longer attempted at all; the local privileged path now always reports `app_installation` as the outstanding manual step, converging only through `WC_APP_INSTALLATION_KNOWN_READY` (the readiness workflow's existing hint). `gh_stub.py`'s installation-endpoint answer was deleted, and `quickstart.md` steps 3-4 were corrected to point convergence at dispatching the readiness check rather than re-running the local command, since the local command can no longer ever observe it. T044 made `check_scratch_marker` return ready unconditionally when `WC_CHECK_ONLY=true` (exported by `provision-e2e-target.sh` from `--check-only`) -- the schema's `ready` invariant (top-level `ready` iff every element is ready) rules out reporting the element itself not-ready while still reporting overall readiness, so "not applicable" is represented as ready on the read-only path. T045 added a `GITHUB_REPOSITORY` fallback for the self-refusal resolution, refusing outright on the mutating path (never on `--check-only`, which mutates nothing) when neither signal resolves. T046 now checks `act_scratch_marker`'s exit code and halts before any further privileged action if the write fails. T047 stopped folding stderr into the compared value in `check_container_image_pin`, capturing stdout only and re-invoking once (stderr-only) purely to classify a failure. T048 added an explicit UTF-8 `reconfigure()` on the stub's stdout/stderr. T049 was verified and left as a warning by design: hardening it to a hard failure would block the normal commit-then-push-later local development sequence (including this implement stage's own gate-check-before-push cycle) far more often than it would catch a real mistake, and the failure mode if a SHA is truly never pushed is not vacuous -- the target's own Actions run fails loudly and immediately at the unresolvable `uses:` ref, right at the actual defect. Regression coverage for all of T043/T045/T046/T047/T048 was added to `t9_maintainer_feedback.sh`; `t2`/`t3`/`t8`/`t9`'s existing scenarios that relied on `gh_stub.py`'s now-removed installation state were updated to signal installation through `WC_APP_INSTALLATION_KNOWN_READY` instead (centralized in `lib.sh`'s `seed_fully_onboarded`, which now sets/unsets that hint from its own with-install argument).
