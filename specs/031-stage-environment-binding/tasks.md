...
- [X] T001 Verify the pinned `actionlint` 1.7.7 (matching `release.yml` Gate 1a) accepts the job-level `environment:` mapping form and its `deployment` sub-key (research.md D8 — flagged, not resolved, in planning: `deployment` is not part of any GitHub-published schema this plan can cite). Run actionlint 1.7.7 against a scratch workflow file/job declaring `environment: {name: ${{ inputs.environment }}, deployment: ${{ inputs.environment-deployment }}}`. If it accepts the mapping cleanly, proceed with T003–T012 as written and note this in T013. If it rejects the `deployment` key, record the exact failure text and prepare an `-ignore` pattern for `release.yml` Gate 1a mirroring the existing `job_workflow_sha` precedent (research.md D8) — apply it in T013, in the same PR, per D8's stated fallback. **Outcome (2026-08-06):** actionlint 1.7.7 rejects the key (`unexpected key "deployment" for "environment" section. expected one of "name", "url"`), 30 diagnostics, nothing else. The `-ignore` fallback was applied and then replaced on review: Gate 1a now counts the diagnostics across all ten files instead of suppressing them, so the allowance cannot hide an unrelated error or go stale silently. See research.md D8's Outcome section.
...
- [X] T010 [P] [US1] Wire `.github/workflows/rebase.yml`: add the two `workflow_call` inputs at the file level; add T002's block to both jobs (`discover`, and the matrixed `rebase` job) — the block is written once in the matrixed job's own definition. **Correction (2026-08-06):** this task's parenthetical claimed `environment:` is "evaluated per job, not per matrix instance". That is wrong, and the comment it produced in `rebase.yml`/`watchdog.yml` has been rewritten: `matrix` is an available context for the key and every leg gates independently, so N legs mean N approvals and N deployment records. Only the *authoring* is once per job; the *evaluation* is once per leg.
...
- [X] T016 [P] Add a "Deployment environments" section to `docs/adoption.md` (implementation-stage edit per plan.md's Project Structure) covering all five FR-012 caveats: the per-iteration (per-run) approval behavior and the once-per-feature-stage workaround; the concurrency-slot interaction (a pending job holds its per-spec concurrency slot); the environment-secrets non-goal and why it fails silently (kebab-case secret names, wrapper-resolved `secrets.*` in the environment-less calling job); the create-on-reference caveat (a typo yields a new, unprotected, ungated environment); and the private-repo paid-plan (Team/Pro) prerequisite. Add a short "Stage reference" intro bullet pointing adopters here. **Revised (2026-08-06, code review):** the section now carries seven caveats, not five — a per-job approval multiplier with a per-stage count table, and the OIDC subject-claim change that breaks Bedrock adopters' AWS trust policies, were added; the concurrency bullet was corrected (a newer call cancels the pending one, it does not queue behind it) and the plan-tier bullet was corrected to Pro/Team/Enterprise for environments and Enterprise for required reviewers and wait timers. **Revised again (2026-08-06, second code review):** the environment-secrets bullet gave the wrong reason — it blamed the stages' kebab-case secret names, which are `workflow_call` parameter names and not stored-secret names at all (a different namespace from the one that forbids hyphens). The reason is only that the wrapper resolves `secrets.*` in its own environment-less calling job, which GitHub's docs confirm cannot be bridged (`on.workflow_call` has no `environment` keyword). Corrected here, in `spec.md` (both statements), and in `contracts/environment-binding.md`.
...
- [X] T020 Grep all ten stage files for the FR-013 traceability comment fixed in T002, confirming it precedes every one of the 30 `environment:` blocks — no block added without its comment. Distinct from T013's broader plumbing check: this pass verifies comment presence specifically, so a future silent upstream change to any of the four empirical behaviors stays detectable (FR-013). **Revised (2026-08-06, second code review):** this pass checked *presence*, not *text*, and so passed while `intake.yml` alone still carried the pre-08-06 wording — which cited the empirical item covering a YAML literal (`deployment: false`), not the expression form every binding actually ships. Corrected to the standardized block; 30 of 30 now match.
- [X] T022 (2026-08-06, second code review) Add `lint-workflows.yml` **Gate 7** — the PR-time coverage check for FR-004's uniformity claim, which nothing else could make: T013/T015's greps and this task list's own desk-checks were one-time human passes, and `release.yml` Gate 1a counts the bindings it can *see*, so neither can notice a job that carries none. The gate derives the published-stage set from `on.workflow_call` rather than a list (a hardcoded list is how two stage files went unlinted for a release, issue #149), asserts both inputs are declared with their contract types and defaults, and asserts every job forwards both sub-keys verbatim; jobs whose body is `uses:` are exempt, and finding zero stages is itself a failure. Ships with `.github/scripts/verify-gate-7.py`, which runs the shipped gate — extracted from the workflow, never copied — against 14 synthetic stages carrying one known defect (or known non-defect) each, per the Gate 5/6 precedent that an unfired detector is indistinguishable from a broken one.
- [X] T021 Walk quickstart.md's full scenario set (1–9) end-to-end against the finished workflow files, recording in the PR body which were exercised live (a scratch public adopter repository, per the spec's Assumptions) versus desk-checked only. Scenarios 1, 2, and 8 are verifiable in this repository alone (T013, T015); Scenarios 3, 4, 5, 6, 7, and 9 require the scratch adopter repository and remain the acceptance vehicle for GitHub's own protection-rule behavior, not something this repository's CI exercises. **Revised (2026-08-06, second code review):** quickstart.md's prerequisite was the last place still claiming private repos need Team/Pro; it now states what the corrected docs state (Pro/Team/Enterprise for the environment, Enterprise for required reviewers and wait timers), which matters precisely here — provisioning the wrong tier makes these scenarios silently pass against a gate that never fires.
