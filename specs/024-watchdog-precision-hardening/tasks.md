...
- [ ] Fix the stale comment at `.github/workflows/watchdog.yml:1372` ("...and falls back to the normalizedFacts path in triage") — that fallback branch was deleted by T030's fingerprint-basis change; update the comment to match the current signal-id-only fingerprint step.

## Maintainer Feedback

- [ ] Make the act job's empty-dedup case explicitly report-only. The "missing triage-decision artifact → treat as report-only" fallback in `.github/workflows/watchdog.yml` (around `:2224-2235`) sets `dedup=""`, but the `Ensure pipeline-defect issue` guard only excludes `data-integrity` and `unknown`, so an empty dedup value falls through to the comment-on-existing-issue branch and executes `gh issue comment ""` (around `:2386`), which fails the job under `bash -e`. Either have the fallback set a value the guard already treats as report-only, or make the guard treat an empty dedup the same way it treats `unknown` (`:2018-2025`, `:2348`, `:2442`) — one mechanism, applied consistently at every dedup consumer.
