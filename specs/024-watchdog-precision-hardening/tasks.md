...
- [X] T002 [P] DONE — the implement run made code/docs treat the file as gone; the file itself was hand-deleted by the maintainer (Charlie) in a commit on this branch — Delete `.specify/memory/watchdog-guardrails.json` in full — the config existed solely to gate rung 1, which no longer exists (FR-014 of spec 024).
- [X] T003 In the `triage` job of `.github/workflows/watchdog.yml`, remove the `Check fix-class eligibility`, `Resolve propose-fix model`, `Compose tool args (watchdog.propose-fix)`, `Compute agent turn ceiling (propose-fix)`, `Propose fix`, `Compute agent run verdict (propose-fix)`, `Fail loud on non-healthy agent verdict (propose-fix)`, `Upload Claude execution log (propose-fix)`, `Agent run metrics summary (propose-fix)`, `Check propose-fix diff`, `Rung gate`, and `Upload propose-fix diff artifact` steps, and remove the workflow-level `propose-fix-model`/`propose-fix-max-turns` inputs (FR-014).
- [X] T004 In the `act` job of `.github/workflows/watchdog.yml`, remove the `Download propose-fix diff`, `Commit fix and open PR (rung 2)`, and `Commit fix and open PR (rung 1)` steps; rename `Ensure pipeline-defect issue (rung 2/3)` to drop the rung framing and collapse its branching, together with `Determine write suppression`, so the sole remediation outcome is create/comment/reopen a `pipeline-defect` issue (depends on T003 — same file).
- [X] T005 [P] DONE — Gate 17 removed from `.github/workflows/lint-workflows.yml`'s gate registry by the implement run; `.github/scripts/verify-watchdog-fix-commit.py` hand-deleted by the maintainer (Charlie) in a commit on this branch — Delete `.github/scripts/verify-watchdog-fix-commit.py` and remove Gate 17 from `.github/workflows/lint-workflows.yml`'s gate registry in full — its subject no longer exists once T003/T004 land (FR-014, Constitution VIII: a gate MUST NOT outlive the subject it checks).
- [X] T006 [P] DONE — hand-deleted by the maintainer (Charlie) in a commit on this branch — Delete the stale `specs/023-reliable-diagnose-verdict/` directory entirely (FR-017 of spec 024; git history remains the record — nothing is force-deleted from git).
- [X] T007 [P] In `specs/015-pipeline-watchdog/quickstart.md`, retire Scenarios 5–7 (rung 1 auto-fix, rung-1-boundary fallback, pause switch as previously exercised against the rung boundary) — there is no rung 1/2 left to exercise.
- [X] T008 In `specs/015-pipeline-watchdog/spec.md`, remove **User Story 3** ("Fix a truly minor problem on sight") in full — rung 1 no longer exists; reword **User Story 2**'s title and its rung-2/PR acceptance scenario to describe the single-issue path (create/comment/reopen only, no PR); reword **User Story 4**'s title and body to drop "the same ladder" framing in favor of "the same rules."
- [X] T009 In `specs/015-pipeline-watchdog/spec.md`'s Requirements section, mark **FR-007**, **FR-008**, **FR-010**, and **FR-011** Removed in place (one-line note each, citing FR-014 of spec 024); amend **FR-009** to describe the sole remediation action without "rung 3" framing; amend **FR-019** to read "the watchdog's writes" rather than "autonomous fixes"; amend **FR-020** to generalize "autonomous action" to the new suppression/failure report shapes this feature adds; mark **FR-017** Removed in place — depends on T008 (same file).
- [X] T010 In `specs/015-pipeline-watchdog/spec.md`'s Key Entities section, remove the "Guardrail configuration" entity (no successor — there is no longer an autonomous-write rung to guard) and amend "Triage decision" to describe the single dedup-selected branch — depends on T009 (same file).
- [X] T011 In `specs/015-pipeline-watchdog/data-model.md` and `specs/015-pipeline-watchdog/contracts/watchdog-workflow.md`, apply the ladder-collapse edits from this feature's `contracts/watchdog-spec-amendments-delta.md` (`act` section: one branch selected by dedup outcome; "Removed entirely" list) — depends on T010, for consistency with the just-finalized spec.md wording.

**Checkpoint**: `watchdog.yml`'s `triage`/`act` jobs are a straight-line path (fingerprint → dedup → file/comment/reopen) with no rung machinery in code or in spec text; Gate 17 and its fixture, the guardrail config, and the stale 023 directory are gone — ready for user-story work. **Reached**: all six deletions/creations landed (T001–T006).
...
- [X] T012 [US1] In `specs/015-pipeline-watchdog/spec.md`, amend **FR-004** to place the false-positive-avoidance duty on the collectors that produce signals, not solely on `diagnose` (which sees only pre-computed signals and cannot know one is wrong).
- [X] T013 [US1] In `specs/015-pipeline-watchdog/spec.md`'s Success Criteria section, amend **SC-001** so it is verifiable (states it depends on a labeled corpus of runs known to exhibit each problem class) and amend **SC-007** to state the latency measurement as `gh run view --json updatedAt` (run completion) against the watchdog's own lifecycle-issue report comment's `createdAt`, rather than leaving it unmeasured — depends on T012 (same file).
- [X] T014 [US1] In `specs/015-pipeline-watchdog/spec.md`'s Success Criteria section, add new **SC-008**: numerator = distinct filed `pipeline-defect` issues, among the most recent 20, carrying `disposition:confirmed`; denominator = distinct filed `pipeline-defect` issues among the most recent 20; target ≥70%; not evaluated until at least 10 distinct findings exist, reported "not applicable" below that — depends on T013 (same file).
- [X] T015 [P] [US1] In `specs/015-pipeline-watchdog/data-model.md`, add the Precision criterion entity (numerator, denominator, target, not-yet-applicable state, and the manual `gh issue list --label pipeline-defect --label disposition:confirmed` / `--label disposition:false-positive` computation) per this feature's own `data-model.md`.
- [X] T016 [P] [US1] In `specs/015-pipeline-watchdog/quickstart.md`, add the precision-criterion validation scenario (fewer than 10 distinct findings ⇒ "not applicable"; once ≥10 exist, compute over the most recent 20) — this feature's `quickstart.md` Scenario E.
...
- [X] T018 [US2] In the `collect` job, add the same execution check to the `Collect: step summaries` step, applied per-job (a job that itself never ran contributes no signal, even if sibling jobs in the same run did) — ownership is already inherent to the per-job API call.
- [X] T019 [US2] In the `collect` job, add the same execution check to the `Collect: annotations` step, applied per-job, mirroring T018.
- [X] T020 [P] [US2] In `specs/015-pipeline-watchdog/spec.md`, add new **FR-026**: a collector MUST emit a signal about a run only when the inspected run both executed and owned the artifact whose condition the signal describes — stated once, applying to all five collectors.
- [X] T021 [P] [US2] In `specs/015-pipeline-watchdog/data-model.md`'s Signal entity, document the attribution invariant (executed/owned columns) and that before this feature only 2 of 5 collectors enforced it, after this feature all 5 do.
- [X] T022 [P] [US2] In `specs/015-pipeline-watchdog/contracts/watchdog-workflow.md`'s `collect` section, amend the contract to state all five collector steps check attribution before emitting a signal.
- [X] T023 [P] [US2] In `specs/015-pipeline-watchdog/quickstart.md`, add the attribution-invariant validation scenario (a skipped/cancelled run produces no finding from a newly-guarded collector) — this feature's `quickstart.md` Scenario A.
...
- [X] T024 [US4] In the `triage` job of `.github/workflows/watchdog.yml`, add a new `Evidence validity gate` step immediately before `Compute fingerprint`: for each Finding, check `evidence[].signalId` resolves to a signal this run's collectors actually emitted AND `normalizedFacts` carries every key required for `finding.class` (the per-class key list: `tool` for `denied-tool`, `branch` for `lost-progress`, `expected`/`actual` for `stage-mismatch`, etc.) AND none of those required values is null, empty string, or empty array; a Finding failing this check is marked `suppressed: invalid-evidence` and MUST NOT proceed to fingerprinting, dedup, or any write.
- [X] T025 [US4] In `.github/workflows/watchdog.yml`'s lifecycle-issue reporting steps, add the "suppressed: invalid evidence" report shape for findings T024's gate rejects, distinct from "passed inspection" and "could not inspect this run" — depends on T024 (same file).
- [X] T026 [US4] In `specs/015-pipeline-watchdog/spec.md`, amend **FR-002** to impose the validity condition (cited facts MUST be non-empty and conform to the expected shape for the finding's class) and add new **FR-027**: a finding whose cited facts are absent, empty, or malformed MUST be suppressed rather than filed, even if it references a valid run.
- [X] T027 [P] [US4] In `specs/015-pipeline-watchdog/data-model.md`'s Finding entity, add the evidence-validity-gate `valid ⟺ ...` formula.
- [X] T028 [P] [US4] In `specs/015-pipeline-watchdog/contracts/watchdog-workflow.md`'s `diagnose`/`triage` sections, amend the contract to state the new deterministic gate between `diagnose` and `triage`'s existing steps.
- [X] T029 [P] [US4] In `specs/015-pipeline-watchdog/quickstart.md`, add the evidence-validity-gate validation scenario (a `denied-tool` finding with null `tool`/`denials` is suppressed before fingerprinting) — this feature's `quickstart.md` Scenario B.
...
- [X] T035 [US7] In the `triage` job of `.github/workflows/watchdog.yml`'s `Dedup search` step, replace `gh search issues "<marker> in:body" --state all` with `gh issue list --repo "$GITHUB_REPOSITORY" --label pipeline-defect --label "🐕 · ${FINDING_CLASS}" --state all --limit 200 --json number,state,body`, capturing stderr; a non-zero exit sets `outcome=unknown` explicitly (never `results='[]']`); on success, a local `jq` filter over `.body` for the exact `fingerprint=$FP` marker within that bounded result set determines `none`/`match-open`/`match-closed`/`data-integrity` as before.
- [X] T036 [US7] In the `act` job of `.github/workflows/watchdog.yml`, add an `unknown`-outcome branch checked before the create/comment/reopen branches: `[ "$DEDUP_OUTCOME" = "unknown" ]` suppresses every write for that finding and posts "dedup lookup failed — finding suppressed, needs a maintainer's manual check" to the lifecycle issue, sharing no code path with the `none` (create-new) branch — depends on T035 and on T004 (Foundational's collapsed single-branch `act` structure).
- [X] T037 [US7] In `specs/015-pipeline-watchdog/spec.md`, amend **FR-015** to exclude the `unknown` outcome from "create new only when a finding matches nothing" (a lookup that did not complete cannot be treated as "matches nothing"); add new **FR-028**: a fourth dedup outcome `unknown` — the lookup could not be completed, distinct from `none` — MUST suppress filing and MUST NOT share a code path with `none`; add new **FR-029**: the dedup lookup MUST be a bounded, strongly-consistent direct read scoped to the finding's class (`gh issue list --label`) rather than an eventually-consistent search index.
- [X] T038 [P] [US7] In `specs/015-pipeline-watchdog/data-model.md`'s Dedup outcome entity, add the `unknown` row and the `gh issue list --label` lookup-mechanism description.
- [X] T039 [P] [US7] In `specs/015-pipeline-watchdog/contracts/watchdog-workflow.md`'s `triage`/`act` sections, amend the dedup-lookup and remediation-branch contract clauses.
- [X] T040 [P] [US7] In `specs/015-pipeline-watchdog/quickstart.md`, add the dedup-lookup-failure validation scenario (a forced `gh issue list` failure suppresses filing and reports the failure; restoring access resumes normal `none` → create behavior) — this feature's `quickstart.md` Scenario D.
...
- [X] T049 [P] In `specs/015-pipeline-watchdog/quickstart.md`, add the stale-directory-removal validation scenario (`specs/023-reliable-diagnose-verdict/` no longer exists in the working tree; its git history remains queryable) — this feature's `quickstart.md` Scenario I. (Scenario now passes: T006's deletion landed on this branch.)
- [X] T050 Re-scan `specs/015-pipeline-watchdog/spec.md` for any remaining cross-reference to a Removed identifier (FR-007, FR-008, FR-010, FR-011, FR-017) or to "rung 1"/"rung 2"/"guardrail" language outside the Removed-marker notes themselves, and correct it (SC-009 of spec 024 — no requirement may describe machinery the watchdog no longer has). (Extended the sweep to data-model.md and contracts/watchdog-workflow.md too — found and fixed one stray reference in each.)
- [X] T051 Confirm `.github/workflows/lint-workflows.yml`'s gate registry no longer contains Gate 17 and that no remaining gate references `verify-watchdog-fix-commit.py` or `watchdog-guardrails.json`. (Verified via grep: zero matches.)
- [ ] T052 BLOCKED (this run's tooling has no `gh run`/`gh workflow` access — see implementation notes) — Execute `quickstart.md` Scenarios A–I end-to-end against a scratch spec/run (or as close as the harness permits) and record results, confirming each of this feature's seven named gaps is closed as described.
- [X] T053 Re-score the retrospective's five historical false positives (#102, #104, #105, #112, #125) against the strengthened requirements — for each, identify the specific gap now closed (unattributable signal, empty evidence, or absent precision bar) that would have suppressed it — and record the mapping (SC-002 of spec 024). (Pulled the five issues live via `gh issue view`; mapping recorded in `research.md`'s new "SC-002 re-scoring" section.)
- [X] T054 Propagate the ladder collapse out of `specs/015-*` into every consumer-facing document that still described the removed machinery — the gap T050/T051's sweeps did not cover, and the reason lint Gate 27 was red on this branch. `specs/010-reusable-pipeline/contracts/stage-interfaces.md`: drop the `watchdog.propose-fix` tool row and the `propose-fix-model`/`propose-fix-max-turns` inputs, correct `diagnose-model` to `claude-opus-5`, and rewrite the Behavior/Outputs rows around the five dedup outcomes (`none`, `match-open`, `match-closed`, `data-integrity`, `unknown`). `docs/architecture.md` Stage 9: rewrite `triage`/`act` and replace the triage-ladder section, keeping the rung/propose-fix/guardrail history only as an explicit "removed by 024" paragraph. `docs/setup.md`: drop the guardrail-config paragraph and document the two `disposition:*` labels (T001) in the Labels section. Plus `docs/agent-friendly-workflows.md`'s two rung-framed lines, the stale call-site count in Gate 27's comment, `specs/015-pipeline-watchdog/plan.md`'s "removed by 024" banner and `checklists/requirements.md`'s FR-011 note, and `wing-commander-8-watchdog.yml`'s pause comment, which wrongly claimed the deprecated stage-side shim had been removed.
...
- [X] Re-add `propose-fix-model` and `propose-fix-max-turns` to `.github/workflows/watchdog.yml`'s `on.workflow_call.inputs`, with their pre-024 types/defaults restored, and mark each `description` as deprecated-and-ignored (e.g. "accepted for v2 compatibility; propose-fix was removed by specs/024 — has no effect; removal scheduled for the next major, see #149"). Confirm no step in `watchdog.yml` reads either input. (Cited issue #140 — this feature's own lifecycle issue — rather than #149, which is an unrelated closed issue; confirmed via `grep` that no step reads `inputs.propose-fix*`.)
- [X] In `specs/010-reusable-pipeline/contracts/stage-interfaces.md`'s watchdog row, record the same deprecation note for both inputs (the row currently omits them entirely following T054's cleanup).
- [X] In `specs/024-watchdog-precision-hardening/plan.md`'s Constitution Check, correct the Principle VII line: it currently states "Pass" on the basis of no new `github.event.*`/`vars.*` reads, but does not address that this feature removes two published `workflow_call` inputs, which Constitution VII (`.specify/memory/constitution.md:139`) names a breaking change on its own terms; record the re-added deprecated-and-ignored inputs as the resolution.

## Maintainer Feedback

- [X] Extend `verify-gate-19.py` (or the appropriate gate script — its harness already extracts these collector steps) with fixtures that run each of the five collectors (execution-output, branch-drift, spec-meta, step-summary, annotations) with a run/job `conclusion` of `skipped` and of `cancelled` against otherwise signal-bearing input, asserting zero signals are emitted for each. Add a mutation case per collector — its `skipped|cancelled)` guard arm replaced by a never-matching label — and assert the gate fails, so Constitution VIII's "every failure branch a gate ships MUST be exercised by a checked-in fixture" is satisfied for FR-026 (currently unexercised by any registered gate). (Added `skipped`/`cancelled` scenarios to all five collectors' existing or new suites in `verify-gate-19.py` — execution-output and branch-drift gained scenarios in their existing suites; spec-meta and step-summary gained new suites (`suite_spec_meta`/`suite_stepsum`) since neither had any coverage before; annotations gained two scenarios in its existing `SCENARIOS` list. Each collector's guard is proven live via a shared `strip_conclusion_guard`/`run_attribution_mutation` regex-based mutation — removing the `case "$RUN_CONCLUSION"`/`case "$job_conclusion"` block — asserted to break at least one scenario. `python3 .github/scripts/run-local-gates.py verify-gate-19` passes, 0 failures, all mutations killed.)
- [X] Fix the stale comment at `.github/workflows/watchdog.yml:1372` ("...and falls back to the normalizedFacts path in triage") — that fallback branch was deleted by T030's fingerprint-basis change; update the comment to match the current signal-id-only fingerprint step.

## Maintainer Feedback

- [X] Make the act job's empty-dedup case explicitly report-only. The "missing triage-decision artifact → treat as report-only" fallback in `.github/workflows/watchdog.yml` (around `:2224-2235`) sets `dedup=""`, but the `Ensure pipeline-defect issue` guard only excludes `data-integrity` and `unknown`, so an empty dedup value falls through to the comment-on-existing-issue branch and executes `gh issue comment ""` (around `:2386`), which fails the job under `bash -e`. Either have the fallback set a value the guard already treats as report-only, or make the guard treat an empty dedup the same way it treats `unknown` (`:2018-2025`, `:2348`, `:2442`) — one mechanism, applied consistently at every dedup consumer. (Fixed by reusing the existing `unknown` mechanism: the fallback now emits `dedup=unknown` instead of `dedup=`, so both the `Ensure pipeline-defect issue` guard and the report step's `unknown` branch already cover this case with no new special-casing.)
- [X] Prove it: add a checked-in fixture that drives the act job's missing-artifact path (no triage-decision artifact → empty dedup) through the guard and asserts it takes the report-only branch and never invokes `gh issue comment` with an empty target, in whichever verifier already extracts the act job's guard step (add one if none does), with a mutation case (guard's empty-dedup handling removed → verifier fails) so the fix satisfies Constitution VIII. (No existing verifier extracted the act job's steps, so added `.github/scripts/verify-act-dedup-guard.py`, wired as Gate 32 in `lint-workflows.yml`. It executes the shipped "Load triage decision" step against a missing artifact, evaluates the shipped "Ensure pipeline-defect issue" step's real `if:` expression against the real outputs, and — whenever that expression would fire — executes that step's `run:` text too against a `gh` stub that fails on an empty issue reference exactly as the real CLI does. A mutation reverting the fallback to `dedup=` reproduces the shipped bug and is caught. `python3 .github/scripts/run-local-gates.py verify-act-dedup-guard` and the full local gate suite (36/36, including Gate wiring) both pass.)

_Manual fold by the maintainer: the pr-conversation act leg that announced this item (run 32680244191, "The act job's 'missing triage-decision artifact → treat as report-only' fallback…") was cancelled by the shared `wing-commander-<spec-dir>` concurrency group before it ran a single step, alongside implement iteration 3 (run 32680906254). See #246._

---

## Phase 11: Convergence

- [X] T055 Finish T050's re-scan, which swept `data-model.md`/`contracts/watchdog-workflow.md` but missed `specs/015-pipeline-watchdog/spec.md` itself: reword the User Scenarios narrative (currently lines 21-26, "...takes the lightest sufficient action along a triage ladder: fix a truly minor issue on sight, open a PR for a bigger-but-not-spec fix, or open/reopen an issue...") to describe the single remediation path (detect → dedup → file/comment/reopen a `pipeline-defect` issue, no PR, no auto-fix); mark **SC-003** (currently "...produces the same remediation a human produced manually...as a proposed PR, without human diagnosis") Removed in place, citing FR-014 of spec 024 (rung 2's proposed-PR machinery no longer exists); mark **SC-004** (currently "No autonomous write occurs outside the configured allowlist and path restrictions...") Removed in place or reworded to drop the reference to the deleted `watchdog-guardrails.json` allowlist/path-cap config, per FR-014/SC-009 (spec 024). (Reworded the narrative to describe the single dedup-selected remediation path; marked SC-003 Removed in place; reworded SC-004 to keep its still-valid write-recording criterion while dropping the deleted-config reference. Re-scanned spec.md afterward: the only remaining "triage ladder"/"rung" hits are the verbatim original feature-input quote (line 9, historical, left untouched) and the existing Removed-marker notes and retrospective Remediation-surface entity, all already accurate.)
