...
- [ ] T002 [P] PARTIAL — code/docs treat the file as gone, but the file itself could not be deleted (this run's tooling has no `git rm`/`rm` access) — Delete `.specify/memory/watchdog-guardrails.json` in full — the config existed solely to gate rung 1, which no longer exists (FR-014 of spec 024).
...
- [ ] T005 [P] PARTIAL — Gate 17 removed from `.github/workflows/lint-workflows.yml`'s gate registry, but `.github/scripts/verify-watchdog-fix-commit.py` itself could not be deleted (this run's tooling has no `git rm`/`rm` access) — Delete `.github/scripts/verify-watchdog-fix-commit.py` and remove Gate 17 from `.github/workflows/lint-workflows.yml`'s gate registry in full — its subject no longer exists once T003/T004 land (FR-014, Constitution VIII: a gate MUST NOT outlive the subject it checks).
- [ ] T006 [P] BLOCKED (this run's tooling has no `git rm`/`rm` access — see implementation notes) — Delete the stale `specs/023-reliable-diagnose-verdict/` directory entirely (FR-017 of spec 024; git history remains the record — nothing is force-deleted from git).
...
- [X] T049 [P] In `specs/015-pipeline-watchdog/quickstart.md`, add the stale-directory-removal validation scenario (`specs/023-reliable-diagnose-verdict/` no longer exists in the working tree; its git history remains queryable) — this feature's `quickstart.md` Scenario I. (Scenario currently fails: see T006 — the directory could not actually be deleted in this run; the scenario text describes the intended end state.)
