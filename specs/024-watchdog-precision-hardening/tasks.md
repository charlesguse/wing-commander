...
- [ ] Prove it: add a checked-in fixture that drives the act job's missing-artifact path (no triage-decision artifact → empty dedup) through the guard and asserts it takes the report-only branch and never invokes `gh issue comment` with an empty target, in whichever verifier already extracts the act job's guard step (add one if none does), with a mutation case (guard's empty-dedup handling removed → verifier fails) so the fix satisfies Constitution VIII.
