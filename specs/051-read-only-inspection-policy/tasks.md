...
- [x] T003 [P] [US1] In `.github/actions/wing-commander-tool-args/action.yml`'s `Compose tool args` step, add a `STATIC_SUFFIX` string per `contracts/gate-extensions.md`'s Gate 21 section — `" Each command in a pipeline or `;`/`&&` chain is checked separately, and a `>`/`>>` redirect or a `cd … &&` prefix is denied regardless of this list — use the Read/Grep/Glob tools, or a single command, for anything multi-step."` — and append it to `shell_commands` once, after every branch of the if/else that sets it has run (immediately before the "5. Emit" comment / `echo "allowed-tools=$effective_allowed"` line, currently line 354), so it lands in the `UNRESTRICTED`, `UNRESTRICTED_EXCEPT`, `EMPTY`, and `ENUMERATED` templates alike (research.md D3).
- [x] T004 [US1] In `specs/010-reusable-pipeline/contracts/stage-interfaces.md`'s per-stage table, add the seven-primitive inspection set (`Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*)`) to the `intake`, `clarify`, and `implement.cycle` rows' Default allowed cell, inserted immediately after `Bash(cat:*)` — the same position `plan.*`/`tasks.*` already use — so the literal-cell order match Gate 27's `compare()` enforces still holds. `implement.retry`'s row already reads "same as `implement.cycle` plus …" and needs no separate edit (the relative-row set comparison inherits `implement.cycle`'s addition automatically).
- [x] T005 [P] [US1] In `.github/workflows/intake.yml`'s `default-allowed-tools` literal (line 572), insert `Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*)` immediately after `Bash(cat:*)`, matching T004's table order exactly.
- [x] T006 [P] [US1] In `.github/workflows/clarify.yml`'s `default-allowed-tools` literal (line 479), insert `Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*)` immediately after `Bash(cat:*)`, matching T004's table order exactly.
- [x] T007 [P] [US1] In `.github/workflows/implement.yml`, insert `Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*)` immediately after `Bash(cat:*)` in **both** the `implement.cycle` `default-allowed-tools` literal (line 687) and the `implement.retry` `default-allowed-tools` literal (line 1117), matching T004's table order exactly in each.
...
- [X] T030 In `.github/workflows/implement.yml`, fix the "Run local gate suite (cycle)" step (line ~703-706) and its mirror "Run local gate suite (retry)" (line ~1160-1163): today both run `python3 .github/scripts/run-local-gates.py` as a plain step with no `id` and no `continue-on-error`, so on a spec branch that is red at cycle start (the normal case — the agent's job in that cycle is to make it green) the step fails and the job dies before the agent ever runs, reproducing the `stalled`-job misdiagnosis #345 documents ("failed before it could run its own steps"). This violates FR-009a, which requires the gate-suite run to degrade to a stage-summary note and never fail the stage. Resolve by EITHER (a) removing both steps and their paired preflights (`gate-suite-preflight-cycle` / `gate-suite-preflight-retry`) — the FR-009 self-check is already available to the agent itself via the `Bash(python .github/scripts/run-local-gates.py:*)` grant Gate 27's `check_mandated_commands` pins, so the pre-agent run is redundant — OR (b) adding `continue-on-error: true` to both steps, giving each an `id`, and passing their outcome into the corresponding agent prompt (e.g. an env/prompt line: "the suite was red at cycle start: <first failure>") so a red gate informs the cycle instead of ending it. Keep the FR-009a preflight only if the suite step stays; remove it too if the step goes. Source: PR #349 review by charlesguse; lifecycle issue #338.

---

## Phase 7: Convergence

- [X] T031 Restore a deterministic, `timeout-minutes: 10`, preflight-gated gate-suite step in `.github/workflows/implement.yml`'s `implement.cycle` and `implement.retry` jobs (same placement research.md D5 specifies: immediately after `Install actionlint for the agent` / its retry-leg equivalent, before `Compose tool args`), reusing the preflight logic T030 removed (`python3 -c "import yaml"`, `command -v jq`, `command -v actionlint`; degrade to a `$GITHUB_STEP_SUMMARY` note, never fail, when one is missing). This time give the gate-suite step itself (not just the preflight) an `id` and `continue-on-error: true`, and surface its outcome — pass, red-at-start with the first failure, or skipped-missing-prerequisite — to the corresponding agent prompt as context (T030's option (b), never applied: T030's actual commit chose full removal instead). Required because T030's removal left FR-009's "under an explicit timeout" and FR-009a's preflight-and-degrade contract unenforced (both now depend entirely on whether/how the agent chooses to invoke `Bash(python .github/scripts/run-local-gates.py:*)` itself), while the canonical policy text already shipped at `specs/010-reusable-pipeline/contracts/stage-interfaces.md:296-300` (landed by T002) still asserts, as present fact, that the check "is permitted only for `implement.cycle`/`implement.retry`, run by a deterministic step ahead of the agent step under an explicit timeout, preceded by a preflight... that degrades to a step-summary note... when one is missing" — a description T030's chosen resolution made false. research.md's D5 "Alternatives considered" already rejected running the suite solely inside the agent's own Bash call, for exactly the reason this gap reproduces: "the FR-009a preflight-and-degrade contract is a deterministic judgment... which a prompt instruction cannot guarantee is honored before the agent tries the command anyway." per FR-009/FR-009a (contradicts)
