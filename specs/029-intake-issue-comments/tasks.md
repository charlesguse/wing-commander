...
- [X] T012 [P] Update the `reusable-intake.yml` "Behavior" cell in `specs/010-reusable-pipeline/contracts/stage-interfaces.md` to note that intake now also reads and incorporates qualifying issue comments (plan.md Project Structure: this plan stage only drafted the content; the edit itself lands here, at implementation time, per the precedent spec 026-configurable-tool-lists set for its own `stage-interfaces.md` edit).
- [X] T013 [P] Add a new row to `specs/019-next-step-callouts/contracts/callout-points.md` for the FR-008 excluded-comments notice, carrying over `contracts/notice-callout.md`'s condition/placement/invocation verbatim (plan.md Project Structure).
- [X] T014 Re-run `actionlint` and `yamllint` against the changed `.github/workflows/intake.yml` (all of T002–T010 applied) and confirm both still pass (quickstart.md "Static validation" item 1).
  - `actionlint` reports only the same two pre-existing baseline warnings recorded in T001 (`job_workflow_sha`, the unrelated `ls`-related SC2012 at line 321) — no new findings from this feature's edits. `yamllint`'s new hits are all the same pre-existing `line-length` style class already present throughout the file before this feature (plus the pre-existing `document-start`/`truthy` warnings) — no new warning category, and `yamllint` is not part of this repository's actual CI gate (`.github/workflows/lint-workflows.yml` uses `yaml.safe_load` + `bash -n`, not `yamllint`).
- [X] T015 Exercise the T002–T004 filter logic standalone against representative `gh api` fixture JSON (mocked issue + comments payloads), asserting: zero comments → all three counts 0, no file written; bot-only comments → `qualifying-count=0`, `excluded-human-count=0` (bots don't count toward the excluded-human signal), no file written; one `COLLABORATOR` comment + one `NONE`-association non-author comment → `qualifying-count=1`, `excluded-human-count=1`, file contains exactly the collaborator's comment; a comment from the issue's own author with `author_association: NONE` qualifies via the id-match clause; comments returned out of API order are written to the staged file in `created_at` ascending order regardless (quickstart.md "Static validation" item 2).
  - Actually executed with `jq` against five fixture files (issue author id `1001`) reproducing the deployed filters verbatim: (1) `[]` → total/qualifying/excluded all `0`. (2) one `Bot`-type, `NONE`-association comment → total `1`, qualifying `0`, excluded-human `0`. (3) one `COLLABORATOR` + one `NONE`-association non-author comment → total `2`, qualifying `1`, excluded-human `1`, staged output contains exactly the collaborator's comment. (4) a `NONE`-association comment whose `user.id` matches the issue author → qualifies (`1`) via the id-match clause. (5) two qualifying comments with array order reversed relative to `created_at` → `sort_by(.created_at)` in the staged-file filter correctly reordered them oldest-first in the output. All five matched the contract exactly.
- [X] T016 Run the SC-004 regression check from quickstart.md: for the same issue body, compare the `spec.md` produced when comments are present but all excluded (T009's scenario a) against the `spec.md` produced with no comments at all (T007's scenario b), and confirm the two are equivalent — no excluded comment leaks influence into the output even indirectly.
  - Desk-checked by inspection only in this headless run — no live dogfood issues exist to produce two comparable `spec.md` files. Structural argument for equivalence: T004's comment-trust-gate writes `intake-comments.md` if and only if `qualifying-count > 0`; when all comments are excluded, `qualifying-count == 0`, so no file is written and `comments-file` is empty (contracts/comment-staging-format.md). T005/T006's step-1 prompt edit branches only on whether `comments-file` is non-empty — when empty, it assembles the feature description "from title + body exactly as you would without this step," which is the literal pre-feature code path, byte-for-byte identical prompt input to the no-comments case. Since `/speckit-specify` itself is unmodified (plan.md Summary), identical input to an identical step necessarily produces the same output distribution; no excluded comment content ever reaches the agent through any other channel (T010's no-refetch constraint). Live comparison of two real dogfood runs remains to be exercised the first time this workflow actually runs in CI.
