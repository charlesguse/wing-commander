...
- **Tie-break toward suppression**: When the watchdog cannot be sure — invalid or malformed cited evidence (FR-027), a dedup lookup that could not complete (FR-028) — it suppresses the write and reports the uncertainty, rather than guessing or falling through to "file it anyway" (spec 024 FR-014/FR-018-FR-020 superseded the original rung/severity tie-break; there is no longer a rung to tie-break between).
- **Remediation surface**: The watchdog's entire remediation surface is one action — create, comment on, or reopen a pipeline-defect issue, selected purely by the dedup outcome (FR-014 of spec 024). The original v1 rung ladder (rung-1 autonomous fixes gated behind an allowlist, rung-2 PRs) is retired; retrospective evidence (`docs/architecture.md`) showed rungs 1–2 never fired across ~200 production runs, so narrowing to the one remediation action that did fire loses nothing in practice.
- **Humans still own merges**: Consistent with the constitution, the watchdog files and comments; humans review and act on anything non-trivial, and the watchdog never merges to `main`.
- **Fingerprint default**: A finding's fingerprint is derived from its problem class plus the deterministic collector signal ids it cites — never model-authored text — chosen so cosmetic run-to-run differences do not change it and the same defect always hashes identically (FR-016, spec 024 FR-006/FR-007).
...
- **Deterministic-judgment principle**: Every gate this feature hardened (FR-002's evidence-validity condition, FR-004's collector-owned false-positive duty, FR-016's deterministic fingerprint basis) is an instance of Constitution Principle IX — judgment that gates a durable action belongs in deterministic code, not an agent's prompt — which spec 024 recorded centrally after re-deriving it three separate times within this one spec's own retrospective.
