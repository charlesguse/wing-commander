...
5. **Given** a run in which the AI-driven stage ran against a scratch repository, **When** the lifecycle issue is narrated, **Then** it names that repository and how to reach it and states that it is deleted when the issue is closed.
6. **Given** a maintainer closes the lifecycle issue, **When** the closure is processed, **Then** the run's scratch repository is deleted.
...
**Independent Test**: Run the existing scenario harness for this workflow and confirm it exercises, and can fail on, each of: a healthy candidate passing, a missing expected artifact failing, a wrong-shape result failing, an AI-driven stage that does not complete failing, the narration carrying the non-clean-bump hint, and the scratch repository being retained on failure and deleted when the lifecycle issue closes. Run it twice and confirm the verdicts are identical — the harness drives controlled stage results, not live agent runs.

**Acceptance Scenarios**:

1. **Given** the scenario harness for this workflow, **When** it runs against the fixed tier, **Then** it asserts a pass for a healthy candidate and a failure for each defective-candidate case above, plus the scratch repository's retain-then-delete lifecycle.
...
- **Deeper tier fails after creating scratch files**: the disposable worktree is still discarded, and nothing reaches this repository's real `specs/` tree or any pushed branch. The run's scratch repository is deliberately kept so the maintainer can inspect it, and goes away when the lifecycle issue is closed.
- **Scratch repository cannot be created** (credentials, rate limit, name collision): the AI-driven stage did not complete, so it is a verification failure on the same single path, narrated as an infrastructure problem rather than a broken candidate.
- **Run dies after creating the scratch repository**: the repository is still reaped no later than the closure of the lifecycle issue, and a scratch repository whose lifecycle issue is already closed or missing is deleted rather than left orphaned.
- **Lifecycle issue is closed while the run is still going**: the scratch repository is deleted all the same — closure is the deletion trigger regardless of run state.
...
- **FR-019**: The AI-driven stage MUST run against a scratch repository created for that run and separate from this repository, giving the stage a clean slate isolated from this repository's real `specs/` tree and from any pushed branch or opened pull request. The scratch repository MUST NOT be deleted when the run finishes — it survives every outcome (pass, fail, or error) so a maintainer can inspect it or interact with it while the lifecycle issue is open — and it MUST be deleted when that lifecycle issue is closed.
- **FR-020**: Because an AI-driven stage is non-deterministic, the executable scenario harness required by FR-015 MUST assert the tier's pass and failure paths against controlled candidate behaviour (stubbed or recorded stage results) rather than against live agent runs, so that harness outcomes are deterministic and a harness failure always indicates a real regression. The harness MUST NOT create or delete real scratch repositories.
- **FR-021**: An AI-driven stage that does not complete — error, timeout, unavailable model/credentials, or a scratch repository that could not be created — MUST produce the same single verification-failure outcome as any other deeper-tier failure, and the narration MUST state that the stage did not complete and distinguish that from a candidate-artifact failure, so the maintainer can tell an infrastructure problem from a broken candidate.
- **FR-022**: The lifecycle issue MUST name the run's scratch repository and how to reach it, so a maintainer reading only the issue can inspect the stage's target, and MUST state that the repository will be deleted when the issue is closed.
- **FR-023**: No scratch repository may outlive its lifecycle issue: closing the lifecycle issue MUST result in the corresponding scratch repository being deleted, including for runs that ended abnormally and left a repository behind and for runs whose lifecycle issue was closed without the run completing. A scratch repository whose lifecycle issue is already closed or no longer exists MUST be deleted rather than left orphaned.
...
- **AI-driven stage run**: a throwaway spec-kit-driven stage executed against the candidate inside the run's scratch repository; its completion and documented output are gating assertions of the end-to-end tier.
- **Scratch repository**: a repository created per run, separate from this one, that the AI-driven stage runs against; named on the lifecycle issue, retained while that issue is open, deleted when it closes.
...
- **SC-010**: The executable scenario harness produces identical verdicts across repeated runs (no flaky outcome from the AI-driven stage), because the stage result is controlled in the harness, and no harness run creates or deletes a real scratch repository.
- **SC-011**: Every scratch repository the tier creates is still reachable while its lifecycle issue is open and is gone once that issue is closed: at any point in time, the count of scratch repositories whose lifecycle issue is closed or absent is zero.
- **SC-012**: A maintainer reading only the lifecycle issue can reach the run's scratch repository and knows it disappears when they close the issue.
...
- Where the *AI-driven* stage runs is **answered — a per-run scratch repository** (option C on the lifecycle issue), with the maintainer's amendment that it is retained until the lifecycle issue is closed instead of being deleted at the end of the run. No permanent `wing-commander-end-to-end-test` repository is created. The retention is deliberate: it lets a maintainer of this repository interact with the stage's target while triaging or demonstrating a run, knowing the repository is guaranteed to be deleted afterwards.
- The scheduled job therefore needs repository create and delete rights — a broader grant than the alternatives — and something must delete a scratch repository when its lifecycle issue closes. That reaping obligation is stated as behaviour (FR-023); how it is triggered is left to planning.
