...
5. **Given** a run in which the AI-driven stage ran against the scratch repository, **When** the lifecycle issue is narrated, **Then** it names that repository and the branch holding this run's evidence, and says the branch is left in place and force-reset by the next run for this issue.
6. **Given** a second run for the same lifecycle issue, **When** the stage runs, **Then** it starts from an empty tree — nothing the previous run left on that branch can satisfy the tier's assertions.
...
**Independent Test**: Run the existing scenario harness for this workflow and confirm it exercises, and can fail on, each of: a healthy candidate passing, a missing expected artifact failing, a wrong-shape result failing, an AI-driven stage that does not complete failing, the narration carrying the non-clean-bump hint, and the scratch repository being resolved (configured, unconfigured, and not visible to the token) without ever being created or deleted. Run it twice and confirm the verdicts are identical — the harness drives controlled stage results, not live agent runs.

**Acceptance Scenarios**:

1. **Given** the scenario harness for this workflow, **When** it runs against the fixed tier, **Then** it asserts a pass for a healthy candidate and a failure for each defective-candidate case above, plus the scratch repository's resolve-never-create-never-delete behaviour.
...
- **Deeper tier fails after creating scratch files**: the disposable worktree is still discarded, and nothing reaches this repository's real `specs/` tree or any pushed branch. The run's branch in the scratch repository is deliberately kept so the maintainer can inspect it.
- **Scratch repository is not configured, or is not visible to the pipeline's token**: the AI-driven stage did not complete, so it is a verification failure on the same single path, narrated as an infrastructure problem rather than a broken candidate, naming the variable to set or the App installation to add.
- **Run dies part-way through the stage**: the branch is left exactly as that run left it, and the next run for the same lifecycle issue force-resets it to an empty tree before scaffolding — a half-written branch can never be mistaken for a passing one.
- **Lifecycle issue is closed while the run is still going**: nothing to reap; closure has no repository-lifecycle consequence at all.
...
- **FR-013**: Every artifact the tier creates MUST remain inside either the disposable isolated checkout, which is discarded on every outcome (pass, fail, or error), or the run's own branch of the scratch repository per FR-019; no scratch artifact may land in this repository's real `specs/` tree, in any pushed branch of this repository, or in any opened pull request.
...
- **FR-015**: The pass and failure paths of the tier MUST be asserted by the repository's executable scenario harness for this workflow, covering at minimum: a healthy candidate passing, a missing expected artifact failing, a wrong-shape or non-zero-exit result failing, an AI-driven stage that does not complete failing, the narration carrying the non-clean-bump hint, and the scratch repository being resolved rather than provisioned — including the unconfigured and not-visible-to-the-token failures, and an assertion that no repository-creating or repository-deleting call is made at all (FR-020).
...
- **FR-019**: The AI-driven stage MUST run against a repository separate from this one, on a branch of its own derived from the lifecycle issue, giving the stage a clean slate isolated from this repository's real `specs/` tree and from any pushed branch or opened pull request. That branch MUST be reset to an empty tree before the candidate is scaffolded onto it, so no artifact of a previous run can satisfy any assertion of this one. The stage MUST NOT create or delete repositories: the scratch repository is created once by a maintainer and named in configuration. It MUST survive every outcome (pass, fail, or error), so a maintainer can inspect the branch afterwards.
- **FR-020**: Because an AI-driven stage is non-deterministic, the executable scenario harness required by FR-015 MUST assert the tier's pass and failure paths against controlled candidate behaviour (stubbed or recorded stage results) rather than against live agent runs, so that harness outcomes are deterministic and a harness failure always indicates a real regression. The harness MUST NOT touch a real repository.
- **FR-021**: An AI-driven stage that does not complete — error, timeout, unavailable model/credentials, or a scratch repository that is unconfigured or not reachable with the pipeline's own token — MUST produce the same single verification-failure outcome as any other deeper-tier failure, and the narration MUST state that the stage did not complete and distinguish that from a candidate-artifact failure, so the maintainer can tell an infrastructure problem from a broken candidate.
- **FR-022**: The lifecycle issue MUST name the scratch repository and the branch carrying this run's evidence, so a maintainer reading only the issue can reach the stage's target, and MUST state that the branch is left in place and force-reset by the next run for that issue.
- **FR-023**: This feature MUST NOT hold any repository-administration right. It creates no repository and deletes none — neither the scratch repository nor any other — so the credential it runs under never carries a grant that would also let it delete the repository it maintains. Cleanup of the scratch repository is a maintainer action, not a pipeline one.
...
- **AI-driven stage run**: a throwaway spec-kit-driven stage executed against the candidate on the run's branch of the scratch repository; its completion and documented output are gating assertions of the end-to-end tier.
- **Scratch repository**: one pre-created repository, separate from this one, named in configuration, that the AI-driven stage runs against on a per-lifecycle-issue branch it force-resets each run; named on the lifecycle issue, never created or deleted by the pipeline.
...
- **SC-005**: Across pass, fail, and error outcomes, zero scratch artifacts from the deeper tier appear in this repository's real `specs/` tree, in any pushed branch of this repository, or in any opened pull request.
...
- **SC-010**: The executable scenario harness produces identical verdicts across repeated runs (no flaky outcome from the AI-driven stage), because the stage result is controlled in the harness, and no harness run touches a real repository.
- **SC-011**: The count of repositories this feature creates or deletes is zero, in every run and every outcome — and the count of repository-administration permissions its credential holds is likewise zero.
- **SC-012**: A maintainer reading only the lifecycle issue can reach the branch holding that run's evidence, and knows it stays until the next run for that issue overwrites it.
...
- The script-level checks of the deeper tier continue to operate inside the same disposable isolated checkout and scratch feature that the lightweight tier already established, rather than provisioning their own.
- Where the *AI-driven* stage runs was originally **answered — a per-run scratch repository** (option C on the lifecycle issue), created and deleted by the pipeline. That answer was **amended during implementation** to one pre-created scratch repository with a per-run branch, for two reasons found while wiring it up:

  1. It is not implementable as specified on this repository. `charlesguse` is a **user** account, so creating a repository there goes through `POST /user/repos`, which GitHub documents for OAuth and classic-PAT scopes only — a GitHub App installation token, which is what every job here runs under, has no documented way to call it. The first real minor/major candidate would have failed on `gh repo create`.
  2. Deleting a repository needs `Administration: write`, and the App's grant is not per-call: the same token that could delete a scratch repository could delete **this** repository, at every one of the pipeline's agent steps. Gate 12 refuses those call sites for exactly this reason, and widening `docs/setup.md` to satisfy it would have bought a convenience with a standing risk.

  The amendment keeps every property the original answer was chosen for — a clean slate per run, isolation from this repository's real `specs/` tree, and a target the maintainer can inspect afterwards — by making the **branch** the per-run unit (force-reset to an empty tree before each scaffold) rather than the repository. The cost is one manual setup step, recorded as `WING_COMMANDER_AUTO_UPDATE_SPEC_KIT_E2E_SCRATCH_REPO` in `docs/setup.md`, and one behaviour change: unconfigured, minor/major candidates fail verification instead of being adopted unverified.
