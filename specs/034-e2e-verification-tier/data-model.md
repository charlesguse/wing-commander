...
| `scratch_repo` | string | The configured `OWNER/NAME` — carried through so the combine step and the issue narration (FR-022) can name it regardless of pass/fail/error. |
| `scratch_branch` | string | `auto-update-spec-kit/e2e-<issue-number>` — carried alongside `scratch_repo`, because the repository alone no longer identifies this run's evidence. |
...
## Scratch repository (a real GitHub repository, not a file — pre-created by a maintainer, one per consuming repository)

| Field | Type | Notes |
|---|---|---|
| Name | `vars.WING_COMMANDER_AUTO_UPDATE_SPEC_KIT_E2E_SCRATCH_REPO`, an `OWNER/NAME` | Configuration, not convention: this feature has no way to create a repository (see the contract's lifecycle section) so it cannot name one into existence either. |
| Owner | Whatever the maintainer created it under | Never a fixed Wing-Commander-owned account (constitution VI). |
| Visibility | private, recommended | No candidate-verification content is meant for public consumption. |
| Branch (the per-run unit) | `auto-update-spec-kit/e2e-<lifecycle-issue-number>` | Deterministic (research.md) — the branch name *is* the mapping back to its owning lifecycle issue; no separate ledger. Reset to an empty tree and force-pushed on every run, so a previous cycle's leftovers can never satisfy this cycle's assertions. |
...
| Created by | A maintainer, once, by hand (FR-019/FR-023) | The pipeline only *resolves* it: unset, or not visible to the App token, fails the stage with a reason naming the variable to set or the App installation to add. |
| Deleted by | Nobody, ever, in this pipeline (FR-023) | Deleting a repository needs `Administration: write`, a grant that would also let every agent step delete the consuming repository. Cleanup is a maintainer action. |
| Permission needed | `Contents: read and write` on the scratch repository, i.e. the App installed on it | Already in the App's documented grant — this feature widens `docs/setup.md`'s permission list not at all. |
...
| `scratch_repo_pointer` | string | Present whenever `e2e-stage` ran (i.e., whenever the tier is `lightweight+end-to-end`, regardless of pass/fail): names the scratch repository AND the branch holding this run's evidence, and states the branch is left in place and force-reset by the next run for this issue (FR-022, present even on the tier's overall pass — SC-012 doesn't gate this pointer on failure). |
...
(any minor/major verify run, regardless of pass/fail/error)
  └─ e2e-stage resolved the configured scratch repo, force-reset
     auto-update-spec-kit/e2e-<issue> to an empty tree, and scaffolded onto it;
     the branch survives this run's outcome unconditionally

(no repository is ever created or deleted, on any trigger — there is no
 issues-closed job and no sweep; the branch is simply overwritten by the
 next run for the same lifecycle issue)
```
