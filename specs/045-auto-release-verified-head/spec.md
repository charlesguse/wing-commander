...
- Q: How much of the 8-stage lifecycle must the end-to-end run exercise against the test repository for its verdict to count as real verification? → A: The full intake → cleanup lifecycle, every stage including the implement⟲converge loop and its handoff points, but driven against a deliberately trivial feature sized so that loop converges in one iteration — full-stage coverage at close to contract-prefix cost. The accepted gap is that multi-iteration convergence, retry, and escalation paths go unexercised, so a defect that only appears on a second iteration still reaches adopters; FR-008a states that limit rather than leaving it implied. (FR-008, FR-008a)
- Q: How should the schedule's cadence be made configurable, given that a schedule expression cannot be read from a repository variable? → A: It is not made settings-configurable. The cadence is a fixed interval in the workflow file, changed by a pull request when it needs to move — the same shape the watchdog, the auto-updater, and the private-image dogfood workflow already use. This deliberately declines the alternatives (a minimum-interval variable plus a last-attempt timestamp, or a day filter): both add state the no-op path must read correctly, for a knob that moves rarely. (FR-002)
- Q: How is patch vs. minor decided automatically, and what happens when a range of merged work carries conflicting signals? → A: Always patch, unless a merged pull request in the range from the latest release tag to the verified head carries an opt-in "minor" label — in which case any single minor signal in the range wins, so a mixed range resolves to the highest bump requested. A missing label degrades to a patch rather than blocking the release: it understates an additive change to adopters, which the floating major tag makes harmless, where a blocked release would reintroduce the manual step this feature removes. (FR-017, FR-017a, FR-017b)
...
2. **Given** an attempt that found new work, **When** the end-to-end run drives its trivial feature against the test repository, **Then** every published lifecycle stage from intake through cleanup runs and each one's documented output is asserted — no stage is skipped to save budget.
3. **Given** a range of merged work in which no pull request carries the minor label, **When** the version is computed, **Then** it is a patch increment; **Given** a range in which at least one does, **Then** it is a minor increment.
4. **Given** a release has just been cut automatically, **When** the next scheduled check runs with no further merges to main, **Then** no second release is cut for the same head.
5. **Given** the automatic path is cutting a release, **When** the tag, the floating major tag, and the release notes are produced, **Then** they are produced by the existing release automation — this feature supplies inputs to it and creates no tag, floating tag, or release of its own.
6. **Given** a release was cut automatically, **When** a maintainer reads the release, **Then** it is indistinguishable in shape from a hand-dispatched one (same notes structure, same Breaking-changes section stating "None").
...
- **The verification feature needs more than one implement⟲converge iteration**: the driven feature is sized to converge in one, so a run that does not is treated like any other verification outcome — if the lifecycle does not complete within its bounded budget, that is a verification failure and no release is cut.
- **A range mixes labelled and unlabelled merges**: the highest bump requested in the range wins — one minor label anywhere makes the release a minor, and the outcome does not depend on which pull request is read last.
- **A merged pull request carries the minor label but the work is in fact breaking**: it is still released as a non-breaking minor. The label requests a minor bump only; it has no breaking variant, and breaking detection remains out of scope (FR-019).
- **Cadence needs to change**: a maintainer edits the schedule line in the workflow file and opens a pull request — there is no settings-side knob, by choice (FR-002).
...
- **FR-002**: The schedule's cadence MUST be a fixed interval declared in the workflow file itself, following the precedent of this repository's other scheduled workflows (the watchdog, the auto-updater, the private-image dogfood). Changing the cadence is a one-line edit reviewed as a pull request. No repository variable sets, gates, or throttles the schedule, and no last-attempt timestamp is tracked: every tick either finds new work or is a no-op under FR-003.
...
- **FR-008**: The end-to-end run MUST exercise the full intake→cleanup lifecycle — every published stage, including the implement⟲converge loop and its handoff points — against the test repository, and each stage's documented output MUST be asserted.
- **FR-008a**: The feature the end-to-end run drives MUST be deliberately trivial, sized so the implement⟲converge loop converges in one iteration, so that full-lifecycle coverage costs close to a contract-shaped prefix. Exercising multi-iteration convergence, retry, and escalation paths is consequently out of scope for this verification: a defect that only appears on a second iteration is not caught here.
...
- **FR-017**: The next version MUST be computed from the most recent release tag rather than supplied by a human. The increment MUST be a patch by default, and a minor when any merged pull request in the range from the latest release tag to the verified head carries the opt-in minor label. That label is the only authoritative signal — no other classification is read or inferred.
- **FR-017a**: A range carrying conflicting signals MUST resolve to the highest bump requested in it: a single minor label anywhere in the range makes the release a minor, regardless of how many unlabelled merges accompany it. The resolution MUST depend only on the set of pull requests in the range, never on their order or on which is read last, so the same range always yields the same version.
- **FR-017b**: A missing label MUST degrade to a patch release rather than block one. An additive change whose pull request was never labelled is released as a patch — it understates the change to adopters but publishes nothing incorrect, and the floating major tag advances either way.
...
- **End-to-end verdict**: the machine-readable pass/fail result of driving every published stage, intake through cleanup, against the test repository, plus the failing check and its expected-vs-observed detail. The only input the release decision reads.
- **Minor opt-in label**: the label a maintainer puts on a pull request to declare its change additive. Its presence anywhere in the range from the latest release tag to the verified head is the only signal that turns a patch into a minor; it never requests a major.
- **Version decision**: the next version computed from the latest release tag, plus whether it is a patch or a minor increment — patch unless the minor opt-in label appears in the range. Never a major.
...
- **SC-015**: In every passing verification, the number of published lifecycle stages that did not run, and the number of exercised stages whose documented output went unasserted, is zero.
- **SC-016**: The computed increment is a function of the range alone: re-running the version computation over the same range yields the same patch-or-minor result every time, and a range containing at least one minor-labelled pull request yields a minor in 100% of cases.
...
- Detecting whether unreleased work is breaking is out of scope in every form — no heuristic, no label-driven inference, no interface diff. The minor opt-in label of FR-017 requests a minor bump only and has no breaking variant; the manual breaking dispatch remains the only route to a major.
- The cadence lives in the workflow file rather than in a repository variable, because a schedule expression cannot be read from one and the alternatives (a minimum-interval variable plus a last-attempt timestamp, or a day-filter variable) add state the no-op path would have to read correctly. A cadence change being a pull request is accepted as the cost of having no extra knob, and matches every other scheduled workflow here.
- The end-to-end run's feature is a fixture sized for one-iteration convergence, not a realistic feature. Multi-iteration convergence, retry, and escalation paths therefore go unverified before a tag; that gap is accepted in exchange for full-stage coverage at roughly prefix cost, and is the reason FR-008a states it rather than leaving it implied.
- **Accepted-gap statement (updated by `specs/055-unattended-e2e-gates`)**: the end-to-end run's four human gates — the clarification exchange and the spec-draft, plan, and finalize pull-request merges — are driven by a dedicated fixture maintainer identity rather than removed or skipped. No gate is configured away in the fixture repository, so this FR-008's full-lifecycle claim stands unamended; there is no accepted gap from human-gate handling to record. Were a gate ever removed by fixture configuration instead, it would be named here, with what consequently goes unverified before a tag.
...
- The same set of stages — all of them — runs on every attempt. Coverage is not varied per run, per change size, or to save budget on a quiet range.
