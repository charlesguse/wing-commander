...
- [X] T022 [P] In `docs/setup.md`'s repository variable table (§3), add rows for `WING_COMMANDER_AUTO_RELEASE_PAUSED` (unset = not paused; `true` = kill switch, read job-level in `auto-release.yml` per T021 so no job starts at all) and `WING_COMMANDER_AUTO_RELEASE_E2E_REPO` (unset; `OWNER/NAME` of the pre-created, maintainer-owned end-to-end test repository, same convention as `WING_COMMANDER_AUTO_UPDATE_SPEC_KIT_E2E_SCRATCH_REPO`), placed alongside the existing watchdog/auto-updater pause-switch rows and the auto-updater's scratch-repo row (plan.md Project Structure).
- [X] T023 Corrected by specs/049-single-home-release-idioms (FR-018): this task's original text directed adding `auto-release.yml` to `release.yml`'s `shell_exempt` array and implied that registration happened. Confirmed against `origin/main` (2026-09-14) that it did not, and does not need to: `auto-release.yml` triggers on `schedule`/`workflow_dispatch`, never `workflow_call`, so `wc_published_stages.py` never derives it as a published stage — Gate 48's closure check (the successor to the Gate 1a pass-2 check this task originally named) therefore never forces it into either `SHELL_LINTED` or `SHELL_EXEMPT`, and no entry for it exists in `.github/scripts/verify-stage-shell-lint.py`'s `SHELL_EXEMPT` dict (which contains exactly `watchdog.yml`, `auto-update-spec-kit.yml`, `pr-conversation.yml`) or in `release.yml`. This task's job was to record the deliberate choice research.md D15 flagged; recording it correctly — as a finding, not a completed registration — is what closes it. Kept `[X]`.
- [X] T024 Run the `review-step-gating` skill (CLAUDE.md) against the finished `.github/workflows/auto-release.yml`, given the number of gated (`if:`) steps this design implies (pause check, no-op early exit, infra-unreachable early exit, timeout branch, dispatch-failure branch, version-collision branch — research.md D15) — fix any findings before the feature is considered done.
- [X] T025 Run `python .github/scripts/run-local-gates.py` from the repository root and confirm it stays green with `.github/workflows/auto-release.yml` and the T023 `release.yml` change in place.
...
- [X] T027 CRITICAL: guard `dispatch-release` (`.github/workflows/auto-release.yml`) against dispatching a release for a commit newer than the one `verify-e2e` actually verified, per FR-016 (missing). `release.yml`'s own checkout carries no `ref:` override, so `gh workflow run release.yml` tags whatever the default branch's tip is **at dispatch time**, not `needs.detect.outputs.head-sha` captured when `detect` ran — and `verify-e2e` can run for up to this workflow's own `timeout-minutes` (150) before `dispatch-release` starts, during which `main` can advance. Before calling `gh workflow run`, resolve the default branch's current tip (e.g. `gh api repos/$GITHUB_REPOSITORY/commits/<default-branch> --jq .sha`, or an equivalent `git ls-remote`) and compare it to `needs.detect.outputs.head-sha`; if they differ, skip the dispatch entirely and set `release-outcome` to a new, distinct value (e.g. `stale-head`) — this is neither `released` nor `failed`, matching the spec's own edge case ("no release is cut this attempt and the next scheduled check verifies the new head — a tag must never land on a commit no end-to-end run ever saw"). Extend `report`'s job (same file) with a matching branch: on `release-outcome == 'stale-head'`, write a `$GITHUB_STEP_SUMMARY` line naming the verified head and stating that `main` advanced past it before dispatch, and do **not** file or update the `auto-release:failed` issue — this is expected behavior, not a defect. Re-run `python .github/scripts/run-local-gates.py` and the `review-step-gating` skill after this change, since it touches `dispatch-release`'s `if:`/output shape and adds a new `report` branch (CLAUDE.md).
