...
by the maintainer's manual path — the shape of the evidence is the owner's
...
own. After this feature, the automatic path identifies its own run
unambiguously; if it cannot, it says so and reports nothing as released.
...
2. **Given** an automatic attempt whose own release run is found and
   concludes successfully, **When** the report is written, **Then** it
   records the release and links the run that the attempt itself caused.
3. **Given** an automatic attempt whose own release run cannot be identified
   ...
   states that the attempt's own run was not observed, does not report a
   release, and does not close the standing failure issue.
...
1. **Given** a release request that names the verified commit, **When** the
   ...
   created, **Then** the tag is created on that exact commit.
2. **Given** a release request that names the verified commit, **When** the
   ...
   **Then** no tag is created, no release is published, and the refusal names
   both the requested commit and the tip observed.
...
- **A run that matches on every visible attribute but is not ours.** Two
  ...
  the version alone; a stale run from an earlier attempt must never be
  adopted.
- **The requested run never appears.** A dispatch can be accepted and the run
  ...
  attempt must report "not observed" honestly: it must neither claim a
  release nor assert that the release failed.
- **The release actually happened but was never correlated.** The dispatch
  ...
  its run. The report must not claim a release it could not confirm; the next
  scheduled check must reach a correct conclusion anyway (it sees the new
  tag, finds no unreleased work, and stays quiet) rather than attempting a
  duplicate release.
...
- **A maintainer's manual run and an automatic run request the same version.**
  ...
  automatic attempt must not read the other's refusal as its own.
...
- **FR-002**: The correlating evidence MUST be carried through the release
  ...
  manual dispatch in particular — cannot satisfy it. [NEEDS CLARIFICATION:
  which form of correlating evidence does the owner want? (a) the release
  workflow gains a run title that includes the version and an
  attempt-identifying token supplied as an optional input, and the automatic
  path selects on that title; (b) no release-workflow change — the automatic
  path selects on start time strictly after its own request, plus the
  dispatch event type, plus the automation actor; (c) both, with the title
  identifying the attempt and the time bound guarding against an earlier
  attempt's identically-titled run.]
- **FR-003**: Correlation MUST be bounded in time as well as in identity: a
  ...
  even if every other attribute matches.
...
- **FR-005**: When no run satisfies the correlating evidence within the
  ...
  It MUST NOT report a release and MUST NOT close the standing failure
  issue.
...
- **FR-007**: The report MUST record a release as having happened only on
  evidence the attempt can attribute to itself. [NEEDS CLARIFICATION: what
  does the owner want to treat as authoritative proof that this attempt's
  release happened? (a) the correlated run's own conclusion, with the run
  identified per FR-002; (b) the resulting tag state — the exact version tag
  exists and points at the verified commit — with the correlated run used
  only for diagnostics and log links; (c) both, requiring the correlated run
  to have succeeded AND the tag to point at the verified commit before
  anything is reported as released.]
- **FR-008**: A cancelled release run MUST NOT be reported as a successful
  release.
...
- **FR-010**: The guarantee in FR-009 MUST be enforced by the release
  ...
  narrowing the window before the request. [NEEDS CLARIFICATION: how should
  the release automation be taught to enforce it? (a) it accepts an optional
  commit input, checks that commit out, and refuses to tag unless that commit
  is still the default branch's tip at tag time; (b) it accepts an optional
  expected-head input which it asserts against the branch tip at tag time,
  still tagging the tip and refusing on mismatch — no checkout override;
  (c) no release-automation change: the automatic path compares the tagged
  commit against the verified commit after the fact and reports a mismatch,
  accepting that a wrong tag can be published and must be reported rather
  than prevented.]
...
- **FR-015**: The manual release dispatch MUST keep working with exactly the
  inputs it requires today. Any input added by this feature MUST be optional,
  and omitting it MUST produce today's behaviour unchanged.
...
- **FR-018**: Both guarantees MUST be covered by deterministic checks that
  ...
  recency-based selection or removes the tag-time refusal fails rather than
  merges quietly.
...
- **Release request**: The act of asking the release automation to cut a
  ...
  makes it carry, in addition, whatever evidence FR-002 and FR-010 settle on:
  which attempt is asking, and which commit it means.
...
- **Correlated run**: The release run that a given automatic attempt caused,
  established by evidence rather than by recency. Distinct from "the newest
  release run", which is what the attempt reads today.
...
- **Standing failure issue**: The one durable report consecutive failures
  ...
  run is the concrete harm FR-007 prevents.
...
- **SC-005**: A change that reverts either guarantee (selecting the release
  run by recency again, or dropping the tag-time commit refusal) is caught by
  a deterministic check before it can merge.
...
- Immutability of published tags is unchanged: a wrongly-tagged commit cannot
  be un-tagged, which is why FR-009 is framed as prevention. Option (c) of
  FR-010 is offered as an explicit trade-off, not as an equal alternative.
...
- The waiting period for the correlated run to appear is a tuning knob, not a
  ...
  current value is assumed adequate until evidence says otherwise.
