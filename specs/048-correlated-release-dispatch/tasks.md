...
## Maintainer Feedback (code review of PR #342)

- [X] T025 In `.github/workflows/auto-release.yml`'s `dispatch-release` step (~lines 733-782), the correlation loop `break`s as soon as `count -eq 1` and the very next lines do a single `git fetch origin refs/tags/${VERSION}` + tag-state read with no wait for the correlated `release.yml` run to reach a terminal state. A run still in checkout/lint has no tag yet, so `tag_matches=false` and the report job (falling through to the `current_tip == HEAD_SHA` branch) files a false "release dispatch failed" pipeline-defect while the release succeeds moments later. Add a bounded wait after correlation — poll the correlated run's `status`/`conclusion` via `gh run view --json status,conclusion` (or `gh run watch` with a timeout) until terminal, or a fixed bounded interval when `correlation != found` — before the tag fetch. Keep the release verdict decided from tag state alone (FR-007); the wait only decides *when* that state is read. Add/extend Gate 53's self-test to cover a not-yet-tagged read.
      Done: added a bounded poll of `gh run view "$correlated_run_id" --json status --jq .status` (60 attempts, 10s apart) when `correlation = found`, and a fixed 90s wait otherwise, both before the tag fetch — reading only `status`, never `conclusion`, so the verdict still comes from tag state alone. Added Check 5 (`wait_before_tag_read_errors`) plus its self-test mutation to the gate script (now Gate 59, see T030) and its fixtures/docstring.
- [X] T026 In `.github/workflows/auto-release.yml`'s `report` job, the "no new work since ${LATEST_TAG}" branch (the `HAS_NEW_WORK != true` / `TAG_EXISTS == true` case, near line 924-930) exits without calling `close_failure_on_success`, unlike the `TAG_MATCHES == true` path (line ~1039). A run that tagged successfully but whose own report never closed the failure issue leaves a standing `auto-release:failed` issue open forever, since the next scheduled run takes this branch and exits before ever re-checking. Call `close_failure_on_success` (or the same search-based close) on this branch too when the latest tag already points at HEAD.
      Done: the `TAG_EXISTS == true` branch now calls `close_failure_on_success "${LATEST_TAG} already released."`; `close_failure_on_success` took an optional message parameter (defaulting to its prior `${NEXT_VERSION} released.` wording) so this branch — where `NEXT_VERSION` is never set — states what actually resolved it.
- [X] T027 `.github/workflows/release.yml`'s new "Refuse to tag unless the requested commit is still the branch tip" step (~line 242) and `.github/workflows/auto-release.yml`'s tag-mismatch classification (~line 1050) both hard-code `refs/heads/main`, while `release.yml`'s own `Checkout` step (line 68) already resolves the branch dynamically via `github.event.repository.default_branch`. Use the same default-branch value in all three places so the new refusal/classification cannot compare against the wrong ref on a repository whose default branch isn't `main`.
      Done: release.yml's refusal step now reads `DEFAULT_BRANCH: ${{ github.event.repository.default_branch }}` (safe there — this job only runs on `workflow_dispatch`, same as the Checkout step it now matches). auto-release.yml's `report` job runs off a `schedule` trigger, whose event payload carries no `repository` key, so `github.event.repository.default_branch` cannot be reused as-is there; its classification step instead reads the default branch live via `gh repo view "$GITHUB_REPOSITORY" --json defaultBranchRef --jq '.defaultBranchRef.name // empty'` (the same pattern `detect` already uses for the E2E test repository), falling back to `main` only if that read fails.
- [X] T028 In `.github/workflows/auto-release.yml`'s correlation loop (~line 750), `[ "$created_epoch" -gt "$request_time" ]` excludes a `release.yml` run created in the same UTC second as the request, incorrectly downgrading a real match to `not-observed`. Change to `-ge`.
      Done.
- [X] T029 In `.github/workflows/release.yml`'s tag-time refusal step (~lines 236-244), a short or malformed `commit` input fails safe (it will never equal the observed tip) but produces a confusing refusal message. Validate up front that `inputs.commit` is exactly 40 hex characters when non-empty, and if not, refuse with a message stating that plainly instead of the generic tip-mismatch message.
      Done: a `grep -Eq '^[0-9a-f]{40}$'` check runs before the tip comparison and refuses with a dedicated "is not a 40-character hex commit SHA" message.
- [X] T030 This PR's Gate 53 (`verify-correlated-release-dispatch.py`, registered in `.github/workflows/lint-workflows.yml`) collides with gate numbers PR #341 (spec 046) is registering on top of the same `main`. Before this PR merges, re-run T001's "confirm the next available gate number" check against `main`'s current state (and PR #341's pending numbers) and renumber this PR's Gate 53 to whatever is actually free, updating `lint-workflows.yml`, `verify-correlated-release-dispatch.py`'s own references, and `specs/048-correlated-release-dispatch/contracts/regression-gate.md` accordingly.
