...
- **FR-001**: Every agent step that invokes the run-summary action MUST also emit a structured metrics record for that run. The population is defined by call site, not by whether a summary rendered: when this was drafted the action was silently failing to load at every call site (watchdog #261, fixed separately before this specification merged), and a population defined by observed rendering would have been empty.
...
- **FR-004a**: The emitted record MUST be retrievable by the persistence mechanism after the workflow run concludes, and MUST be retained at least as long as the transcript it derives from — a record that expires before collection reads it re-opens the exact bleed this feature closes.
...
- **FR-019a**: Persistence MUST run out of band: it collects a workflow run's records only after that run has concluded, and MUST NOT be a step inside any stage job. It MUST also be manually invocable for an arbitrary concluded run, because its live trigger wiring can only be exercised after it reaches the default branch (the spec-033 lesson) — its first real dispatch must not be its first test.
...
- **FR-040**: The new coverage MUST be wired into the repository's existing gate registry, so coverage that stops being run is itself a failure, and MUST run the same subject with the same arguments locally as it does in continuous integration; its triggers MUST cover every path it reads — including any contract or schema document this feature adds, which the lint workflow's deliberately literal path list does not cover today — and it MUST NOT be suppressible by the failure of an unrelated gate that merely shares its job.
- **FR-040a**: No part of this feature may add an agent invocation to any run: emission, persistence, and the rollup are all deterministic. Every agent step is a standing per-run cost on the maintainer's quota, and nothing in recording spend should itself spend.
...
- **Pipeline run**: one workflow run — the unit persistence collects for and the unit FR-018's idempotency is scoped to. A specification's whole lifecycle is many pipeline runs; where this document means the lifecycle it says so.
...
- **SC-001**: 100% of agent runs at call sites that invoke the run-summary action also produce a structured record — up from none.
