...
1. **Given** any agent step that renders a run summary today, **When** it completes, **Then** a structured metrics record for that run is produced, carrying the schema version, the stage, the specification identity, the model, the turns used and the budget they were measured against, the token counts, the cost, and how the run ended.
2. **Given** the same run, **When** the record and the rendered summary are compared, **Then** every value they share is the same value, derived once rather than twice.
3. **Given** a run whose transcript is missing, empty, or unparseable, **When** the step completes, **Then** a record is still produced, its unavailable fields are explicitly marked unavailable rather than guessed or omitted, and the step does not fail.
4. **Given** a job containing more than one agent step, **When** it completes, **Then** each agent step has its own distinct record, and no record overwrites another.
5. **Given** an adopting repository that has arranged no storage at all, **When** its stages run, **Then** records are still produced, nothing is pushed to any branch of that repository, and no configuration was required to get either behaviour.
6. **Given** the record's field names, **When** the first record is ever written, **Then** it already carries a schema version — the version is not added later to records that lack one.
...
Someone reading a specification's lifecycle issue can see what that specification has cost so far — across intake, clarification, planning, task generation, every implementation cycle, convergence, finalization, and any rework — without opening a single workflow run.
...
**Independent Test**: Take one specification through several stages and confirm that its lifecycle issue shows a cumulative figure covering every agent run that specification has had, that the figure changes as stages complete, and that reading it requires opening no workflow run.

**Acceptance Scenarios**:

1. **Given** a specification that has completed several stages, **When** its lifecycle issue is read, **Then** the cumulative spend across all of that specification's agent runs is visible there.
...
3. **Given** a specification with several implementation cycles and a retry, **When** the rollup is read, **Then** every one of those runs is counted exactly once.
4. **Given** a specification whose runs include one with unavailable metrics, **When** the rollup is read, **Then** the rollup states that it is incomplete and why, rather than silently under-reporting.
5. **Given** a specification with many stages, **When** the rollup is read, **Then** it does not make the lifecycle issue harder to follow than it is today.
6. **Given** the rollup, **When** it is produced, **Then** it is derived from the same records story 1 defines, not from a second, separately-maintained tally.
...
1. **Given** every agent step that uploads an execution transcript, **When** it uploads, **Then** the retention period is declared explicitly rather than inherited.
...
- **The schema needs a new field later.** The store cannot be rewritten in place, so readers will see both old and new records; the version each record carries is how a reader tells them apart, and the compatibility rules for changing the schema must be stated rather than discovered.
...
- **FR-005**: The record MUST carry, at minimum: a schema version; the stage the run belongs to; the specification identity, where one exists; the model the caller declared; the turns used, counted the way the pipeline's turn budget counts them, together with the intended budget and the enforced ceiling they were measured against; token counts including cache reads and cache creation; the cost; the wall-clock duration; how the run ended; and enough identity to tie the record back to the workflow run and job that produced it. [NEEDS CLARIFICATION: is this the committed v1 field set, or should it be narrower or wider? Once a consumer parses these records the names are a permanent compatibility surface in an append-only store — see Question 3.]
...
- **FR-025**: The schema's compatibility rules MUST be stated: what a reader may assume about a record of a given version, what kind of change requires a new version, and how a reader handles a version it does not know. The store cannot be rewritten in place, so both old and new records will be read together.

#### Rolling up per specification

- **FR-026**: A specification's cumulative spend across all of its agent runs MUST be legible from its lifecycle issue without opening any workflow run. [NEEDS CLARIFICATION: which tiers are committed — the rollup, the durable store, or both? See Question 1.]
...
- **FR-031**: The rollup MUST NOT make the lifecycle issue materially harder to read than it is today. [NEEDS CLARIFICATION: rollup form — one rolling summary edited in place, a compact line appended to each stage's status comment, or both? See Question 2.]

#### Stopping the retention bleed for future transcripts

- **FR-032**: Every site that uploads an execution transcript MUST declare its retention period explicitly rather than inheriting a repository default.
...
- **Agent run metrics record**: the structured, versioned account of one agent invocation — stage, specification, model, turns against budget and ceiling, tokens, cost, duration, outcome, and the run and job identity that produced it. The unit of everything else here. Its field names are its contract.
- **Schema version**: the number every record carries from the first write, and the only thing a reader has to distinguish records written under different field sets in a store that cannot be rewritten in place.
...
- **Rollup**: the per-specification cumulative figure presented on the lifecycle issue, derived from the records rather than tallied separately.
...
- **SC-009**: A specification's cumulative spend is readable from its lifecycle issue in one place, with zero workflow runs opened — down from one run opened per stage today.
- **SC-010**: Every transcript upload site declares a retention period — measured over the sites discovered in the tree, not a fixed list — up from zero of the sixteen sites in this checkout.
...
- **SC-014**: 100% of records carry a schema version, including the first one ever written.
