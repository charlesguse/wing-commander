...
- [X] T023 [US3] Define the per-run cost-line Markdown fragment (data-model.md "Rollup — per-run cost line"), e.g. `**Cost**: $0.42 · 38/60 turns · claude-sonnet-5`, built from the same `wing-commander-metrics-summary` outputs each call site already has in hand, degrading to naming which figures are unavailable rather than omitting the line.
- [X] T024 [P] [US3] In `.github/workflows/clarify.yml`, append the T023 cost line to the body of the status comment this stage already posts (no new comment — FR-031c). Depends on T006 (same file, same job). Required moving the metrics-summary/upload/cost-line block earlier in the job (right after the verdict step) — its original placement near the job's end came after the outcome-announcing callouts, which would have read the cost line's output before that step had run.
- [X] T025 [P] [US3] Same change as T024 in `.github/workflows/cleanup.yml`. Depends on T007. The real outcome announcement here is a raw `gh issue close --comment` (not a `wing-commander-callout`), sourced from a summary file — appended there instead.
- [X] T026 [P] [US3] Same change as T024 in `.github/workflows/finalize.yml`. Depends on T008.
- [X] T027 [P] [US3] Same change as T024 in `.github/workflows/implement.yml`, at each of its three status-comment sites. Depends on T009.
- [X] T028 [P] [US3] Same change as T024 in `.github/workflows/intake.yml`. Depends on T010. Same reordering fix as clarify.yml, inserted after "Resolve created spec" (the earliest point spec-dir is known) instead of after the verdict step directly.
- [X] T029 [P] [US3] Same change as T024 in `.github/workflows/plan.yml`. Depends on T011. Same reordering fix, inserted after the later of its two (auto/pr) verdict steps.
- [X] T030 [P] [US3] Same change as T024 in `.github/workflows/pr-conversation.yml`, at both sites. Depends on T012.
- [X] T031 [P] [US3] Same change as T024 in `.github/workflows/rebase.yml`. Depends on T013.
- [X] T032 [P] [US3] Same change as T024 in `.github/workflows/tasks.yml`. Depends on T014. (`watchdog.yml`'s diagnose site is excluded — contracts/emission-contract.md: it posts to a findings issue, not a spec lifecycle issue.)
- [X] T033 [US3] In the T016-T020 composite/T021 workflow, after a successful append, implement the rollup computation (research.md R9): resolve this run's `spec_dir` from the just-appended records' `spec.spec_dir`/`identity_available`, re-read `destination-path` filtered to that `spec_dir`, and recompute cumulative totals (cost/tokens summed by stage, run count) — skip this step entirely when no persisted record from this run carries spec identity. The lifecycle issue number is read directly from the matching records' own `spec.issue` field rather than re-derived via branch-name/spec-meta.json lookup (R9's fallback description) — simpler and already available since T002 only sets `spec.identity_available: true` when both spec_dir and issue are present together.
- [X] T034 [US3] In the same place, implement the machine-owned comment region (research.md R10, data-model.md "Rollup — cumulative summary"): search the lifecycle issue's comments via `gh api repos/{owner}/{repo}/issues/{issue}/comments` (paginated) for the `<!-- wing-commander-metrics-rollup:begin -->` marker; PATCH it in place (`gh api --method PATCH .../issues/comments/{id}`) if found, else create a new comment; regenerate the full region every update — totals table by stage, an "Incomplete: N of M runs..." notice when any contributing record has an unavailable cost/token field (FR-030), and the per-run `<details>` history list.
- [X] T035 [US3] In the same region-builder, implement per-run history de-dup. Deliberately deviates from the literal "parse the previous comment body back out" approach: since `destination-path` is already append-only and already de-duplicated by `record_key` (the append step in T020 never writes the same key twice), regenerating the full history list fresh from `destination-path` on every update — the same "recompute fresh from the store" principle R9 already applies to the totals — can never produce a duplicate line, without a markdown round-trip parse at all. A repeated rollup update for the same agent run still leaves one cost line and one rolling summary (FR-031b), achieved by construction rather than by incremental parsing.
...
- [X] T047 [P] [US5] Create `.github/scripts/verify-actions-layer-invariants.py`: scan every `action.yml` under `.github/actions/**` for `github.event.*`/`vars.*` reads (mirroring `verify-stage-invariants.py`'s existing regex approach, extended to the actions directory it doesn't cover today — issue #149) and for `uses: anthropics/claude-code-action` (FR-040a); support a waiver file for pre-existing, unrelated violations (same shape as `stage-invariant-waivers.json`: exact file/pattern match, exact count, staleness-checked) but require zero violations, waived or not, in this feature's own new/changed files (`wing-commander-metrics-summary/action.yml`'s additions, `wing-commander-metrics-persist/action.yml`). The comment/expression-span helpers are duplicated from `verify-stage-invariants.py` rather than imported — that file's hyphenated name is not an importable Python module, and refactoring Gate 31 to expose a shared module was judged riskier than a bounded, well-understood duplication.
- [X] T048 [P] [US5] Add fixtures for T047 under `.github/scripts/fixtures/actions-layer-invariants/`: an `action.yml` snippet reading `vars.SOMETHING`, and one invoking `claude-code-action`; assert the gate fails and names both.
- [X] T049 [P] [US5] Create `.github/scripts/verify-metrics-record-schema.py`: validate a JSON file against contracts/metrics-record-schema.md's field table (types, presence, the `*_available` convention) and the `per_model` sum invariant, rejecting and naming the failing field(s).
- [X] T050 [P] [US5] Add fixtures for T049 under `.github/scripts/fixtures/metrics-record-schema/`: a well-formed schema-version-1 record (positive case); a record missing a required field, one with a wrong-typed field, and one with a renamed field (negative cases); a multi-model record whose `per_model` sums correctly and one whose sums are wrong (invariant case).
...
- [X] T052 [P] [US5] Add a fixture for T051 under `.github/scripts/fixtures/metrics-schema-version-tolerance/`: a `schema_version: 2` record plus the small fixture store/rollup computation it is run against.
- [X] T053 [P] [US5] Create `.github/scripts/verify-metrics-persist-retry.sh`: drive T020's append-with-retry composite logic against a local bare git repository fixture with two simulated concurrent writers (one push accepted, the second's initial push rejected then retried), asserting both writers' records survive; a second fixture engineered to reject every attempt asserts the step fails loudly, naming every unwritten `record_key`, rather than hanging or succeeding silently. The sustained-contention fixture uses a bare repo's `pre-receive` hook that declines every push, rather than a timing-based background writer — deterministic under CI's variable scheduling, where a race window sized in fractions of a second is not.
- [X] T054 [P] [US5] Add fixtures for T053 under `.github/scripts/fixtures/metrics-persist-retry/`: the local bare-repo setup for the eventually-successful race and for the sustained-contention exhaustion case. A live git repository does not serialize as a checked-in file the way a JSON record does, so this directory holds a README documenting what each fixture builds at self-test time rather than static fixture files (see the README for detail).
...
- [X] T056 [P] [US5] Add a fixture for T055 under `.github/scripts/fixtures/transcript-retention/`: a workflow snippet with a transcript-pattern `upload-artifact` step and no `retention-days`, asserting the gate fails and names the file/step.
- [X] T057 [US5] Wire all five gates (T047, T049, T051, T053, T055) into `.github/workflows/lint-workflows.yml`: one `run:` line each inside a PR-triggered job, gated `if: !cancelled()` (not bare `always()`, matching this repository's step-gating convention) and not conditional on any other gate's outcome, so `verify-gate-wiring.py` and `run-local-gates.py` pick each up automatically with no separate registration. Depends on T047-T056. Wiring this in surfaced three real regressions from earlier phases, fixed alongside it: (1) `metrics-persist.yml` was missing the runner/container-image/environment passthrough and `verify-image-prerequisites` job every other published stage carries (Gates 7/22/23); (2) the T024/T028 cost-line append had been folded into `clarify.yml`/`intake.yml`'s "Render clarification questionnaire" step, which `verify-clarification-gating.py` extracts and re-executes byte-for-byte — moved to a new step right after it instead; (3) `wing-commander-metrics-summary/action.yml`'s `is_number()` helper was a one-line function definition, which Gate 23's function-detection regex (anchored on a bare trailing `{`) didn't recognize as local, so its calls read as an undeclared required tool — rewritten as a multi-line definition. `python3 .github/scripts/run-local-gates.py` is 53/53 green after these fixes.
- [X] T058 [US5] In the same file's `pull_request.paths` list, add `specs/043-durable-metrics-record/contracts/metrics-record-schema.md` (alongside the existing per-spec contract entries) so an edit to the schema document alone triggers T049's gate (FR-040's explicit callout that the literal path list doesn't cover contract/schema documents by default). Depends on T057 (same file).
...
- [ ] T059 [P] Run quickstart.md's five story validations end-to-end against a disposable test repository (or this repository with cleanup per its "Cleanup" section) — confirm no leftover `metrics` branch or rollup comments remain from the exercise. **Not completed by this automated run**: this needs a live GitHub Actions dispatch (triggering real stage workflows, inspecting real run artifacts and issue comments) that this headless implementation session has no tool access to perform — its permitted commands are local git/lint/gate-runner only, no `gh workflow run`/`gh run watch` or equivalent. `python3 .github/scripts/run-local-gates.py` (53/53 green, see T057) is the closest automated proxy available here; a human or a follow-up run with dispatch access should still drive quickstart.md's five story validations against a real or disposable repository before calling this feature done.
- [X] T060 [P] Confirm FR-036 non-regression across every changed published action/workflow (`wing-commander-metrics-summary`, and every file touched in T006-T046): no existing input, output, or secret was removed or renamed; a caller passing none of the new optional inputs still gets a valid record degraded only in those fields, and the rendered `$GITHUB_STEP_SUMMARY` table is byte-for-byte unchanged (FR-011). Verified statically: `git diff ae1bb65 HEAD -- .github/workflows .github/actions` contains zero removed/modified lines in `wing-commander-metrics-summary/action.yml`'s pre-existing content (purely additive diff) and zero removed `with:`/input/output/secret lines across every touched workflow file — the only "removed" lines in the full diff are byte-identical relocations (the metrics-summary/upload/cost-line step block moved earlier in clarify.yml/intake.yml/plan.yml to fix the ordering defect T024/T028/T029 note) and message-text-preserving `gh issue comment --body` refactors (literal-newline strings rewritten via `printf` to fix the YAML block-scalar syntax errors T027/T029 note), neither of which touches the composite's public input/output/secret contract or the rendered summary table's content.
...
- [X] T061 Add executable coverage that runs `wing-commander-metrics-summary`'s actual "Render agent run metrics summary" script (not a reimplementation) against a healthy fixture transcript, a missing/empty/unparseable one, and a job invoking it more than once (distinct `step_index`, distinct `record_key`, neither record overwriting the other) — asserting the produced record in each case, mirroring the `auto-update-spec-kit-tests/` harness's convention of extracting and executing the shipped `run:` block rather than modelling it. per FR-039, FR-009, FR-010 — `.github/scripts/verify-metrics-summary-record-emission.py`, wired into `lint-workflows.yml`; extracts the real step via `wc_shell_harness.find_step` (the same convention Gate 11/`verify-metrics-turn-accounting.py` uses), validates every produced record (healthy and degraded) against the real `verify-metrics-record-schema.py` gate rather than re-deriving the schema, and includes a mutation check (dropping `step_index` from `RECORD_KEY`) proving the suite can fail for the reason it exists.
- [X] T062 Add a fixture/assertion to `verify-metrics-persist-retry.sh` (or a sibling script) for the zero-artifact discovery path: the persistence composite's discovery step finds no `metrics-record*` artifacts for a run and the result is zero entries appended, zero failure — not merely assumed from FR-021's prose. per FR-021, FR-039 — added `append_batch_with_retry()` (a batch-shaped sibling of the existing single-record functions, added alongside rather than replacing them so the two already-passing scenarios stay untouched) and `test_zero_artifact_discovery_is_zero_failure()`, run from the same `--self-test` entry point `lint-workflows.yml` already wires.
- [X] T063 Add an explicit assertion in `verify-metrics-persist-retry.sh` for first-write destination creation: starting from a destination branch that does not exist, the first append creates it (research.md R8's orphan-commit shape) and the write succeeds — today this only exists as unasserted setup plumbing (`seed_branch()`) shared by the other scenarios. per FR-020, FR-039 — added `first_write_if_missing()` and `test_first_write_creates_missing_destination_branch()`, starting from a bare repo with no branch at all (not `seed_branch()`), asserting the branch is absent beforehand, created by the first write, and carries the first record afterward.
- [X] T064 Add an idempotent-repeat-persistence fixture: run the append-with-retry algorithm twice for the same already-persisted `record_key` and assert the destination store is byte-for-byte unchanged after the second run (zero duplicates), not just that a fresh key round-trips once. per FR-018, SC-005, FR-039 — added `test_idempotent_repeat_persistence_is_byte_for_byte_unchanged()`, comparing a `sha1sum` of `records.jsonl` taken after the first (real) append against one taken after the second (no-op) append.
- [X] T065 Add a fixture driving the rollup region-builder logic (or an equivalent harness over its region-regeneration rule) twice for the same persisted record set and assert the resulting comment region is unchanged the second time — one cost line per run, one rolling summary, no duplication — rather than relying on "regenerated fresh from a de-duplicated store" as an argued-not-tested guarantee. per FR-031b, FR-039 — `.github/scripts/verify-metrics-rollup-idempotent.py`, wired into `lint-workflows.yml`; extracts the real "Update cumulative rollup" step, runs it twice against a stubbed `gh` (tracking POST-then-PATCH) and a stubbed `date` (so the region's "as of" timestamp cannot make two identical runs look different), and asserts the two regions are byte-for-byte identical.
- [X] T066 Add a check confirming the persistence workflow/composite has no write path back to the origin pipeline run's status, checks, or comments — e.g. static assertion that `metrics-persist.yml`/`wing-commander-metrics-persist/action.yml` never reference the origin run's job/step outcome or post to anything but the destination branch and the spec's own lifecycle issue — so "a persistence failure never disturbs the run it reports on" is enforced, not merely designed that way (FR-019/FR-019a already achieve this by construction — out-of-band `workflow_run` trigger, separate workflow — but nothing fails if a future change reintroduces a back-reference). per FR-019, FR-039 — `.github/scripts/verify-metrics-persist-no-writeback.py`, wired into `lint-workflows.yml` with a `--self-test`; scans the persistence layer's three files (composite, `metrics-persist.yml`, wrapper) for check-runs/statuses/deployments writes, `gh pr`/`gh issue` shorthand mutations, and any mutating `gh api --method` call whose target is not the lifecycle issue's comments endpoint, with fixtures proving each forbidden shape is caught.

## Maintainer Feedback

**Global constraints (apply to every item below):** assign the eight new gates numbers 38-45 in their step names (37 is taken; do not renumber any existing gate). Do not rename any published `workflow_call` or composite input (constitution VII). The emission layer and its existing gates are verified good — do not rework them beyond what an item below asks.

### Blockers (merge-blocking, each reproduced by execution)

- [X] **MF-B1** Fix `wing-commander-metrics-persist.yml`'s `workflow_run` trigger: it currently lists the reusable (`workflow_call`-only) stage names, which never produce a `workflow_run` event of their own, so automatic persistence never fires. Switch to the wrapper display names (`Wing Commander · 1 intake` … `Wing Commander · 9 pr conversation`, `Wing Commander · rebase`, `Wing Commander · 8 watchdog`), matching `wing-commander-8-watchdog.yml`'s existing convention. Extend Gate 2 so it fails when a `workflow_run` name resolves to a `workflow_call`-only file, not just to some workflow's `name:`. (FR-019a)
- [X] **MF-B2** Add `metrics-persist.yml` to `release.yml` Gate 1a's `shell_linted` list — `wc_published_stages.py` now derives it as a published stage, and it currently appears in neither `shell_linted` nor `shell_exempt`, reproducing `GATE1a-FAIL: .github/workflows/metrics-persist.yml in neither list` (the same incident class documented at release.yml:137-143).
- [X] **MF-B3** Fix silent record loss on a failed first write in `wing-commander-metrics-persist/action.yml`: the branch-creating push is swallowed with `|| true` while `$DEST_PATH` is already written into the working tree, so attempt 1 of the append loop reads its own unpushed file as `existing_keys`, computes an empty `to_append`, and reports success with zero push. On a failed create-push, reset the destination file (or flag the state) so the retry loop starts from genuinely-absent records, and treat "branch still absent after the create step" as a hard failure naming the keys. Also fix the related short-circuit where a **successful** first write reports `persisted-count=0` for a run that persisted N records. (FR-017, FR-019, edge case: destination unwritable → fail loudly)
- [X] **MF-B4** Fix colliding `record_key`s from matrix stages: `RECORD_KEY="${RUN_ID}:${JOB_KEY}:${STEP_INDEX}"` uses `github.job`, identical across every leg of `rebase.yml`'s matrix and `pr-conversation.yml`'s `act` matrix, both defaulting `step-index` to `'0'` — same-batch legs land with duplicate keys and a later-collected leg gets deduped away with `persisted-count=0` and nothing reported. The jobs-API `job_id` fallback is dead code (exact-matches `.name` against API display names, never `github.job`). Make the key unique per agent invocation — include `strategy.job-index`/`matrix.slug` in `step-index` at the two matrix call sites, or fold the artifact name into the key. (FR-010, FR-018, FR-028, FR-030)

### Follow-ups (not merge-blocking)

- [X] **MF-F1** Rebuild `verify-metrics-persist-retry.sh` on a `wc_shell_harness` extraction of the shipped `run:` blocks (matching `verify-metrics-summary-record-emission.py` / `verify-metrics-rollup-idempotent.py`'s existing pattern in this PR) instead of its own standalone re-implementation, and add a fixture where the FIRST push fails (a rejecting pre-receive hook) so MF-B3's defect class stays caught. This is why MF-B3 shipped past 16 green checks.
- [X] **MF-F2** Either wire up or delete the three unread fixture directories (`fixtures/{transcript-retention,actions-layer-invariants,metrics-schema-version-tolerance}/`) and `fixtures/metrics-persist-retry/README.md` — deleting each currently leaves its gate green, meaning it drifts unread from the inline copy it duplicates.
- [X] **MF-F3** Make `verify-metrics-record-schema.py` parse `contracts/metrics-record-schema.md` rather than hardcoding the schema (register the contract doc as the sixth subject document in `verify-gate-wiring.py`'s terms), or fold the doc's table into the gate as the single source — otherwise the doc's lint-workflows path trigger buys a trigger and no check.
- [X] **MF-F4** Fix the rollup comment update to splice only the machine-owned region rather than PATCHing the whole comment body (FR-031a's established pattern — preamble preserved, region regenerated, per finalize.yml's own splice), and anchor `existing_id` to the bot author, not just the marker string.
- [X] **MF-F5** Report a total Actions-API failure as not-collected (FR-019) rather than a clean zero-record run: `gh api ... 2>/dev/null || true` (used twice) currently makes a bad token or API outage indistinguishable from FR-021's legitimate zero-artifact case.
- [X] **MF-F6** Change the retention gate to discover upload sites by artifact `path` pattern (per the contract) instead of artifact `name:` prefix, so a site uploading a transcript under a different artifact name doesn't evade it.
- [X] **MF-F7** Number the eight new gates 38-45 in their step names, matching every other gate in the file and the contract's own "assigned sequentially at implementation time."
- [X] **MF-F8** In `rebase.yml` and `cleanup.yml`, pass `steps.cost-line.outputs.line` through `env:` instead of splicing it raw into `run:` bodies, matching every other site in this PR.
