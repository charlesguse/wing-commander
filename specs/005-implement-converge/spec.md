...
- A stalled specification is restarted by hand: a restart is only a restart if the iteration the stall report names is one the stage will actually admit under the duplicate-hand-off rule. A stall arrives in either of two states — a pass that recorded its progress before failing leaves the lifecycle record at the failed iteration; a pass whose progress never reached the record leaves it at the previous one — so restart advice derived from the failed iteration alone is admitted in one state and silently dropped as a duplicate in the other.
...
- **FR-016**: When a specification is marked stalled under FR-013, the failure surfaced on the lifecycle issue MUST name the iteration at which a manual restart is to be dispatched, and that iteration MUST be one the duplicate-hand-off rule of FR-011 admits — in both stall states, whether or not the lifecycle record advanced to the failed iteration before the pass failed. Restart advice that FR-011 treats as a duplicate does not satisfy FR-013's requirement that a stalled specification await a manual restart, because no restart of it is possible.
...
- **SC-007**: A maintainer who follows a stall report's restart instruction verbatim starts a real build-and-reassess cycle in 100% of stalls, never a run that no-ops under SC-006 — demonstrated by an executable check that constructs both stall states (lifecycle record advanced to the failed iteration, and record left at the previous iteration) rather than by inspection of a single state.
