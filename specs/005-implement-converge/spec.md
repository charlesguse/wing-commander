...
- An implementation or convergence pass cannot complete because of a resource or tooling failure (as distinct from completing but not yet converging): the stage must surface the failure rather than silently advancing or silently dropping the specification.
...
- **FR-007**: The system MUST guarantee that every specification reaching this stage is handed off to finalization exactly once — either converged or flagged as not converged — and never left cycling indefinitely.
...
- **FR-013**: When an implementation or convergence pass cannot complete because of a resource or tooling failure (as distinct from completing but not yet converging), the system MUST [NEEDS CLARIFICATION: failure behavior not specified — mark the specification stalled and require a manual restart, hand off to finalization flagged as not converged with the failure reported, or automatically retry the same iteration once before stalling?].
...
- **Implementation model (configuration)**: The model capability used to implement the task list — a default with an opt-in to a higher-capability model via a designated label on the lifecycle issue.
