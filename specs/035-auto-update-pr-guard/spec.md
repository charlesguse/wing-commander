...
4. **Given** no open pull request from this feature for the settled candidate, **When** the run reaches the same point, **Then** it proceeds through the full adoption chain exactly as it does today.
...
4. **Given** consecutive guarded runs for the same candidate and the same pull request, **When** the tracking issue is read after several days, **Then** the record does not accumulate one identical entry per run (cadence per FR-007).

---

### User Story 3 - Work resumes on its own once the PR is resolved (Priority: P2)

The maintainer merges (or closes) the open version-bump PR. On its next scheduled run, the pipeline resumes normal behaviour with no manual cleanup: if the PR merged, the pin has moved and there is nothing to propose; if it was closed unmerged, the candidate is still eligible and the pipeline proceeds through the full chain again.
...
2. **Given** the open version-bump PR is closed without merging, **When** the next scheduled run happens, **Then** the candidate is still eligible and the run proceeds through the full adoption chain.
3. **Given** either resolution, **When** the next run happens, **Then** no manual state clearing, label removal, branch deletion, or issue edit was required beforehand.

---

### User Story 4 - A leftover branch with nobody reviewing it does not wedge the pipeline (Priority: P3)

A branch from a previous run survives with no open pull request behind it — because the run failed after pushing, or because the PR was closed. Nothing is under review, so the pipeline is free to proceed: it replaces that branch's contents with the freshly prepared commit and opens the PR, rather than dying at a non-fast-forward push and leaving a maintainer to delete the branch by hand before re-dispatching.

**Why this priority**: This is a convenience on top of the guard, not the fix. It removes the manual "delete the branch before re-dispatching" step that closing an earlier issue required, but it saves no quota and, unlike US1, does not fire on the daily schedule. It is also the part of the feature that most changes what the pipeline is permitted to overwrite, so it earns its own decision.

**Independent Test**: Leave a branch named for the candidate on the consumer repository with no open PR pointing at it, run the chain through to the PR-opening step, and confirm it completes and opens a pull request. Separately confirm that a branch *with* an open PR is never reached by this path, because US1's guard stopped the run long before.

**Acceptance Scenarios**:

1. **Given** a branch for the candidate exists on the consumer repository and no open pull request from this feature references it, **When** the run reaches the PR-opening step, **Then** the step completes and a pull request is opened. [NEEDS CLARIFICATION: is this user story in scope for this feature, or does the feature ship the guard alone and leave a leftover-branch collision as a hard failure for a human to clear? The issue calls the guard "the part that saves the quota" and forcing "only a convenience on top of it", and explicitly filed the behaviour change for separate review.]
...
- **An open version-bump PR proposes an *older* candidate than the one that has now settled** (upstream released again while the first PR waited): see FR-011.
...
- **The candidate settled but no PR was ever opened because a prior run failed mid-chain**: no open PR, so the run proceeds — this is exactly the retry the schedule exists to provide.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Before any Claude-billed step of the auto-update chain begins, the run MUST determine whether this feature already has an open pull request proposing the current candidate version.
...
- **FR-003**: The check MUST associate a matching open pull request with the specific candidate version it proposes, so that an open PR for one version does not mask a decision about a different version.
- **FR-004**: When a matching open pull request exists, the run MUST skip the judgment, preparation, end-to-end, verification, and PR-opening steps entirely.
...
- **FR-007**: A skipped run MUST record the same fact on the tracking issue, with a link to the open pull request, without accumulating one identical entry per run. [NEEDS CLARIFICATION: recording cadence — should the tracking issue be refreshed on every guarded run (same shape as the settle counter's body edit, idempotent but with no "still true today" trace), or written only the first time a given pull request is observed (quietest, but a fortnight-old note gives no evidence the schedule is still alive)?]
...
- **FR-009**: When the open pull request is subsequently merged or closed, the next scheduled run MUST resume correct behaviour with no manual state clearing — no latch to reset, no branch to delete, no label to remove.
...
- **FR-011**: When an open version-bump pull request from this feature exists for a candidate *other than* the one that has just settled, the run MUST [NEEDS CLARIFICATION: skip as well — keeping at most one version-bump proposal in flight at a time and noting on the tracking issue that a newer candidate is queued behind PR #N — or proceed, opening a second bump PR for the newer version while the older one is still open?]
...
- **FR-015**: When the PR-opening step declines to write because of state it will not overwrite, its message MUST name the blocking branch or pull request and state the remedy, rather than surfacing only a transport-level rejection.
- **FR-016**: The repository's executable scenario harness for this workflow MUST cover: the skip routing decision, the proceed routing decision, the PR-opening step against a pre-existing target branch, and the PR-opening step against a pre-existing pull request. Each MUST be able to fail.
...
- **SC-004**: Resuming after the open pull request is merged or closed requires zero manual steps.
- **SC-005**: Removing the guard causes at least one scenario-harness check to fail.
- **SC-006**: No scheduled run writes to the branch of a pull request that is open and awaiting review.
...
- A maintainer keeping a pull request open indefinitely is a legitimate state, not an error; the pipeline waits rather than escalating.
