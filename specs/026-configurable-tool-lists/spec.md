...
- What happens when a consumer provides both an append value and a replacement value for the same list on the same stage? (See FR-010 / [NEEDS CLARIFICATION].)
- What happens when a consumer's appended allowed tool is a tool the pipeline disallows by default? (See FR-011 / [NEEDS CLARIFICATION].)
- What happens when a consumer fully replaces the allowed list and omits a tool the stage itself needs to complete its own bookkeeping (e.g. committing and pushing its results)? (See FR-012 / [NEEDS CLARIFICATION].)
...
- **FR-010**: The system MUST define a single, documented outcome when a consumer supplies both an append value and a replacement value for the same list on the same stage. [NEEDS CLARIFICATION: when both append and replace are supplied for the same list, which takes effect — replacement wins and append is ignored, the two are combined (replacement then appended to), or the combination is rejected as a configuration error?]
- **FR-011**: The system MUST define whether an appended allowed tool can re-enable a tool that the pipeline denies by default. [NEEDS CLARIFICATION: may an appended allowed tool override a default disallowed entry (allow wins), or does the default denial always take precedence for safety, so re-enabling a protected default is not possible via append?]
- **FR-012**: The system MUST define whether a minimal core set of tools the pipeline needs to perform its own lifecycle bookkeeping (e.g. committing and pushing stage results, updating lifecycle metadata) is retained even when a consumer fully replaces the allowed list. [NEEDS CLARIFICATION: on full replacement of the allowed list, does the pipeline still guarantee its own required core tools so the stage can complete, or does "replace" mean literally the consumer's list only, making the consumer responsible for including everything the stage needs to function?]
