...
### User Story 5 - Repeated truncation is visible and does not silently consume the whole budget (Priority: P2)

A feature whose cycles keep running out of turns does not quietly burn its iteration budget one ordinary cycle at a time. Repeated truncation is visible on the lifecycle issue, and the pipeline responds to it rather than repeating the same cycle at the same tier until the cap.

**Why this priority**: Carrying truncated cycles forward removes an escalation that today happens automatically. Some truncations mean "this work is large" — carrying forward is right. Some mean "this work is too hard for this tier" — carrying forward at the same tier just re-runs the wall. Nothing else in the pipeline distinguishes them, so repeated truncation is the only available signal, and a change that consumes the budget without ever reacting to that signal trades a known cost for an invisible one.

**Independent Test**: Drive a sequence of cycles that each truncate with progress, and confirm the pipeline's response changes on repeated truncation rather than being identical every time; separately, confirm each truncation is reported to the lifecycle issue as a truncation.

**Acceptance Scenarios**:

1. **Given** a cycle that truncates with progress immediately after another cycle that truncated with progress, **When** the stage decides what happens next, **Then** the pipeline escalates rather than starting another cycle at the same tier.
2. **Given** a cycle that truncates with progress after a cycle that completed normally, **When** the stage decides what happens next, **Then** it is treated as a first truncation and carried forward at the same tier.
3. **Given** any truncated cycle, **When** the lifecycle issue is read, **Then** that cycle is identifiable as having run out of turns, so a reader can see repeated truncation without opening a run log.
...
- **A truncated cycle whose only commit is its own lifecycle bookkeeping.** The lifecycle record advancing is a *precondition* of the carry-forward, so it cannot also serve as the evidence of progress; a cycle that advanced nothing else made no progress.
...
- **Two consecutive truncations where the first was at the top tier.** There is no tier above it to escalate to; repeated truncation must still be visible and must not loop silently.
...
- **FR-004**: The stage MUST require positive evidence that a truncated cycle moved the feature's work forward before carrying it forward. Advancing the lifecycle record alone MUST NOT count as progress, since FR-002 already requires it. [NEEDS CLARIFICATION: what counts as progress — any commit on the branch beyond the lifecycle-record update, at least one task newly marked complete in the task list, or any change to files outside the spec directory? The source request proposes "the branch tip moved", but the lifecycle-record update FR-002 requires is itself a commit, which would make that test always true and the guard inert.]
...
- **FR-007**: When a cycle is classified truncated and the iteration is below the cap, the stage MUST start the next iteration, at the same tier the truncated cycle ran on, except where FR-011 requires otherwise.
...
- **FR-010**: A truncated cycle MUST consume one iteration of the configured iteration budget in the same way a completed-but-unconverged cycle does. [NEEDS CLARIFICATION: the source request flags this as deserving a deliberate answer rather than a default (R2) — should truncated cycles count against `max-iterations` exactly like unconverged cycles, get a separate bounded allowance, or count only when they made progress?]
- **FR-011**: Repeated truncation MUST change the pipeline's response rather than producing an identical cycle each time. When a cycle is classified truncated and the immediately preceding cycle of the same feature was also truncated, the stage MUST escalate the next attempt to the escalation tier rather than repeating at the same tier. [NEEDS CLARIFICATION: is escalate-on-second-consecutive-truncation in scope for this feature, and where is the consecutive-truncation count carried across separately-dispatched cycles — in the feature's lifecycle record, or somewhere else? The alternative is to ship carry-forward alone and treat re-escalation as a follow-up.]
- **FR-012**: When there is no higher tier to escalate to — the truncated cycles already ran at the escalation tier — the stage MUST still make repeated truncation visible and MUST NOT loop silently at the same tier until the cap.
...
- **FR-018**: Executable coverage MUST drive the stage's shipped classification and convergence decision against synthetic agent run records, and MUST include at minimum: a turn-exhausted run with progress and no convergence commit (asserting truncated **and** not converged); a turn-exhausted run without progress (asserting today's failed path); an ordinary failure (asserting today's failed path); and a normal successful cycle (asserting today's outcome unchanged).
- **FR-019**: Removing the forced not-converged answer of FR-005, removing the no-progress guard of FR-004, or widening truncation to cover ordinary failures MUST each fail a check. Coverage that can only exercise the carry-forward path does not satisfy this requirement.
...
- **Progress evidence**: what distinguishes a truncated cycle that did work from one that burned its budget achieving nothing. It is the guard that keeps carry-forward from consuming the whole iteration budget for free.
...
- **SC-007**: A reader of the lifecycle issue alone can tell a truncated cycle from a failed one and from a normally unconverged one, without opening a run log.
...
- Turn exhaustion usually means "the work was large", not "the tier was too weak" — but not always. That is why repeated truncation, rather than the first truncation, is treated as the signal to escalate (FR-011), and why the no-progress guard (FR-004) escalates immediately when a cycle achieved nothing.
- The lifecycle record is the natural place to carry per-feature cycle state across separately-dispatched cycles; it is already the machine-readable source of truth for a feature's lifecycle and already carries the stage and iteration this feature reads.
- A truncated cycle consumes one iteration of the budget (FR-010) unless clarified otherwise, and the budget therefore remains a bound on total cost rather than on successful cycles only.
