...
def check_action_bump(run_commit_sha: str, workflow_files: list[str]) -> dict | None:
    """NEW (research.md D4). For each workflow_files entry, diffs its
    `uses: owner/action@ref` lines as pinned at run_commit_sha against the
    same file's lines on current main. Returns {workflow_file, action_ref,
    run_pin, main_pin} for the first divergent pin where main's is newer,
    or None if every pin matches or the run's pin is not older."""
