...
| `cited_run` | run URL \| null | issue title/body and trust-filtered comments (wing-commander-issue-context's context-file, #505), read by triage; for a recurred defect the newest watchdog occurrence comment of the loop's own App, and none when the issue was reopened after the run it would cite (#520, contracts/triage.md) | null for a maintainer-filed issue with no cited run (edge case, spec.md) |
| `step` | enum: `triage` \| `route` \| `fix` \| `breach` \| `review` \| `readiness` \| `awaiting-merge` \| `prove` \| `stalled` \| `closed` | Board Item Marker (contracts/board-item-marker.md) + live GitHub state | the resume point; `breach` (#530) is fix's post-push breach with its spec-request not yet filed |
...
| Field | Type | Notes |
|---|---|---|
| `agent_proposal` | enum: `fix` \| `spec` | never widens the backstop's verdict (FR-017). The agent's `category` normalised by `normalize_category()` (stripped, lowercased); any value outside `fix`/`spec` is recorded as `spec` with reason `no_usable_proposal` (#548) |
...
| `proposal_extracted` | boolean | whether a proposal with a `fix`/`spec` category (after `normalize_category()`, #548) was read; stored beside the decision so a default spec is never worded as the agent's judgment (#534) |
