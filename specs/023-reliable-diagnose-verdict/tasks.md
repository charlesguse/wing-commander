...
- [ ] T006 [US1] Validate on a throwaway branch/PR (never `main`) per quickstart.md Scenario 1 (normal run: `Diagnose (retry)` shows `skipped`, unchanged report text, `verify-watchdog-run.sh` still passes) and Scenario 2's retryable-shape fault injection (force a genuine `is_error`/non-`success` terminal result on attempt 1; confirm `retryable=true`, `Diagnose (retry)` runs, and the final outcome reflects whichever attempt succeeded); revert the injected breakage afterward
  - **BLOCKED in this run**: the `gh` CLI (all subcommands, including read-only `gh auth status`) requires interactive approval unavailable in this headless implement stage, so no workflow could be dispatched or inspected. The code change (T002-T005) was verified statically (actionlint, manual trace of the `if:`/`continue-on-error` logic against contracts/watchdog-diagnose-retry-delta.md) but not against a real triggered run. A maintainer with `gh`/Actions access must run this scenario before merging.
...
- [ ] T009 [US2] Validate on the same throwaway branch/PR per quickstart.md Scenario 2's non-retryable-shape fault injection (force a failure before any terminal `result` record, e.g. break `--json-schema`; confirm `retryable=false`, `Diagnose (retry)` is `skipped`, and the report reads "diagnose failed" with single-attempt wording and no added latency) and Scenario 4 (diff `.github/scripts/verify-watchdog-run.sh` before/after — confirm zero changes — and confirm it still fails this run); revert the injected breakage afterward
  - **BLOCKED in this run**: same `gh` CLI unavailability as T006. The verifier-diff half of Scenario 4 was confirmed statically (`git diff --stat -- .github/scripts/verify-watchdog-run.sh` shows zero changes — see T014); the fault-injection/real-run half needs a maintainer with `gh`/Actions access.
...
- [ ] T010 [US3] Fetch the diagnose job log for run 30161188955 via `gh api repos/<owner>/<repo>/actions/jobs/<diagnose-job-id>/logs` to identify which of the four known crash signatures fired (`Action failed with error`, `SDK execution error`, `Workflow initiated by non-human actor`, `json-schema is not valid JSON`); if the run has aged out of log retention, select the most plausible signature to reproduce on a throwaway branch instead, per research.md R3 and quickstart.md Scenario 3 step 1
  - **BLOCKED in this run**: the `gh` CLI is unavailable in this headless implement stage (every `gh` invocation, including read-only `gh auth status`, requires interactive approval this run cannot provide) — the same access boundary research.md R3 already documented at planning time. Static fallback attempted instead: the current `--json-schema` string in the `Diagnose` step's `claude_args` was extracted and validated with `jq` — it parses as valid JSON, and its single-quoting (preserving the embedded double-quoted JSON Schema keys) matches the surrounding comment's documented rationale — so no shell-quoting/escaping bug is evident by static read for the `json-schema is not valid JSON` signature. The `allowed_bots` resolution (`"github-actions,${{ steps.ctx.outputs.bot-slug }}"`, `bot-slug` from `wing-commander-context`'s `create-github-app-token@v1` output) is structurally sound; whether it actually matched run 30161188955's triggering actor cannot be confirmed without the run's log. A maintainer with `gh`/Actions access must fetch the log and confirm which signature fired.
- [ ] T011 [US3] Apply the targeted root-cause fix in `.github/workflows/watchdog.yml` indicated by research.md R3's decision tree for the confirmed signature — depends on T010's finding: no further change needed beyond US1's retry coverage if `SDK execution error`/`Action failed with error`; widen/correct `allowed_bots` (currently `"github-actions,${{ steps.ctx.outputs.bot-slug }}"`) if `Workflow initiated by non-human actor`; or fix the `--json-schema` string's shell-quoting/escaping in the `Diagnose` step's `claude_args` if `json-schema is not valid JSON`
  - **BLOCKED in this run**: no fix applied — deliberately, since T010 could not confirm which signature fired. Guessing and editing `allowed_bots` (a security-relevant actor allowlist) or the schema string without evidence risks an incorrect, unfounded change to `main`-bound workflow config; static analysis found no bug in either as currently written. Once T010 is unblocked and confirms a signature, revisit this task.
- [ ] T012 [US3] Validate on a throwaway branch per quickstart.md Scenario 3 steps 2-3 (re-run or re-inject the confirmed signature after the fix; confirm it no longer occurs, or recovers via US1's retry path if classified retryable) and Scenario 5 (confirm both the no-retry and one-retry cases stay within the new 35-minute job timeout and don't trip `verify-watchdog-run.sh`'s duration-anomaly band)
  - **BLOCKED in this run**: depends on T010/T011, both blocked by the same `gh` CLI unavailability.
...
- [X] T013 [P] Update `docs/architecture.md`'s Stage 9 — Watchdog section to document the bounded one-time retry (`Classify diagnose attempt` and `Diagnose (retry)` steps) and the `diagnose` job's new 35-minute timeout, per plan.md's Project Structure
- [X] T014 Diff `.github/scripts/verify-watchdog-run.sh` against its pre-feature state and confirm zero changes (FR-007, quickstart.md Scenario 4 step 1) — confirmed via `git diff --stat`: zero changes
