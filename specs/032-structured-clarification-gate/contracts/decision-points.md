...
| "Check whether the spec still needs clarification" (`id: clarification`) | `grep -q '\[NEEDS CLARIFICATION' "$SPEC_DIR/spec.md"` → `needed=true\|false` | Reads back the agent step's structured result (`contracts/clarification-schema.md`'s read-back idiom); `needed` = `clarifications` array non-empty. Runs the colon-form cross-check against post-agent `spec.md`; on mismatch, appends the `clarification-mismatch` line to `$GITHUB_STEP_SUMMARY`. `needed` is decided ONLY by the structured array (FR-004) |
...
| "Announce spec PR ready for review" | `if: steps.clarification.outputs.needed == 'false'` | Unchanged — still the negation of the same `needed` output, so it can never be suppressed by an independently-computed condition (closing the #159 mutual-exclusivity failure mode) |
| "Fail on agent API error" | Checks `steps.agent.outcome == 'success'` and a non-empty `.result.is_error` sentinel | Extended per `contracts/clarification-schema.md`'s validation-failure contract: a `success` step outcome whose terminal result is missing the required `clarifications` key, or fails schema validation, is also a failure (FR-002) — folded into this existing step or a new one immediately after it, either satisfies the contract as long as it runs before the clarification/spec-PR-ready steps below execute against a coerced empty array |
...
| "Fail on agent API error" | Same shape as `intake.yml`'s | Same extension as `intake.yml`'s row above, applied to the clarify schema's required keys (`answered`, `clarifications`) |
