...
| implement (⟲ converge) | `implement.post-progress-comment` | `Bash(git log:*),Bash(git diff:*),Bash(git show:*)` (deliberately read-only — the agent composes the summary; a deterministic step posts it) | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage` |
