...
| implement (⟲ converge) | `implement.post-progress-comment` | `Bash(python3 -I .wing-commander-pipeline/.github/scripts/git_read.py:*)` (deliberately read-only — the agent composes the summary; a deterministic step posts it). No raw `git` grant (#518): git log/diff/show take `--output=<path>`, so git goes through the pipeline checkout's `git_read.py` wrapper, which runs only log/diff/show and refuses `--output` | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage,Write,Edit,Bash(git:*),Bash(cd:*),Bash(pushd:*),Bash(popd:*)` |
...
| board-loop | `board-loop.triage-propose` | `Read,Grep,Glob,Bash(python3 -I .github/scripts/git_read.py:*),Bash(cat:*)` (deliberately read-only — board_triage.py, not this step, decides). No `gh issue` grant (#499 code review round 3): the issue is Read from a file `wing-commander-issue-context` (`.github/actions/`) stages, already filtered in code — see that action's own header for the filter rule (specs/029-intake-issue-comments, mirrored from intake.yml). No raw `git` grant (#513): `git log`/`diff`/`show` take `--output=<path>`, which writes a file, so git is reached only through `board_git_read.py` (log/diff/show, `--output` refused) and `Bash(git:*),Bash(cd:*),Bash(pushd:*),Bash(popd:*)` is denied; Gate 93 enforces it. | `WebSearch,WebFetch,Write,Edit,Bash(git:*),Bash(cd:*),Bash(pushd:*),Bash(popd:*),Bash(git push:*),Bash(gh issue close:*),Bash(gh issue edit:*)` |
| board-loop | `board-loop.route-propose` | `Read,Grep,Glob,Bash(python3 -I .github/scripts/git_read.py:*),Bash(cat:*)` (deliberately read-only — board_route_backstop.py, not this step, decides). No `gh issue` grant — same `wing-commander-issue-context` staged file as `triage-propose`. No raw `git` grant — same reason as `triage-propose` (#513). | `WebSearch,WebFetch,Write,Edit,Bash(git:*),Bash(cd:*),Bash(pushd:*),Bash(popd:*),Bash(git push:*),Bash(git commit:*)` |
