...
| board-loop | `board-loop.triage-propose` | `Read,Grep,Glob,Bash(git log:*),Bash(git show:*),Bash(cat:*),Bash(gh issue view:*)` (deliberately read-only — board_triage.py, not this step, decides) | `WebSearch,WebFetch,Write,Edit,Bash(git push:*),Bash(gh issue close:*),Bash(gh issue edit:*)` |
| board-loop | `board-loop.route-propose` | `Read,Grep,Glob,Bash(git log:*),Bash(git diff:*),Bash(git show:*),Bash(cat:*),Bash(gh issue view:*)` (deliberately read-only — board_route_backstop.py, not this step, decides) | `WebSearch,WebFetch,Write,Edit,Bash(git push:*),Bash(git commit:*)` |
| board-loop | `board-loop.fixer` | `Read,Write,Edit,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git log:*),Bash(git diff:*),Bash(git show:*)` | `WebSearch,WebFetch,Bash(git push:*)` — push is deliberately withheld; a deterministic step pushes only after the local gate suite is green (FR-024). No `gh issue view` grant (#499 code review): FR-056 requires comment content reaching the fixer to be filtered in code by author association first, and board-loop has no such trust-gate composite (unlike intake.yml's `comment-trust-gate`) — granting the fixer any `gh issue view` prefix would also authorize `--comments`, which the tool-allowlist mechanism cannot scope down to body-only. Left unfixed pending that design decision; see #499 follow-up. |
| board-loop | `board-loop.reviewer` | `Read,Grep,Glob,Bash(git log:*),Bash(git diff:*),Bash(git show:*),Bash(cat:*),Bash(gh pr view:*),Bash(gh issue view:*)` (deliberately read-only — a context-isolated job, FR-028) | `WebSearch,WebFetch,Write,Edit,Bash(git push:*),Bash(git commit:*)` |
