...
|---|---|---|---|
| intake | `intake` | `Skill,Read,Write,Edit,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git checkout:*),Bash(git switch:*),Bash(git push:*),Bash(git branch:*),Bash(git log:*),Bash(git diff:*),Bash(git show:*),Bash(git ls-tree:*),Bash(echo:*),Bash(ls:*),Bash(mkdir:*),Bash(cat:*),Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*),Bash(printenv SPECIFY_FEATURE_DIRECTORY),Bash(gh issue view:*),Bash(gh issue edit:*),Bash(gh issue comment:*),Bash(gh pr create:*),Bash(gh label create:*)` | `WebFetch,ScheduleWakeup,Monitor,SendMessage` |
| clarify | `clarify` | `Read,Edit,Write,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git push:*),Bash(git log:*),Bash(git diff:*),Bash(cat:*),Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*),Bash(gh issue view:*),Bash(gh issue comment:*),Bash(gh pr list:*),Bash(gh pr view:*),Bash(gh pr edit:*)` | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage` |
| plan | `plan.direct-commit` | `Skill,Read,Write,Edit,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git push:*),Bash(git log:*),Bash(git diff:*),Bash(git show:*),Bash(git ls-tree:*),Bash(git branch:*),Bash(echo:*),Bash(ls:*),Bash(mkdir:*),Bash(cat:*),Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*),Bash(printenv SPECIFY_FEATURE_DIRECTORY),Bash(.specify/scripts/bash/setup-plan.sh:*),Bash(bash .specify/scripts/bash/setup-plan.sh:*),Bash(.specify/scripts/bash/check-prerequisites.sh:*),Bash(bash .specify/scripts/bash/check-prerequisites.sh:*),Bash(gh issue view:*),Bash(gh issue comment:*),Bash(gh pr view:*),Bash(gh pr list:*),Bash(gh issue list:*)` | `WebFetch,ScheduleWakeup,Monitor,SendMessage` |
| plan | `plan.pr` | same as `plan.direct-commit` plus `Bash(git checkout:*),Bash(git switch:*),Bash(gh pr create:*)` | `WebFetch,ScheduleWakeup,Monitor,SendMessage` |
| tasks | `tasks.direct-commit` | `Skill,Read,Write,Edit,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git push:*),Bash(git log:*),Bash(git diff:*),Bash(git show:*),Bash(git ls-tree:*),Bash(git branch:*),Bash(echo:*),Bash(ls:*),Bash(cat:*),Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*),Bash(printenv SPECIFY_FEATURE_DIRECTORY),Bash(.specify/scripts/bash/setup-tasks.sh:*),Bash(bash .specify/scripts/bash/setup-tasks.sh:*),Bash(.specify/scripts/bash/check-prerequisites.sh:*),Bash(bash .specify/scripts/bash/check-prerequisites.sh:*),Bash(gh issue view:*),Bash(gh issue comment:*),Bash(gh pr view:*),Bash(gh pr list:*),Bash(gh issue list:*)` | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage` |
| tasks | `tasks.pr` | same as `tasks.direct-commit` plus `Bash(git checkout:*),Bash(git switch:*),Bash(gh pr create:*)` | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage` |
| implement (⟲ converge) | `implement.cycle` | `Skill,Read,Write,Edit,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git push:*),Bash(git log:*),Bash(git diff:*),Bash(git ls-tree:*),Bash(git branch:*),Bash(echo:*),Bash(git show:*),Bash(ls:*),Bash(cat:*),Bash(grep:*),Bash(head:*),Bash(tail:*),Bash(sort:*),Bash(uniq:*),Bash(wc:*),Bash(cut:*),Bash(yamllint:*),Bash(actionlint:*),Bash(shellcheck:*),Bash(jq:*),Bash(mkdir:*),Bash(.specify/scripts/bash/check-prerequisites.sh:*),Bash(bash .specify/scripts/bash/check-prerequisites.sh:*),Bash(gh issue view:*),Bash(gh issue comment:*),Bash(git rm:*),Bash(python .github/scripts/run-local-gates.py:*)` | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage` |
...
| implement (⟲ converge) | `implement.post-progress-comment` | `Bash(python3 .wing-commander-pipeline/.github/scripts/git_read.py:*)` (deliberately read-only — the agent composes the summary; a deterministic step posts it). No raw `git` grant (#518): git log/diff/show take `--output=<path>`, so git goes through the pipeline checkout's `git_read.py` wrapper, which runs only log/diff/show and refuses `--output` | `WebSearch,WebFetch,ScheduleWakeup,Monitor,SendMessage,Write,Edit,Bash(git:*)` |
...
| watchdog | `watchdog.diagnose` | `Read,Grep,Bash(gh:*),Bash(python3 .wing-commander-pipeline/.github/scripts/git_read.py:*)` (deliberately read-only). No raw `git` grant — same reason as `implement.post-progress-comment` (#518) | `WebSearch,WebFetch,Write,Edit,Bash(git:*),Bash(git commit:*),Bash(git push:*)` † |
| pr-conversation | `pr-conversation.classify` | `Read,Grep,Glob,Bash(python3 .wing-commander-pipeline/.github/scripts/git_read.py:*),Bash(cat:*),Bash(gh pr view:*),Bash(gh issue view:*),Bash(gh search issues:*)` (deliberately read-only). No raw `git` grant — same reason as `implement.post-progress-comment` (#518) | `Write,Edit,WebSearch,WebFetch,Bash(git:*),Bash(git push:*),Bash(git commit:*),ScheduleWakeup,Monitor,SendMessage` |
...
| board-loop | `board-loop.triage-propose` | `Read,Grep,Glob,Bash(python3 .github/scripts/git_read.py:*),Bash(cat:*)` (deliberately read-only — board_triage.py, not this step, decides). No `gh issue` grant (#499 code review round 3): the issue is Read from a file `wing-commander-issue-context` (`.github/actions/`) stages, already filtered in code — see that action's own header for the filter rule (specs/029-intake-issue-comments, mirrored from intake.yml). No raw `git` grant (#513): `git log`/`diff`/`show` take `--output=<path>`, which writes a file, so git is reached only through `board_git_read.py` (log/diff/show, `--output` refused) and `Bash(git:*)` is denied; Gate 93 enforces it. | `WebSearch,WebFetch,Write,Edit,Bash(git:*),Bash(git push:*),Bash(gh issue close:*),Bash(gh issue edit:*)` |
| board-loop | `board-loop.route-propose` | `Read,Grep,Glob,Bash(python3 .github/scripts/git_read.py:*),Bash(cat:*)` (deliberately read-only — board_route_backstop.py, not this step, decides). No `gh issue` grant — same `wing-commander-issue-context` staged file as `triage-propose`. No raw `git` grant — same reason as `triage-propose` (#513). | `WebSearch,WebFetch,Write,Edit,Bash(git:*),Bash(git push:*),Bash(git commit:*)` |
| board-loop | `board-loop.fixer` | `Read,Write,Edit,Glob,Grep,Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(git log:*),Bash(git diff:*),Bash(git show:*)` | `WebSearch,WebFetch,Bash(git push:*)` — push is deliberately withheld; a deterministic step pushes only after the local gate suite is green (FR-024). No `gh issue` grant (#499 code review round 3): the fixer is now covered by FR-056 the same way intake.yml's own fixer-equivalent step is — via `wing-commander-issue-context`'s code-level author-association filter, staged to a file under `/tmp/wing-commander/` (never inside the checkout, so the fixer's own `git add`/`git commit` cannot pick it up), rather than any `gh issue` tool grant that couldn't be scoped to body-only. |
| board-loop | `board-loop.reviewer` | `Read,Grep,Glob,Bash(python3 .github/scripts/git_read.py:*),Bash(cat:*)` (deliberately read-only — a context-isolated job, FR-028). No `gh issue` grant — same `wing-commander-issue-context` staged file as `triage-propose`. No `gh pr` grant (#503): `gh pr view N --comments` returns every PR comment unfiltered (FR-056), so the job's gather step stages the diff, the commit list and the PR's title and body under `/tmp/wing-commander/` and the prompt names those files; the PR conversation is not staged. Gate 93 holds the prompt's paths to the ones the gather step writes. No raw `git` grant — same reason as `triage-propose` (#513). | `WebSearch,WebFetch,Write,Edit,Bash(git:*),Bash(git push:*),Bash(git commit:*)` |
