...
- [X] T052 Fix the `docker login` target derivation in all eleven stage files' `verify-image-prerequisites` steps per FR-009 / US4 AC1 (partial): the step currently does `registry="${IMAGE%%/*}"` unconditionally, which is only a registry host when the reference carries one. For a Docker Hub reference it yields a namespace (`myorg/img:tag` → `docker login myorg`) or the image itself (a bare `img:tag` → `docker login img:tag`); login then fails and the job exits 1, blocking a stage whose image GitHub's own `container.credentials` would have pulled correctly — so an adopter with a private Docker Hub image is turned away by this feature's own pre-check. Treat the first path component as a registry host only when it looks like one (contains `.` or `:`, or is exactly `localhost`); otherwise invoke `docker login` with no server argument so it authenticates against Docker Hub's default. Keep the eleven copies byte-identical to each other (Gate 22/23 discipline), leave the `docker pull "$IMAGE"` reference itself untouched (FR-008 forbids rewriting the adopter's reference — this changes only the login target), and re-run `python3 .github/scripts/run-local-gates.py` plus `actionlint` over every file touched. Done 2026-08-18: replaced the unconditional `registry="${IMAGE%%/*}"` with a `case` that only treats the first path component as a registry host when it contains `.` or `:` or is exactly `localhost`, populating a `login_args` bash array (`("$candidate")` or empty) passed to `docker login "${login_args[@]}" ...` — so a bare/namespaced Docker Hub reference logs in against Docker Hub's default with no server argument. Applied byte-identically to all eleven stage files (confirmed via grep for `login_args=()`, 11/11); `docker pull "$IMAGE"` untouched. `python3 .github/scripts/run-local-gates.py` (20/20 gates, including Gate 22/23) and `actionlint` over all eleven files report zero new diagnostics (only the pre-existing `environment.deployment`/`job_workflow_sha`/unrelated-`ls` findings at shifted line numbers).
- [X] T053 Fix the pull-failure messaging for a half-supplied credential pair in the same eleven steps per FR-010 (partial): login is attempted only when **both** `container-registry-username` and `container-registry-password` are non-empty, but the failure branch reports "no credentials were supplied for this image" only when **both** are empty. With exactly one supplied, no credential is ever sent to the registry, yet the message claims "the registry rejected the supplied credentials or image reference" — the opposite of FR-010's requirement that the message identify the missing credential rather than surface a misleading cause. Add the third branch, naming which of the two secrets is absent, still forwarding the raw Docker error as the other two branches do. Keep the eleven copies byte-identical and re-run the local gate suite and `actionlint` over every file touched. Done 2026-08-18: added two `elif` branches between the existing both-empty and both-supplied cases, each naming the specific missing secret (`container-registry-username was not supplied (container-registry-password was)...` and the symmetric case), still forwarding the raw `docker-pull-error.log` contents. Applied byte-identically to all eleven stage files (confirmed via grep, 11/11); `python3 .github/scripts/run-local-gates.py` (20/20) and `actionlint` over all eleven files report zero new diagnostics.

---

## Phase 11: Convergence

**Purpose**: Close one remaining gap a post-implementation assessment found in T052's own fix: T052 correctly stopped misclassifying a *namespaced* Docker Hub reference's namespace as a registry host, but left the *slash-free* case — the exact "bare `img:tag`" example T052's own task text names as broken — genuinely unfixed.

- [X] T054 Fix the `docker login` target derivation's remaining no-slash gap in all eleven stage files' `verify-image-prerequisites` steps per FR-009a / FR-010 / US4 AC1 (partial — T052 only fixed the namespaced case): `candidate="${IMAGE%%/*}"` returns the *entire* image reference when `$IMAGE` contains no `/` at all (a bare official Docker Hub reference such as `python:3.12`, `alpine@sha256:...`, `ubuntu:24.04`); that whole string almost always contains a `:` (from its own tag or digest), so T052's `case "$candidate" in *.*|*:*|localhost)` still matches `*:*` and treats the bare reference itself as a registry host, producing `docker login python:3.12` — Docker rejects this as an invalid registry endpoint, and the job reports the false "the registry rejected the supplied credentials for $IMAGE" even though the credentials and image are both fine and GitHub's own `container.credentials` on the real job would have pulled it correctly. This is byte-for-byte the same broken invocation T052's own task text names as one of the two cases it was meant to close (`img:tag` → `docker login img:tag`) but the fix as implemented does not close it. Only classify `candidate` as a registry host when `$IMAGE` actually contains a `/` (i.e. `candidate` is a genuine first path component distinct from the rest of the reference) **and** that component looks host-like (contains `.` or `:`, or is exactly `localhost`); a slash-free `$IMAGE` MUST always resolve to no server argument, regardless of what its own tag/digest colon contains. Keep the eleven copies byte-identical to each other (Gate 22/23 discipline), leave `docker pull "$IMAGE"` itself untouched (FR-008), and re-run `python3 .github/scripts/run-local-gates.py` plus `actionlint` over every file touched. Done 2026-08-18: `candidate` is now computed only inside a `case "$IMAGE" in */*) candidate="${IMAGE%%/*}" ;; esac` (defaulting to `""` otherwise), so a slash-free reference always falls into the existing case statement's `*` arm and resolves to no server argument, regardless of what its own tag/digest colon contains; the namespaced case T052 fixed is unchanged. Applied byte-identically to all eleven stage files (confirmed via grep for `candidate=""`, 11/11). `python3 .github/scripts/run-local-gates.py` (20/20 gates, including Gate 22/23) and `actionlint` over all eleven files report zero new diagnostics (only the pre-existing `environment.deployment`/`job_workflow_sha`/unrelated-`ls` findings at shifted line numbers).
