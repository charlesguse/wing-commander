...
**Independent Test**: Drive an implement run whose first step fails, and confirm the lifecycle record is marked stalled, the stall label is applied, and a notice is posted on the lifecycle issue — the same three effects an exhausted-retry stall produces today. Then drive the same failure through a stage that carries no chain state and confirm the notice arrives there too, from the same shared path.
...
6. **Given** a run of any other gated stage — clarify, finalize, intake, pr-conversation, or tasks — that dies at entry, **When** the run ends, **Then** the same notice appears on the lifecycle issue, produced by the same shared mechanism rather than a stage-specific one.
...
6. **Given** a run that declined to proceed for a declared reason — a missing credential, a missing spec-kit skill, a malformed hand-off — **When** the note is posted, **Then** it is the shorter "this stage could not start" note naming what was missing and who can fix it, not the stall notice, and the maintainer can tell the two apart at a glance.

---

### User Story 3 - A refusal is still a refusal, and a healthy run is untouched (Priority: P2)

Nothing that is quiet today becomes noisy, with one declared exception. A duplicate dispatch the guard drops, a run against a closed lifecycle issue, a successful cycle, and a cycle that fails and is already reported — all behave exactly as they do now. A run that declines to proceed for a declared reason gains a short note saying the stage could not start, but its record and its labels are left exactly where they were: a refusal is not a stall.
...
**Independent Test**: Exercise each currently-quiet path in turn — duplicate dispatch, closed lifecycle, successful cycle, exhausted retry — and confirm the record, the labels, and the issue's comments are exactly what they are today; then exercise a declared refusal and confirm the record and labels are likewise untouched while the could-not-start note appears.
...
6. **Given** any run at all, **When** it ends, **Then** at most one notice exists for that run — a stall notice or a could-not-start note, never both and never two of either.
7. **Given** a run that declines to proceed for a declared reason and exits non-zero doing so, **When** the run ends, **Then** the record still reads the stage it read before, no stall label is applied, and the only thing posted is the could-not-start note.
8. **Given** a run that declines to proceed, **When** the treatment is chosen, **Then** it is chosen from a signal the refusing step emitted, not from the absence of an output.
...
6. **Given** one of the six gated stages wired without the shared notice path, or wired to a bespoke condition of its own, **When** the checks run, **Then** a check fails; every call site is covered, not only the implement one.
...
- **A pre-flight refusal that is expressed as a failed run** — missing credentials, a missing spec-kit skill, a malformed hand-off. Today these are silent by the same accident this feature removes. They stay out of the stall path — the record and the labels are left alone, as the refusal contract requires — but they no longer stay silent: the issue gets a short note saying the stage could not start, naming what was missing and who can fix it. See FR-005.
- **A refusing step that emits no signal.** If a step declines to proceed but says nothing that distinguishes it from a crash, it is treated as a crash. The safe default is to speak loudly rather than to infer a refusal from an absent value — inferring from absence is the defect being fixed.
- **A stage that has no bookkeeping job at all.** Five of the six gated stages carry none today. They gain the minimal one this notice needs and nothing further; the notice arriving is the only new behaviour those stages acquire.
...
- **FR-001**: When a gated stage run ends without publishing its outcome — the job terminated abnormally, or never started because something it depends on failed — the stage MUST still mark the specification's lifecycle record as stalled, apply the stall label to the lifecycle issue, and post a notice there. These are the same three effects an exhausted-retry stall produces today for implement.
...
- **FR-004**: Widening when the notice runs MUST NOT make any currently-quiet path noisy, apart from the refusal note FR-005 declares. A duplicate dispatch the idempotency guard skips, a run against a closed lifecycle issue, and a successful cycle MUST each leave the record and the labels untouched and post nothing new.
- **FR-005**: A pre-flight refusal MUST remain distinguishable from an abnormal termination, and each MUST receive its own declared treatment. An abnormal termination MUST produce the three effects of FR-001 — record marked stalled, stall label applied, notice with the restart runbook posted. A refusal — a run that declines to proceed for a declared reason and exits non-zero doing so, such as a missing credential, a missing spec-kit skill, or a malformed spec hand-off — MUST leave the lifecycle record and the stage labels exactly as they were, and MUST post a shorter note stating that the stage could not start, naming what was missing and who can resolve it. Neither treatment may be silent.
- **FR-005a**: The distinction between a refusal and an abnormal termination MUST be carried by a positive signal the refusing step emits. It MUST NOT be inferred from an absent, empty, or unset value, which is the mechanism that produced this defect. A failure that emits no such signal MUST be treated as an abnormal termination.
- **FR-006**: At most one notice MUST be produced per run, on every path — either a stall notice or a could-not-start note, never both and never two of either kind. A run that both recorded a failed outcome and then terminated abnormally MUST NOT produce two.
- **FR-007**: The notice MUST describe the stop that actually occurred. A run that ended before the stage attempted any work MUST NOT be reported as an attempt that failed, MUST NOT name a model tier it never used, and MUST NOT claim an escalation was exhausted. It MUST name where the run stopped and link the failing run.
...
- **FR-012**: Executable coverage MUST exercise the notice path against a modelled stage run that actually failed, and MUST assert that the record was marked and the notice produced. It MUST cover the refusal treatment as well as the abnormal-termination one, and MUST additionally cover the case where the record itself cannot be read or written during an abnormal termination, asserting that the notice is still posted and states the record could not be updated. Coverage whose modelled dependency cannot fail does not satisfy this requirement.
- **FR-013**: Removing the guard that lets the notice survive a dead dependency, narrowing the condition so an abnormally-terminated run can no longer reach the notice, or widening it so a refused, duplicate-dispatch, closed-lifecycle, or cancelled run reaches the stall path, MUST each fail a check. Wiring any of the six stages to a bespoke condition instead of the shared shape MUST likewise fail a check.
...
- **FR-016**: The declared inputs, outputs, and secrets of every affected stage MUST NOT change, and no adopter may need to edit a wrapper workflow to receive this fix.
- **FR-017**: This feature MUST deliver the notice for all six stages that enter through the shared lifecycle gate — clarify, finalize, implement, intake, pr-conversation, and tasks. No gated stage may stop silently: the lifecycle issue is the requester's only view of where a specification stands, so a red run plus the watchdog is not a sufficient record even for a stage that carries no chain state.
- **FR-017a**: The notice path MUST be built once — a single shared shape, re-used at each stage's entry — rather than as six independently authored conditions. The conditions review MUST cover that shared shape plus each of the six call sites.
- **FR-017b**: The five stages that carry no bookkeeping job today MUST gain the minimal one this notice requires and nothing more. No other behaviour of those stages may change as a result of this feature.
...
- **Could-not-start note**: the lighter report a refusal produces — an issue comment naming what was missing and who can resolve it, with no record mark, no label change, and no restart runbook. It says the stage never began; it does not say the specification stalled.
- **Abnormal termination**: a stage run that ends without publishing an outcome, whether because a step failed, because the job never started, or because something it depends on failed.
- **Refusal**: a stage run that declines to proceed for a declared reason, announced by a signal the refusing step emits. It is not a stall; it leaves the record where it was and reports itself through the could-not-start note. Today it is indistinguishable from an abnormal termination because both leave the same absent value behind.
...
- **A general "every job publishes an outcome" convention for the whole fleet.** This covers the entry of the six gate-calling stages, fixed with one shared shape and its own conditions review. It is not a refactor of how every job in every workflow reports.
...
- **Any behaviour of the five non-implement stages beyond the notice.** They gain the minimal bookkeeping the notice requires (FR-017b) and nothing else — no retry, no chain state, no new outputs.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Every gated stage run that ends without recording an outcome leaves a stall notice on the lifecycle issue and a stalled lifecycle record — up from none today, for all six stages.
- **SC-002**: The number of gated stage runs whose lifecycle record still reads "in progress" after the run has ended falls to zero.
...
- **SC-005**: No path that posts nothing today posts anything new, apart from the declared refusal note: duplicate dispatch, closed lifecycle, and successful cycles are byte-for-byte unchanged in what reaches the issue.
- **SC-006**: At most one notice is produced per run on every path — a stall notice or a could-not-start note, never both — including a run that recorded a failed outcome and then terminated abnormally.
- **SC-007**: Removing the guard that lets the notice survive a dead dependency fails a check; narrowing the condition so the new path is unreachable fails a check; widening it so a refused run reaches the stall path fails a check; wiring a stage past the shared shape fails a check; disabling the new coverage fails a check.
- **SC-008**: The change reaches adopters without any wrapper edit, and every affected stage's declared inputs, outputs, and secrets are unchanged.
...
- **SC-010**: A run that refuses to proceed leaves the record at the stage it was already at, applies no stall label, and posts a note that names the missing prerequisite — measured across each declared refusal reason.
- **SC-011**: All six gated stages produce the notice when they die at entry, and the notice for each is produced by the same shared shape, verified by every call site being covered by the same coverage.
...
- Every gated stage already declares, as inputs, everything the notice needs to identify the specification and the issue. Nothing new has to be threaded through a wrapper for the notice to know what it is reporting on, at any of the six call sites.
...
- The distinction between a refusal and an abnormal termination has to be carried by a positive signal, not by the absence of a value. A treatment that depends on a variable being empty is what produced this defect and is not an acceptable mechanism for the fix.
- A refusal is worth a word even though it is not a stall. The lifecycle issue is the requester's only view, so a stage that could not start is reported there — but reporting it must not move the specification's recorded stage, because nothing about the specification changed.
- Building one shared notice shape for all six stages is cheaper to review than six conditions, and a shared shape is what makes a single conditions review sufficient. The cost is that the five non-implement stages acquire a bookkeeping job they do not have today; that job exists only to carry the notice.
