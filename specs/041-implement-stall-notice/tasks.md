...
- [ ] T-MF4: Re-run `.github/scripts/verify-chain-stop-refusal-exclusion.py` (User Story 3) unchanged — it reuses `wc_chain_stop_conditions.CALL_SITES`/`default_needs`/`evaluate`, so it inherits the new conjunct and fixture automatically; confirm it still passes rather than assuming it.
...
- [ ] T-MF6: Do not reword the exhausted-retry stall notice and do not renumber any gate. Verify `python3 .github/scripts/run-local-gates.py` is fully green, in particular Gate 23 and Gate 33.
