...
- [X] T021 [US3] Write `.github/scripts/verify-chain-stop-refusal-exclusion.py` (or extend `wc_shell_harness.py` usage in an existing script): drive `wing-commander-preflight` (T002) with `require-credential: "true"` and both credential inputs empty, forcing its refusal branch; assert `steps.check.outputs.refused == 'true'` and `.reason` non-empty (quickstart.md §4 step 1). Then, using Gate 28's fixture table (built in T025), assert that with `needs.<job>.outputs.refusal-reason` set from this value, every one of the seven survivor-job conditions (T006/T008/T010/T012/T014/T016) evaluates `false` — the refusal and abnormal-termination paths cannot both fire for the same run (quickstart.md §4 step 2, FR-006).
- [X] T022 [US3] Add fixtures asserting FR-004's currently-quiet paths stay byte-for-byte quiet after T006-T016's widening: a duplicate dispatch the idempotency guard skips, a run against a closed lifecycle issue (`Note closed lifecycle and stop`, e.g. `implement.yml:452` equivalent per stage), and a successful cycle — confirm none of the seven survivor-job conditions evaluates `true` for these `needs.*` shapes (extend Gate 28's fixture table, T025, or a standalone script — SC-005).
- [X] T023 [US3] Add a fixture asserting a cancelled run (`needs.*.result` irrelevant, run-level `cancelled`) produces `false` on every one of the seven survivor-job conditions and leaves every in-job refusal-callout step unrun (`!cancelled()`, T006/T008/T010/T012/T014/T016 — FR-009, Edge Cases "The run is cancelled").
- [X] T024 [US3] Add a fixture asserting that on a refusal path, across all six stages, the composite is never invoked (only the in-job `wing-commander-callout` is) — so `spec-meta.json`'s `stage` field and every `stage:*` label are provably untouched (SC-010; combine with T021's harness or keep standalone).
...
## Maintainer Feedback — Gate 23 fix: exclude image-prerequisite failure from the chain-stop notice

- [X] T-MF1: Add `needs.verify-image-prerequisites.result == 'success' &&` as the leading conjunct of all seven survivor-job `if:` conditions: `.github/workflows/implement.yml:1836` (`stalled`), `clarify.yml:932` (`stalled`), `finalize.yml:1015` (`stalled`), `intake.yml:1248` (`stalled`), `pr-conversation.yml:2294` (`stalled`), `tasks.yml:1329` (`stalled`), `tasks.yml:1453` (`stalled-approved`). Do not renumber any gate and do not touch any other arm (the existing `needs.verify-image-prerequisites.result == 'failure'` OR-arm, the `!cancelled()` guard, the exhausted-retry arm, or the refusal-exclusion conjuncts stay as-is). Rationale (per Gate 23, `.github/workflows/lint-workflows.yml` ~L1991): these jobs currently run real steps inside `${{ inputs.container-image }}` — the same image `verify-image-prerequisites` just failed to validate — where `gh`/`jq`/`git` cannot be trusted.

- [X] T-MF2: In `.github/scripts/wc_chain_stop_conditions.py`'s `fixtures_for()`, change the `"upstream dependency failure, entry job skipped"` row's expectation from `True` to `False` (currently ~line 135-136), for every call site — the notice must no longer fire when `verify-image-prerequisites` itself failed.

- [X] T-MF3: In `.github/scripts/verify-chain-stop-notice.py` (Gate 33), add a fifth entry to `MUTATIONS` that strips the new `needs.verify-image-prerequisites.result == 'success' &&` leading conjunct, following the existing `_mut_remove_status_guard`-style pattern, so a future accidental removal of this guard fails the gate (FR-013 / Constitution VIII).

- [X] T-MF4: Re-run `.github/scripts/verify-chain-stop-refusal-exclusion.py` (User Story 3) unchanged — it reuses `wc_chain_stop_conditions.CALL_SITES`/`default_needs`/`evaluate`, so it inherits the new conjunct and fixture automatically; confirm it still passes rather than assuming it.

- [X] T-MF5: Amend `specs/041-implement-stall-notice/research.md` (amend D8 or add a new decision) recording: an image-prerequisite failure is still a stage dying at entry, but it is now deliberately excluded from the chain-stop notice because the survivor job itself runs in the adopter's own (already-failed) container and cannot be trusted to complete there; `verify-image-prerequisites`'s own loud failure is the adopter-facing signal for this specific cause instead. Note explicitly, for traceability, that this narrows spec.md's Acceptance Scenario 3 and the matching Edge Cases bullet ("the dependency one level above the implement job fails ... the notice is posted") to exclude specifically the `verify-image-prerequisites` upstream case — the rest of Acceptance Scenario 3 (a stage's own entry job failing or being skipped for a non-image reason) is unaffected and still fires the notice.

- [X] T-MF6: Do not reword the exhausted-retry stall notice and do not renumber any gate. Verify `python3 .github/scripts/run-local-gates.py` is fully green, in particular Gate 23 and Gate 33.
