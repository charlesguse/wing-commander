...
- What happens when the new version is a major (potentially breaking) release versus a minor or patch release? A major or minor release triggers the deeper end-to-end verification (FR-004) in addition to the lightweight check; a patch release runs only the lightweight check. On passing its applicable verification, any release type — including a major — proceeds to a reviewable version-bump PR, and the human PR review is the adoption gate (FR-014).
...
- **FR-002**: The system MUST run automatically on a recurring **daily** check so that new eligible releases are picked up without manual triggering. It MUST also be possible to trigger it on demand. A freshly published version MUST NOT be adopted the same day it appears while a newer patch of the same minor line is still landing: the process waits until the same-minor patch stream has settled and targets that settled latest release, then adopts on passing verification (no fixed calendar stabilization window). *(Cadence resolved from #153, Q1 option C: check daily; adopt on passing verification, but only once any newer patch of the same minor has also settled. Per the requester's caveat, if the research the process performs — see FR-013 — shows upstream warrants a more conservative pace, that finding is recorded on the lifecycle issue and may justify a longer stabilization window.)*
...
- **FR-004**: The verification step MUST be tiered. It MUST always run a lightweight check that the Spec-Kit-dependent behavior the pipeline relies on still operates — at minimum, that the `.specify/` scripts and spec-kit-driven stages run and produce expected outputs. For minor and major upgrades it MUST additionally run a representative end-to-end check: running at least one real spec-kit-driven stage (e.g. generating a throwaway spec) and confirming it succeeds. A candidate is "working" only if all checks applicable to its release type pass. *(Verification scope resolved from #153, Q2 option C: tiered — lightweight check always; end-to-end check additionally for minor/major upgrades.)*
...
- **FR-014**: The system MUST subject larger upgrades to deeper verification than routine ones: minor and major upgrades additionally undergo the end-to-end verification of FR-004, whereas patch upgrades run only the lightweight check. On passing its applicable verification, every version jump — patch, minor, and major — MAY auto-proceed to a reviewable version-bump pull request; the human review of that PR is the adoption gate (no version type is auto-adopted without human review — see FR-017). Major upgrades therefore receive no separate blocking gate beyond the deeper smoke test and the mandatory PR review. *(Version-scope policy resolved from #153, Q3 option C: all version jumps, including major, auto-proceed to a reviewable PR when verification passes; PR review is the gate. FR-018 still applies — an upgrade that needs more than a version bump is routed to human attention rather than auto-prepared.)*
...
- The requester's #153 reply asked that the version-bump PR be able to **(optionally) auto-merge**. This conflicts with a NON-NEGOTIABLE constitutional rule — a human merges every PR into `main` and the bot never merges (Constitution Principle V), reflected in FR-017 and SC-006. This spec therefore keeps human-merge and does NOT include auto-merge; enabling optional auto-merge for this workflow would require a separate constitution amendment and is out of scope for this feature. This tension is flagged back to the requester rather than silently resolved either way.
