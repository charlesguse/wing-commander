...
- [ ] No base-branch check before merge: `gh pr list --json` at `:729` omits `baseRefName`, and `auto-release-e2e-merge-decision.sh` never checks a base, so a retargeted plan PR could be merged. Require the expected base for each of the three PRs; treat a mismatch as a gate stall naming the reason (FR-023).
- [ ] Read/script failures never become gate stalls: `gh issue view ... || true` (`:695`), `gh pr list ... || echo '[]'` (`:728`), and swallowed decision-script errors (`2>/dev/null || true`) all degrade silently to `fail-timeout` instead of a gate stall or infra verdict. T028 only counts write failures — extend the same bounded-consecutive-failure handling to reads and script crashes.
- [ ] A run a human answers first still fails: the pass assertion at `:894-906` requires a reply from the harness login specifically and returns `fail-wrong-output` when a human answered instead. Per FR-010, accept a human's answer for the clarification assertion (or assert only that the question was answered, not by whom).
- [ ] Merge gates not positively asserted (FR-016/FR-018): the pass-path evidence at `:911-913` falls back to `null` and never fails on a missing or unattributed merge. Assert each of the three PRs was merged, by the harness login (see the username-secret item above), and fail the pass otherwise.
- [ ] `mode` missing from the credential verdicts: the three verdict calls in the `maintainer-credential` step (`:308,321,333`) don't pass `$MODE` (also absent from that step's `env`), so container-mode runs report "(mode: unknown)". Add it.
- [ ] Marker predicate pasted twice — the `[!IMPORTANT]` + phrase jq lives in `auto-release-e2e-clarify-decision.sh:42` and again inline at `auto-release.yml:895`. Per CLAUDE.md's single-home rule, have the workflow call the helper instead of re-typing the predicate.

### Should fix
- [ ] `MAX_CLARIFICATION_ROUNDS` in the workflow env is dead (the script hardcodes 3) — thread it through so the bound is set in one place.
- [ ] The verify script's docstring and output say "Gate 63" while `lint-workflows.yml` registers Gate 66 — fix the mismatched number.
- [ ] The summary line says "clarification: not opened" when `rounds=0` even if a human answered — correct the wording.
- [ ] The whole comments JSON is passed as one argv to the decision scripts, hitting Linux's 128 KB per-argument limit on a long issue — pass it on stdin or as a file instead.

### Verify
- [ ] Confirm a PR waiting on required checks (merge state `BLOCKED`) is not declared a gate stall, since `BLOCKED` is also what pending required checks report and is currently treated as an immediate stall.
