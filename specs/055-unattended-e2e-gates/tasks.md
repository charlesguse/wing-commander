...
- [ ] T011 [US1] Run quickstart.md Scenario 1 against a manually dispatched attempt (`WING_COMMANDER_AUTO_RELEASE_PAUSED` must be cleared first: the switch gates the `detect` job and every job after it, so a dispatch does nothing while it is set, and a run that passes goes on to cut a real release, see quickstart.md prerequisite 4 and #396; clearing it also lets the daily schedule fire, so do it when you are ready to act on a release): confirm the clarification question is answered, all three PRs are merged, and the issue closes `stage:done` with zero human acts, and record the run's evidence per the scenario's step 5 (User Story 1's Independent Test; SC-001, SC-002).
