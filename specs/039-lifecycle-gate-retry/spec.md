...
4. **Given** a successful read that returns a state value the gate does not recognise, **When** the gate runs, **Then** it fails loudly without retrying — an unrecognised answer is an answer, not an absence of one.
...
**Independent Test**: Revert the retry and confirm a check fails; separately, make every failure class retryable and confirm a check fails; then confirm both checks pass on the delivered feature.
...
- **The read succeeds but returns nothing** (a zero-exit call that produces an empty state). Today this is folded into the same failure as a non-zero exit. Whether an empty answer is a transient absence worth retrying or a permanent oddity worth failing on is an open question — see FR-009.
- **The API rejects the read for rate-limiting reasons** rather than a server fault. This is transient in principle but a short bounded backoff may not outlast it; see FR-009 for how it is classified.
- **A successful read returns a state value the gate does not recognise.** Unchanged from today: fail loudly, refuse to guess, and do not retry — the retry exists for an absent answer, not an unexpected one.
...
- **The stage that entered through the gate is `implement`.** When the gate fails outright, the run stops before the chain's bookkeeping resolves, so nothing is recorded and nothing is posted to the lifecycle issue. The retry makes this rarer but cannot make it impossible; whether this feature also addresses the silent stop is an open question — see FR-016.
...
- **FR-001**: The lifecycle gate MUST retry its state read when the read fails for a transient reason — a server-side fault, a timeout, or a dropped or reset connection — rather than failing the stage on a single unsuccessful attempt.
...
- **FR-006**: When the retry budget is exhausted, the reported failure MUST state that the read was retried, how many attempts were made, and what the last attempt reported.
...
- **FR-008**: The gate's behaviour when the read succeeds MUST be unchanged, including its refusal to guess on a state value it does not recognise; an unrecognised value MUST NOT be retried.
- **FR-009**: Every failure the gate can observe MUST fall into exactly one of the two treatments — retried or failed immediately — with no third, undefined path. This includes failures the read does not classify for itself, rate-limit rejections, and [NEEDS CLARIFICATION: a read that exits successfully but yields an empty state — retry it as a transient absence, or fail immediately as an unexplainable answer?]. The chosen default MUST be stated in the gate itself, so a future failure class the gate has never seen has a defined and defensible outcome.
...
- **FR-011**: Executable coverage MUST drive the shipped gate's own step against a stubbed API that fails transiently on its first calls and then succeeds, and MUST assert both that the gate succeeded with the correct state and that more than one read was attempted.
...
- **FR-013**: Reverting the retry, or widening it so that permanent failures are retried, MUST fail a check. Coverage that can only exercise the success path does not satisfy this requirement.
...
- **FR-016**: [NEEDS CLARIFICATION: when the gate ultimately fails, must the entering stage still record its outcome and post a notice to the lifecycle issue — so an exhausted retry does not stop the chain silently — or is that consequence out of scope for this feature?]
...
- **Failure classification**: the decision, made per attempt, of whether an unsuccessful read is worth asking again. It is what separates the retried treatment from the immediate-failure treatment, and what makes the reported error specific.
...
- **SC-006**: Reverting the retry fails a check; widening the retry to cover permanent failures fails a check; removing or disabling the new coverage fails a check.
...
- The read's failures are distinguishable in practice: the underlying API reports server faults, missing resources, and credential problems in text the gate can tell apart, as the source incident's `HTTP 502` line demonstrates. Where a failure cannot be classified, FR-009 governs.
