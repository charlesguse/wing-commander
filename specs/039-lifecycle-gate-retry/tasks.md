...
- [ ] T005 [US2] Extend `.github/scripts/verify-lifecycle-gate-retry.py` (T003) with two budget-exhaustion scenarios (this story's Acceptance Scenarios 4 and 6, spec Edge Case "every attempt fails transiently"):
...
- [ ] T006 [US2] Extend `.github/scripts/verify-lifecycle-gate-retry.py` with message-content assertions layered on top of the not-found and credential-rejected scenarios T007 introduces for User Story 3 — add these assertions in this task if T007 has not yet run, or extend them in place if it has (same file, sequential edit; either order is fine since both scenarios are additive):
...
- [ ] T007 [US3] Extend `.github/scripts/verify-lifecycle-gate-retry.py` with three fast-fail scenarios (this story's Acceptance Scenarios 1, 2, 4):
