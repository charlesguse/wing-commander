...
- [X] T012 Add a transient-then-not-found scenario to `.github/scripts/verify-lifecycle-gate-retry.py`: stub `HTTP 502: 502 Bad Gateway` on call 1 and `Could not resolve to an issue with the number of 184.` on call 2. Assert `rc != 0`, `GH_CALL_COUNT == 2` (stops at the second read, does not retry a third time), and the reported failure is the not-found message (contains "may not exist"), not a budget-exhaustion or transient-warning message per US3/AC3 (missing)
- [X] T013 Add a rate-limited-403 scenario to `.github/scripts/verify-lifecycle-gate-retry.py`: stub `HTTP 403: API rate limit exceeded for installation ID 12345678.` (no scope/authentication wording) on every call. Assert the step retries rather than failing on the first attempt — `GH_CALL_COUNT == 3` and the reported failure is a budget-exhaustion message (recognised-transient or unclassified, not a credential-rejected message naming the token) per Edge Case "The API rejects the read for rate-limiting reasons" and research.md D2 (missing)

---

## Phase 9: Convergence

- [X] T014 Add a scenario to `.github/scripts/verify-lifecycle-gate-retry.py` proving the `sanitize()` rendering research.md D4 specifies actually runs, not merely ships: stub a not-found failure whose diagnostic is longer than 300 characters, carries a literal `%` early enough to survive truncation, and contains embedded `\n`/`\r\n` sequences. Assert `rc != 0`, `GH_CALL_COUNT == 1`, the reported output still contains the "may not exist" wording, contains the `… (truncated)` marker, contains the escaped `%25` for the literal percent sign, and — split on real newline characters — contains no line other than a `::error::`/`::warning::` command, proving the embedded raw newlines never reached the annotation unescaped. Per FR-017, FR-018, research.md D4, and Edge Case "The diagnostic text is very long, multi-line, or contains characters that would break the reported error line" (partial — the behavior is implemented in `action.yml`'s `sanitize()` but no scenario in this feature's own coverage exercises a diagnostic long/multi-line/percent-bearing enough to prove it)
