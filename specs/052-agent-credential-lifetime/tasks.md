...
- [X] Close two remaining Gate 68/69 holes (FR-020, FR-021): (a) require each stage's agent jobs to explicitly call the refresh, agent-ran, and credential-status composites (today, deleting one or pasting the refresh block back under a different name still passes Gates 60/68/69); add the `extraheader`/`set-url origin` idiom to Gate 60 as the one-home check CLAUDE.md asks for. (b) Flag any post-agent read of `env.*` whose value was set from the pre-agent token, and match the additional spellings that currently pass: `fromJSON(...)['token']`, `fromJSON(toJSON(steps.ctx.outputs)).token`, `toJSON(steps.ctx.outputs)`, `steps['ctx']['outputs']['token']`. — done: (a) Gate 68 gained a new per-agent-step composite-call-count check (keyed on `uses:`, immune to a rename, catching deletion and inline-paste alike; `NO_REMOTE_REFRESH_JOBS` exempts the two jobs with no remote to refresh), and Gate 60 gained a new `extraheader-refresh` single-home check for `wing-commander-refresh-remote`'s own idiom. (b) `TOKEN_REF_RE` now covers bracket notation on every segment, `fromJSON(...)` with one level of nested parens and either a dot or bracket token key, and a bare `toJSON(steps.<id>.outputs)` dump. Four new self-test mutations per gate.
...
- [X] T048 Add a `--self-test` entry point to `.github/scripts/verify-credential-relay-shell.py` (Gate 69) that reintroduces both regressions the gate exists to catch — feed check 1 a `$GITHUB_ENV` fixture where the FIRST write wins instead of the last, and feed check 2 the pre-T046 remote-refresh form that only runs `git remote set-url` without first clearing the stale `http.https://github.com/.extraheader` entry — and assert each mutation fails; wire a "Gate 69 self-test" step into `lint-workflows.yml` next to the existing Gate 69 step, following Gate 68's two-step wiring pattern. per FR-023 — done: both checks now take an optional `script=` override so the self-test can drive a mutated copy of the shipped text through the same assertions; `mut_relay_first_write_wins` short-circuits the $GITHUB_ENV write once `WC_BOT_TOKEN` is already set, `mut_refresh_remote_no_extraheader_clear` strips the `unset-all` line back to the pre-T046 form; both mutations fail their check, the clean shipped scripts still pass, and `python .github/scripts/run-local-gates.py` shows 108/109 passed with the one failure (`verify-actionlint.py`) pre-existing at cycle start per this run's own starting-point note, unrelated to this task (a stale untracked `.wing-commander-pipeline/` nested checkout in this sandbox, not a tracked file). FR-023 is now fully satisfied: Gate 68's "job not found" branch already had self-test coverage from the second review round, and Gate 69 now does too.
