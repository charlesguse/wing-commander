...
- **The agent step itself pushes after the credential expires.** The spec-branch checkout authenticates as the bot, so the credential the agent pushes with is the same one that expires at minute sixty. A remedy applied only to the steps *after* the agent leaves the agent's own push exposed, and an agent that cannot push loses the whole cycle's work rather than one comment.
- **The re-established credential is not identical to the first.** Anything captured from the original credential — an authenticated remote already configured in the checkout, a value written to a file or an environment for a later step — continues to reference the expired one unless it is refreshed too.
...
- **FR-001**: Every step in an agent stage that acts as the bot after an agent step MUST use a credential that is valid at the moment that step runs, regardless of how long the agent step ran.
- **FR-002**: The mechanism that establishes credential validity for post-agent steps MUST be [NEEDS CLARIFICATION: which remedy does the owner want? (a) re-establish the credential once after each agent step, with every post-agent step reading the refreshed value — cheapest, leaves the agent's own push credential as it is; (b) have each bot-acting composite establish its own short-lived credential per call — removes the exposure class entirely but widens every composite's declared input surface (Constitution VII: a minor version at the next release) and multiplies mints per run; (c) bound each agent step by wall clock below the credential lifetime — no new mints, but trades the turn ceiling for a time ceiling and changes what "exhausted" means].
...
- **FR-007**: The remedy MUST apply to every stage that runs an agent step followed by a bot-acting step — [NEEDS CLARIFICATION: does the owner want all such stages changed in this feature (intake, clarify, plan, tasks, implement, finalize, pr-conversation, and the auto-update stage's end-to-end arm), or only the stage where the defect was observed, with the rest filed as follow-ups? Changing all of them is one uniform sweep and one review; changing one keeps this PR small but leaves seven stages carrying a known defect, and the durability check of FR-016 cannot be turned on until the sweep completes].
- **FR-008**: The credential an agent step itself pushes with MUST be addressed explicitly: [NEEDS CLARIFICATION: is the agent's own push credential in scope for this feature, or is a long agent's failed push an accepted risk carried to a follow-up? Bringing it in scope closes the case where an entire cycle's work is lost rather than one comment, but reaches into the checkout the agent inherits rather than only the steps around it].
...
- **FR-020**: A deterministic check MUST fail when a bot-acting step after an agent step does not follow the shipped remedy, naming the workflow, the job, and the step.
...
- **FR-025**: An adopting repository MUST require no configuration change to keep working: no new required input, no new required secret, and no change to any existing input, secret, or output name unless the chosen remedy makes one unavoidable, in which case the change MUST be declared as a compatibility surface change with its version implication stated.
...
- **SC-005**: Every agent stage in the repository is covered by the shipped remedy, or its exclusion is recorded with a reason, with zero stages neither covered nor recorded.
...
- **SC-008**: An adopting repository upgrading across this change needs zero configuration edits, or every required edit is named in the release notes with its version implication.
...
- Agent cycles exceeding the credential lifetime are expected and permitted, not a malfunction to be prevented — the turn ceiling is the intended bound. (Remedy (c) in FR-002 is the one option that revisits this assumption, which is why it is a question for the owner rather than a default.)
...
- Establishing a credential is cheap and fast relative to an agent cycle, so the cost of doing it more than once per job is not a reason to prefer one remedy over another.
...
- The three consequences in the source issue are independent: the tolerance fix and the accurate-stall-notice fix are worth shipping on their own merits and remain correct under any of the three remedies in FR-002.

## Out of Scope

- Changing the turn ceiling, the iteration cap, or any budget the implement loop already enforces (except insofar as remedy (c) of FR-002 is selected, which the owner decides).
