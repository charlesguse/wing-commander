...
- [X] **Item 6** (PR conversation, in-scope fix, re-posted after #415 cancelled its original fold leg): `wing-commander-5-implement.yml:99`, `wing-commander-6-finalize.yml:44`, and the `|| false` siblings in `wing-commander-1-intake.yml:37`, `-2-clarify.yml:41`, `-3-plan.yml:56`, `-4-tasks.yml:57,97` pass `findings-filing-enabled: ${{ vars.WING_COMMANDER_<STAGE>_FINDINGS_FILING_ENABLED || true/false }}` straight through to a `type: boolean` `workflow_call` input, unlike the numeric `findings-cap` input one line below on each of those lines, which wraps the same pattern in `fromJSON(... || '3')`. A repository variable set to the literal string `false` does not coerce to the boolean `false` here, so it cannot disable a default-on stage (FR-029 requires each filing knob be "a declared, typed input"). Fix: wrap all seven occurrences as `fromJSON(vars.WING_COMMANDER_<STAGE>_FINDINGS_FILING_ENABLED || 'true')` (implement, finalize) or `... || 'false')` (intake, clarify, plan, tasks x2), matching the `findings-cap` parity pattern exactly. Add one harness or quickstart assertion proving the literal string `false` turns filing off on the `implement` stage. Fixed all seven occurrences to the `fromJSON(...)` parity form. Harness added: `verify-stage-findings-wiring.py` gained `check_wrapper_coercion`/`evaluate_wrapper_filing_enabled`, a structural regression guard failing loudly if any of the six wrapper workflows' `findings-filing-enabled:` line stops matching the exact `fromJSON(vars.<VAR> || '<default>')` form, plus three `--self-test` cases: one proving a reverted bare-`||` fixture is caught, one proving `json.loads('false')`/`json.loads('true')` (the coercion `fromJSON` performs) produce real booleans, and one confirming the real six wrappers pass. Wired into both `--self-test` and the plain run, so it rides Gate 71 with no new gate registration.
