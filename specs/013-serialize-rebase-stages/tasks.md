...
- [X] T008 [US1] (Validated 2026-07-19, scratch spec 990-scratch-a: plan run 29668466048 held `wing-commander-specs/990-scratch-a` 01:22:00–01:24:21Z; rebase run 29668487654's matrix job for the branch sat **pending** the whole time and started 01:24:23Z, two seconds after the stage released the group; the stage's publish (plan PR #65) succeeded and the branch tip was untouched until the queued rebase ran.) Run quickstart.md Scenario 1 against a scratch specification: dispatch `wing-commander-3-plan.yml` (or `-4-tasks.yml`) for `spec/NNN-scratch`, and while it is running, push a commit to the default branch so `wing-commander-rebase.yml`'s `push` trigger fires. Confirm in the Actions UI that the rebase matrix job for this branch is **queued** (not running) for the duration of the stage run, that the stage's publish succeeds, and that the queued rebase then runs and completes normally once the stage releases `wing-commander-specs/NNN-scratch`.

- [X] T009 [US1] (Validated 2026-07-19, scratch spec 991-scratch-b: first plan job 88143309049 ran 01:25:15–01:27:46Z; duplicate dispatch's plan job 88143324970 queued at 01:25:28Z, started 01:27:48Z — two seconds after the first released the group — and then hit the existing duplicate-attempt guard, skipping the agent as designed.) Run quickstart.md Scenario 6 (US1 Edge Case, FR-008): dispatch a stage for a scratch specification, and while it is running, trigger a duplicate dispatch of the same stage for the same specification. Confirm the second run queues behind the first under the same `wing-commander-specs/NNN-scratch` group and only starts once the first completes.
...
- [X] T010 [US2] (Validated 2026-07-19, scratch spec 990-scratch-a, using `wing-commander-3-plan.yml` as the stage: conflicted rebase job in run 29668720766 ran 01:31:50–01:32:02Z; the stage's plan job was created 01:31:53Z mid-rebase, stayed pending, and started 01:32:05Z only after the rebase settled (abandoned, branch untouched), checking out the branch exactly as the rebase left it. Note: the AI conflict-resolution window is short on push-triggered rebases because `claude-code-action` currently rejects the `push` event — see the pre-existing-defect note on issue #53.) Run quickstart.md Scenario 2 against a scratch specification: trigger a rebase that hits a conflict needing AI resolution (keeping the rebase job running), and while it runs, dispatch `wing-commander-4-tasks.yml` for the same specification. Confirm the stage's job appears queued in the Actions UI until the rebase job completes, then starts and checks out the branch exactly as the settled rebase left it.
...
- [X] T011 [P] [US3] (Validated 2026-07-19: while the plan stage for 990-scratch-a was running, rebase run 29668487654's matrix job for 991-scratch-b ran to completion concurrently, 01:23:00–01:23:12Z, and force-pushed the rebased branch — no cross-specification waiting.) Run quickstart.md Scenario 3: set up two scratch specifications A and B; dispatch a rebase-triggering push affecting A's branch and, in the same window, a stage for B. Confirm both runs proceed concurrently in the Actions UI with no measurable added delay.

- [X] T012 [US3] (Validated 2026-07-19, scratch spec 990-scratch-a: with a plan stage holding the group, push A's rebase job 88144441578 queued at 01:40:43Z; push B 20s later queued job 88144462951 and GitHub **cancelled** the older pending job (research.md D4 supersede, observed live); the surviving job started 01:43:03Z — two seconds after the stage released — and brought the branch current with both main-line advances, no manual intervention.) Run quickstart.md Scenario 4: repeat Scenario 1 (T008) to the point where a rebase queues behind a running stage, then immediately dispatch a second rebase-triggering push (or stage dispatch) for the same specification while the first rebase is still queued. Confirm the specification's branch is still brought current — either by the request that survives to run, or by a subsequent manual `gh workflow run wing-commander-rebase.yml` simulating the nightly schedule.

- [X] T013 [P] [US3] (Validated 2026-07-19: uncontended rebase run 29669050024's matrix jobs started 2–3s after creation (runner pickup only, no concurrency queuing) and succeeded; an uncontended stage dispatch's plan job likewise started 4s after creation — matching the spec-008 uncontended baseline.) Run quickstart.md Scenario 5: with no stage or rebase in flight for a scratch specification, advance the default branch and confirm the resulting rebase runs immediately with no queuing; separately, dispatch a stage for a specification with no rebase in flight and confirm it likewise starts immediately (cross-check timing against `specs/008-auto-rebase/quickstart.md` Scenario 1).
